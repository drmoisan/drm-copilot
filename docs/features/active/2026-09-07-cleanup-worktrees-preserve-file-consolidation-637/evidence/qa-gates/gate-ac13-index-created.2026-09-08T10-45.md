# Gate — AC-13, an absent destination index is created and reported as CREATED
...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-13.

RouteSubstitution:
- Plan command (denied in this worktree):
  `pwsh -NoProfile -Command "wsl -d Ubuntu -- bash -lc 'cd /mnt/c/Users/DanMoisan/repos/drm-copilot/.claude/worktrees/agent-a817693674107cbe5 && bats -t -f CREATED tests/shell/test_cleanup_worktrees_preserve.bats tests/shell/test_cleanup_worktrees_preserve_eol.bats'"`
- Substitute: the CI route. The plan's `pwsh`-wrapped WSL form is refused unconditionally in
  ...
  run and reads the TAP lines named below. **`[P5-T7]` stays unchecked in the plan until then.**
...
1. `an absent destination index is created and reported as CREATED`

The plan's targeted form expects exit code 0 and the TAP plan line ``1..1`` for the filter
``CREATED``. Under the full-suite CI substitute the plan line is the whole-suite count instead, so
...
Driven against the `index-absent/` fixture with `CLEANUP_WT_CONSOLIDATION_PATH=/dev` and a
`target_path` of `null`, so the derived index path is `/dev/MEMORY.md`, which does not exist:

    PRESERVE|<wt>|agent-memory/atomic-executor/first-lesson.md|GENUINELY_NEW
    ACTION|preserve-index|/dev/MEMORY.md|CREATED
    rc=0  PRESERVE_EXIT=1

Three things are asserted and all three were observed.

1. The `CREATED` record names the derived index path.
2. `/dev/MEMORY.md` does not exist afterwards. The read-only phase decided; it did not write. The
   test asserts that absence, which also keeps the run from creating a file outside the repository.
3. Index creation contributes to exit 1. The accumulator reads 1 after the phase, which is the
   deliberate choice recorded in the specification: two incompatible index layouts exist in this
   repository, this library does not have the information to choose one and does not synthesize
   frontmatter, so creating an index is surfaced loudly rather than reported as a clean run.

The accumulator is read by re-running the read-only phase outside `run`, because `run` executes in
a subshell and the global would not survive it. Re-running is free of side effects precisely
because the phase performs no writes, which is the property the second assertion establishes.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 306 an absent destination index is created and reported as CREATED
