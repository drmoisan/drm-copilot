...
function assembleTree(scanned) {
    ...
    const matchedChildrenByParentKey = new Map();
    const orphans = [];
    for (const subagent of scanned.subagents) {
...
function buildNode(input) {
    const { key, agentType, description, depth, models, transcriptsByKey, matchedChildrenByParentKey, orphans, } = input;
    ...
    const orderedChildren = isRoot
        ...
        : orderedMatched;
    const children = orderedChildren.map((subagent) => buildNode({
