# mermaid-diagram-claude-runtime (Promoted)
...
repository already emits Mermaid into generated Markdown (the codex-native-converter subagent
topology renderer at `scripts/dev_tools/codex_native_converter/reporting.py` and
`extensions/drm-copilot/src/lib/codex-native-converter/reporting-render.ts`, plus the
`render_subagent_tree` MCP tool), so unvalidated Mermaid can reach committed artifacts.
...
- **Scope containment.** The existing Mermaid emitters (codex-native-converter, `render_subagent_tree`)
