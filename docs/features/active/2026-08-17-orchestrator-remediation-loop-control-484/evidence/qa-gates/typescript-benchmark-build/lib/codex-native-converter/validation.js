...
exports.validateConversionPlan = validateConversionPlan;
 ...
 */
function validateRequiredInputs(runOptions) {
    const findings = [];
 ...
 */
function validateMappingRecords(mappingRecords) {
    const findings = [];
    for (const mappingRecord of mappingRecords) {
        // A required artifact with no safe mapping is a blocking finding.
        if ((mappingRecord.conversionClass === models_1.ConversionClass.UNSUPPORTED ||
            mappingRecord.targetRole === models_1.TargetRole.UNSUPPORTED) &&
            ...
            findings.push(buildFinding({
                ...
                recommendedAction: "Review the artifact manually or add a verified " +
        ...
        for (const note of mappingRecord.notes) {
            for (const [noteFlag, findingCode, findingMessage,] of NOTE_FLAG_TO_FINDING) {
                if (note.includes(noteFlag)) {
                    findings.push(buildFinding({
                        ...
                        recommendedAction: "Resolve the flagged mapping or leave the " +
 ...
 */
function validateDuplicateTargets(mappingRecords, plannedEmissions) {
    const findings = [];
    ...
    for (const plannedEmission of plannedEmissions) {
        ...
        const group = sectionEmissionsByTarget.get(plannedEmission.targetPath) ?? [];
    ...
    for (const mappingRecord of mappingRecords) {
        ...
        targetCounts.set(mappingRecord.targetPath, (targetCounts.get(mappingRecord.targetPath) ?? 0) + 1);
    }
    ...
    for (const [targetPath, records] of mappingRecordsByTarget) {
        if (targetPath === "AGENTS.md" &&
            records.every((record) => record.targetRole === models_1.TargetRole.STANDING_GUIDANCE)) {
    ...
    for (const mappingRecord of mappingRecords) {
        if (mappingRecord.targetPath !== null &&
            ...
            findings.push(buildFinding({
                ...
                sourcePath: mappingRecord.sourcePath,
                targetPath: mappingRecord.targetPath,
                message: "Multiple source artifacts resolve to the same target " + "path.",
                recommendedAction: "Refine the mapping plan so each target path has " +
    ...
    for (const [targetPath, sectionEmissions] of sectionEmissionsByTarget) {
        const distinctPairs = new Set(sectionEmissions.map((emission) => `${emission.sourcePath}\u0000${emission.targetRole}`));
    ...
    for (const targetPath of [...conflictingTargets].sort((left, right) => left < right ? -1 : left > right ? 1 : 0)) {
        for (const plannedEmission of sectionEmissionsByTarget.get(targetPath) ??
            ...
            findings.push(buildFinding({
                ...
                targetPath,
                message: "Multiple planned emissions resolve to the same target path.",
                recommendedAction: "Refine the section-level mapping plan so each target path has " +
 ...
 */
function validateGeneratedOutput(generatedOutput) {
    const findings = [];
    ...
    for (const targetPath of Object.keys(generatedOutput)) {
        ...
        const unresolvedReferences = (0, rewrites_1.detectUnresolvedRuntimeReference)(renderedText);
        if (unresolvedReferences.length > 0) {
            findings.push(buildFinding({
                ...
                sourcePath: null,
                targetPath,
                message: "Generated output retains unresolved source-runtime " +
                    "references: " +
                    unresolvedReferences.join(", "),
                recommendedAction: "Add a verified rewrite or adjust the generated " +
 ...
 */
function validateConversionPlan(runOptions, mappingRecords, plannedEmissions, generatedOutput) {
    const findings = [
