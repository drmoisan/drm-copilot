...
/**
 * The resolved C# toolchain variant chosen by the user, or `undefined` when no
 * variant has been resolved.
 */
export type CsharpVariant = "modern" | "legacy" | undefined;
 ...
 */
export function translateSelectedPackNames(
  ...
  if (csharpVariant === undefined) {
    throw new Error(
      "Cannot translate the 'csharp' pack name: the C# toolchain variant is " +
  ...
  return packs.map((pack) =>
    pack === CSHARP_PACK_NAME ? translatedName : pack,
  );
}
