...
.DESCRIPTION
    Drives .claude/hooks/hook-command-invocation.ps1 directly. Every case in this file is a
...
Describe 'hook-command-invocation (issue #545 D2 Piece 3 and D12)' {
    BeforeAll {
    ...
    Context 'D12 public parser contract' {
        BeforeAll {
            ...
            function Assert-DeclaredOutputType {
                <#
                .SYNOPSIS
                    Assert that a function declares exactly one OutputType, of the given type.
                #>
                param(
                    [Parameter(Mandatory)][string] $Name,
                    [Parameter(Mandatory)][type] $Expected
                )
                $command = Get-Command -Name $Name -CommandType Function
                @($command.OutputType).Count | Should -Be 1
                $command.OutputType[0].Type | Should -Be $Expected
            }
        }

        # These signatures are the contract epic children D and G consume, so they are pinned
        # by test rather than by prose. A child that changes a parameter name or an OutputType
        # breaks this case rather than breaking a caller silently.
        It 'pins the Test-CommandLineInvocation parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Test-CommandLineInvocation') -join ',' |
                Should -Be 'CommandText,CommandWord,SubcommandPath'
            Assert-DeclaredOutputType -Name 'Test-CommandLineInvocation' -Expected ([bool])
        }

        It 'pins the Get-CommandLineOperand parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Get-CommandLineOperand') -join ',' |
                Should -Be 'CommandText,CommandWord,SubcommandPath'
            Assert-DeclaredOutputType -Name 'Get-CommandLineOperand' -Expected ([string[]])
        }

        It 'pins the Get-CommandLineFlagValue parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Get-CommandLineFlagValue') -join ',' |
                Should -Be 'CommandText,CommandWord,FlagName,SubcommandPath'
            Assert-DeclaredOutputType -Name 'Get-CommandLineFlagValue' -Expected ([string])
        }

        It 'pins the Test-CommandLineFlag parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Test-CommandLineFlag') -join ',' |
                Should -Be 'CommandText,CommandWord,FlagName,SubcommandPath'
            Assert-DeclaredOutputType -Name 'Test-CommandLineFlag' -Expected ([bool])
        }

        It 'pins the Test-CommandLineMention parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Test-CommandLineMention') -join ',' |
                Should -Be 'CommandText,CommandWord,SubcommandPath'
            Assert-DeclaredOutputType -Name 'Test-CommandLineMention' -Expected ([bool])
        }
    }
}
