...
const orchestrator_state_remediation_legacy_1 = require("./orchestrator-state-remediation-legacy");
...
function validateOrchestratorStateText(text, options = {}) {
    ...
    errors.push(...(0, orchestrator_state_remediation_legacy_1.validateLegacyRemediationState)(stateMap, strictRemediation));
    ...
    if (flags.requirePrCreationReady) {
        errors.push(...(0, orchestrator_state_completion_1.validateOrchestratorStatePrCreationReadiness)(stateMap));
    }
    ...
    if (flags.requireModelRouting) {
        errors.push(...(0, orchestrator_state_model_routing_existence_1.validateModelRoutingExistence)(stateMap).map((error) => `${LEGACY_ROUTING_GATE_ERROR}: ${error}`));
    }
    errors.push(...(0, orchestrator_state_codex_model_routing_1.validateCodexModelRoutingState)(stateMap, flags.requireCodexModelRouting));
    errors.push(...(0, orchestrator_state_codex_topology_1.validateCodexTopologyState)(stateMap, flags.requireCodexTopology));
    return errors;
}
