...
- [x] **AC-15 (`enforce-prd-feature-before-planner.ps1` work-mode-aware prerequisites, unblocking #501's own remediation).** `tests/scripts/claude-hooks/enforce-prd-feature-before-planner.Tests.ps1` exists and passes, including named tests asserting: (a) for a resolved feature folder whose `issue.md` persists `- Work Mode: full-feature`, the hook requires both `spec.md` and `user-story.md` and emits `PRD_FEATURE_BLOCKED` naming only the files actually missing; (b) for `- Work Mode: full-bug`, the hook requires `spec.md` only, and does not block on a folder that has `spec.md` but no `user-story.md`; (c) for `- Work Mode: minor-audit`, the hook requires neither file and does not block a folder containing only `issue.md`; (d) when the `- Work Mode: ...` marker is absent from `issue.md`, unreadable (`issue.md` missing or unopenable), or carries a value outside `{minor-audit, full-feature, full-bug}`, the hook fails closed to the strictest set (`spec.md` and `user-story.md` both required) and emits a `PRD_FEATURE_BLOCKED` reason that distinguishes "work mode could not be determined" from "prerequisite file missing" — the two conditions must not share one reason string. Pre-fix, the hook hardcodes `$required = @('spec.md', 'user-story.md')` unconditionally (`.claude/hooks/enforce-prd-feature-before-planner.ps1:141`), so sub-checks (b) and (c) fail against unfixed code: a `full-bug` or `minor-audit` folder lacking `user-story.md` by design is blocked exactly as this feature (#501) itself was. State explicitly in the test suite's descriptive text that failing open on an undeterminable work mode (treating it as satisfying every mode's requirement) would reintroduce the defect class #501 corrected — a `PreToolUse` gate that appears to enforce but always allows. Mirror parity: cited via the existing `test_bundled_claude_payload_contains_all_repo_runtime_contracts`, not a new mechanism — the mirrored copy of `enforce-prd-feature-before-planner.ps1` under `extensions/drm-copilot/resources/claude-customizations/.claude/hooks/` must stay byte-identical to the original, verified by that existing test. Coverage: PoshQC Pester line coverage >= 85% for `.claude/hooks/enforce-prd-feature-before-planner.ps1`, with no branch-coverage criterion (Pester does not measure branch coverage) and no coverage exclusion added for this file. Verify: `mcp__drm-copilot__run_poshqc_test` (or `Invoke-Pester -Path tests/scripts/claude-hooks/enforce-prd-feature-before-planner.Tests.ps1`). Scope boundary held: this criterion does not reopen AC-13 — `.codex/hooks/` and the eight SubagentStop validators remain untouched; only this one PreToolUse hook's prerequisite-selection logic is in scope.

## Scope Expansion Recorded at Execution (AC-15, issue #501)

AC-15 is not original scope. It was added mid-remediation after making the `PreToolUse` hooks live (the fix this feature delivers) exposed a pre-existing, independent defect in `.claude/hooks/enforce-prd-feature-before-planner.ps1`: the hook hardcodes its prerequisite file list to `spec.md` plus `user-story.md` with no awareness of the persisted `- Work Mode: ...` marker, contradicting the mode-aware expectations documented in `.claude/skills/feature-promotion-lifecycle/SKILL.md` ("Mode-aware expectations"), under which `user-story.md` is expected only for `full-feature` and should be absent for `full-bug` and `minor-audit`.

This defect predates #501 and is unrelated to the payload-transport-and-shape defect AC-1 through AC-14 correct. It was unreachable while every `PreToolUse` hook failed open (the original defect this feature fixes); once the hooks became live, the hardcoded prerequisite list started blocking every `full-bug` and `minor-audit` `atomic-planner` delegation whose folder correctly lacks `user-story.md` — including this feature's own remediation cycle, which is a `full-bug` item. The user was presented with the resulting options and explicitly chose to fix `enforce-prd-feature-before-planner.ps1` within this branch rather than deferring it to a separate issue. AC-15 records that authorized decision. It is scope expansion, not scope creep: it was requested and approved, not self-assigned.

AC-1 through AC-14 remain the complete and already-delivered record of the original transport/shape fix; none of them describes or covers `enforce-prd-feature-before-planner.ps1`, which reads its payload correctly and was never part of the transport/shape defect class.
