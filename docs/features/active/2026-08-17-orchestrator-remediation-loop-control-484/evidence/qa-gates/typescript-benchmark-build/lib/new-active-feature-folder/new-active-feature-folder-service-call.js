 ...
 */
function requireReportedPathExists(fileSystem, path) {
    if (!fileSystem.exists(path)) {
        throw new Error(`new_active_feature_folder reported a path that does not exist: ${path}`);
    }
}
function newActiveFeatureFolderServiceCall(input) {
    ...
    const fileSystem = input.fileSystem ?? new index_1.RealFolderFileSystem();
