...
The `atomic-executor` subagent at [.codex/agents/atomic-executor.toml](.codex/agents/atomic-executor.toml) enforces the persistent execution-mode behaviors (anti-replanning, preflight-only blocking, persistence across turns, resume) and preloads the shared skills that support them:
