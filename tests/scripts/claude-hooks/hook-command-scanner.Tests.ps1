...
.DESCRIPTION
    Drives .claude/hooks/hook-command-scanner.ps1 directly. Every case in this file is a
...
Describe 'hook-command-scanner (issue #545 D2 Piece 1 and Piece 2)' {
    BeforeAll {
    ...
    Context 'D12 public parser contract' {
        ...
        It 'pins the Read-CommandLineSegment parameter list and OutputType' {
            (Get-DeclaredParameterName -Name 'Read-CommandLineSegment') -join ',' | Should -Be 'CommandText'
            $command = Get-Command -Name 'Read-CommandLineSegment' -CommandType Function
