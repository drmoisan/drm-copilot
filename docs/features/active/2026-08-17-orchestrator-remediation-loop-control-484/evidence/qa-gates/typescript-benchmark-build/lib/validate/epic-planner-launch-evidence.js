...
exports.validateEpicPlannerLaunchEvidence = validateEpicPlannerLaunchEvidence;
...
function launchPath(value, context, field) {
    ...
    let normalized = (0, file_system_1.toPosixPath)(value);
...
function readJson(relative, context, label) {
    const path = `${(0, file_system_1.toPosixPath)(context.workspaceRoot).replace(/\/+$/, "")}/${relative}`;
    ...
    try {
        ...
        return {
            text,
            errors: [
                `Execution readiness ${label} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
            ],
        };
    }
    if (!isRecord(value)) {
...
function validateCompletedReceipt(receipt, prefix) {
    ...
    const bound = parseTimestamp(receipt["session_bound_at"], prefix, "session_bound_at", errors);
    const completed = parseTimestamp(receipt["completed_at"], prefix, "completed_at", errors);
...
function validateSpec(receipt, expected, context, prefix) {
    const specPath = launchPath(receipt["spec_path"], context, `${prefix}.spec_path`);
    const errors = [...specPath.errors];
    if (specPath.relative === undefined) {
        return errors;
    }
    const receiptPath = launchPath(receipt["receipt_path"], context, `${prefix}.receipt_path`);
    if (receiptPath.relative !== undefined &&
    ...
    const spec = readJson(specPath.relative, context, `${prefix} launch specification`);
    ...
    if (!isNonEmpty(digest) || !SHA256_RE.test(digest)) {
        ...
        (0, node_crypto_1.createHash)("sha256").update(spec.text, "utf8").digest("hex") !== digest) {
    ...
    const matching = Array.isArray(launches)
        ? launches.filter((item) => isRecord(item) && item["launch_id"] === receipt["launch_id"])
        : [];
    if (matching.length !== 1) {
        errors.push(`${prefix}.launch_id must identify exactly one specification launch.`);
    }
    else {
        ...
        if (launch !== undefined) {
            for (const [key, value] of Object.entries(expected)) {
                if (launch[key] !== value) {
                    errors.push(`${prefix} specification launch.${key} must match the feature.`);
                }
            }
        }
    }
...
function validateReceipt(feature, index, context) {
    const prefix = `Epic planner checkpoint features[${index}] launch receipt`;
    const path = launchPath(feature["launch_receipt_path"], context, `${prefix} path`);
    ...
    const statusPath = launchPath(receipt["status_path"], context, `${prefix}.status_path`);
    errors.push(...statusPath.errors);
    const featureStatus = launchPath(feature["launch_status_path"], context, `${prefix}.status_path`);
    if (statusPath.relative !== undefined &&
        statusPath.relative !== featureStatus.relative) {
        errors.push(`${prefix}.status_path must match the feature status file.`);
    }
    errors.push(...validateCompletedReceipt(receipt, prefix), ...validateSpec(receipt, expected, context, prefix));
...
function validateStatus(feature, index, receipt, receiptPath, status, statusPath, context) {
    ...
    const launchId = receipt["launch_id"];
    const entry = isRecord(launches) && typeof launchId === "string"
        ? launches[launchId]
        : undefined;
    if (!isRecord(entry)) {
    ...
    const entryReceipt = launchPath(entry["receipt_path"], context, `${prefix} launch.receipt_path`);
    errors.push(...entryReceipt.errors);
    if (entryReceipt.relative !== undefined &&
        entryReceipt.relative !== receiptPath) {
        errors.push(`${prefix} launch.receipt_path must match the feature receipt.`);
    }
    ...
    if (statusPath.slice(0, statusPath.lastIndexOf("/")) !==
...
function validateEpicPlannerLaunchEvidence(state, context) {
    ...
    features.forEach((item, index) => {
        ...
        const featureStatus = launchPath(item["launch_status_path"], context, `Epic planner checkpoint features[${index}] launch status path`);
        errors.push(...featureStatus.errors);
        if (featureStatus.relative !== undefined &&
            statusPath !== undefined &&
            featureStatus.relative !== statusPath) {
            errors.push("Epic planner preparation features must share one launch_status_path.");
        }
        if (featureStatus.relative !== undefined && statusPath === undefined) {
            statusPath = featureStatus.relative;
            const result = readJson(statusPath, context, "the shared preparation launch status");
            errors.push(...result.errors);
            status = result.value;
        }
        if (receipt.receipt !== undefined &&
            ...
            status !== undefined &&
            statusPath !== undefined) {
            errors.push(...validateStatus(item, index, receipt.receipt, receipt.path, status, statusPath, context));
        }
    });
    return errors;
}
