# [P2-T6] B-1 and B-3 Python — pass-after evidence
...
Discovery found 35 tests in 148ms.
Starting code coverage.
Running tests.
[+] C:\Users\DanMoisan\repos\drm-copilot\.claude\worktrees\agent-add102e7ba6e997d5\tests\scripts\claude-hooks\enforce-python-batch-budget.Tests.ps1 861ms (408ms|324ms)
...
The [P0-T11] baseline passed count was `32`. The observed passed count of `35` is exactly 3 greater,
the increment being the three tests added by [P2-T2]. The failed count is 0.
...
All three `It` titles are present among the `testcase` names, each with no `failure` child:

```
enforce-python-batch-budget.ps1.session identity, containment, and rehydrate filter.discards an absolute candidate path in a sibling directory whose name extends the root | failureChild=False
enforce-python-batch-budget.ps1.session identity, containment, and rehydrate filter.admits a candidate path that is exactly the resolved root | failureChild=False
enforce-python-batch-budget.ps1.session identity, containment, and rehydrate filter.falls through to the worktree-derived id when the session-id file is unreadable | failureChild=False
```

The `discards an absolute candidate path in a sibling directory whose name extends the root` case
carried a `failure` child in the [P2-T3] fail-before run and carries none here, which is the
fail-before / pass-after pair for B-1 in the Python hook.
...
| --- | --- | --- | --- |
| [P0-T11] baseline | 121 | 8 | 93.8 |
| [P2-T6] this run | 123 | 6 | 95.3 |
...
The `.claude/hooks/enforce-powershell-batch-budget.ps1` row reports `covered=0 missed=129` and is
recorded but not asserted: this was a scoped run of the Python suite only, so that row describes a
file no executing test touched. That file's numbers were asserted by [P1-T6], which recorded
`covered=123 missed=6` for it under its own scoped run.
...
The two asserted rows are the Python ones, per the plan's rule that each Form D task asserts only the
rows belonging to the hook whose suite that task just ran:

| File | Line | Statement | [P0-T11] before | [P2-T6] after |
| --- | --- | --- | --- | --- |
| `.claude/hooks/enforce-python-batch-budget.ps1` | 151 | `Write-Verbose "Ignoring unreadable session-id file ..."` | `mi=2 ci=0` | `mi=0 ci=2` |
| `.claude/hooks/enforce-python-batch-budget.ps1` | 152 | `$fromFile = ''` | `mi=1 ci=0` | `mi=0 ci=1` |

Both rows report `mi=0` with `ci` greater than 0, which is the pass-after half of the B-3 evidence
pair for the Python hook. The line numbers remain valid because [P2-T4] preserved the file at 454
lines.

The two PowerShell rows report `ci=0` here only because the PowerShell suite did not execute in this
scoped run. Their pass-after values were recorded by [P1-T6] as `line=154 mi=0 ci=2` and
`line=155 mi=0 ci=1`.
