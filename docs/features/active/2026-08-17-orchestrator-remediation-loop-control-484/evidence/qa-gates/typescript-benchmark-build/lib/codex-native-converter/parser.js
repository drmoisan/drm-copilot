...
exports.parseSourceArtifact = parseSourceArtifact;
...
const HARD_GATE_PATTERN = /\b(you MUST|MUST(?! NOT)|REQUIRED|SHALL(?! NOT)|is required|non-negotiable)/m;
const FORBIDDEN_PATTERN_RE = /\b(MUST NOT|do NOT|NEVER|is forbidden|is prohibited|SHALL NOT|you must not)/im;
const LAUNCHER_WRAPPER_PATTERN = /(```[\s\S]*?```|poetry run|node |npx |pwsh |bash |cmd\.exe|Resolve.*Prompt|launch)/m;
const TOOL_REQUIREMENT_PATTERN = /(mcp__|\btools:\b|\buses:\b|\bwith:\b|\btool_call|function_call|tool_definitions|allowed-tools)/m;
/**
 ...
 */
function detectCues(heading, content) {
    const cues = [];
    if (heading.trim() !== "") {
        cues.push({ kind: models_1.SemanticCueKind.HEADING, value: heading });
    }
    const numberedMatch = NUMBERED_WORKFLOW_PATTERN.exec(content);
    if (numberedMatch) {
        cues.push({
            kind: models_1.SemanticCueKind.NUMBERED_WORKFLOW,
    ...
    if (hardGateMatch) {
        cues.push({
            kind: models_1.SemanticCueKind.HARD_GATE,
    ...
    if (forbiddenMatch) {
        cues.push({
            kind: models_1.SemanticCueKind.FORBIDDEN_PATTERN,
    ...
    if (launcherMatch) {
        cues.push({
            kind: models_1.SemanticCueKind.LAUNCHER_WRAPPER,
    ...
    if (toolMatch) {
        cues.push({
            kind: models_1.SemanticCueKind.TOOL_REQUIREMENT,
 ...
 */
function buildSection(sourcePath, lines, heading, level, startLine, endLine) {
    const bodyText = rstrip(lines.slice(startLine - 1, endLine).join("\n"));
    const sectionStem = heading
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "") || "body";
 ...
 */
function splitSections(sourcePath, rawText, contentStartLine) {
    ...
    for (let lineNumber = contentStartLine; lineNumber <= lines.length; lineNumber += 1) {
        const headingMatch = HEADING_PATTERN.exec(lines[lineNumber - 1] ?? "");
        if (headingMatch === null) {
            continue;
        }
        if (lineNumber > currentStartLine) {
            const pendingSection = buildSection(sourcePath, lines, currentHeading, currentLevel, currentStartLine, lineNumber - 1);
    ...
    if (currentStartLine <= lines.length) {
        const pendingSection = buildSection(sourcePath, lines, currentHeading, currentLevel, currentStartLine, lines.length);
        if (pendingSection.content.trim() !== "") {
            sections.push(pendingSection);
        }
    }
 ...
 * @throws Error When the source file cannot be read.
 */
function parseSourceArtifact(fileSystem, sourceRoot, sourcePath, sourceEcosystem, sourceKind) {
