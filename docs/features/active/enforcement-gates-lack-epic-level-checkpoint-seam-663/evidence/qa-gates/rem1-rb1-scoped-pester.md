# Remediation Cycle 1 RB1 Scoped Pester Verification ([P1-T7])
...
EXIT_CODE: 0
Output Summary: Resolved list: 16 entries. PassedCount 643 ([P0-T7] 631 plus 12), FailedCount 0, FailedBlocksCount 0, FailedContainersCount 0. Each of the six "issue #663 remediation denies" names on exactly two PASSED lines; the #663 exempts trailer row, D4 row 18, and LACS allow 3 each on two PASSED lines; both Parity names present.

## Name-Count Check (lines matching exactly)

```
2 | PASSED: issue #663 remediation denies an escaped double quote hiding an output redirection (CR-1)
2 | PASSED: issue #663 remediation denies an escaped double quote hiding an output redirection after a semicolon (CR-1)
2 | PASSED: issue #663 remediation denies an escaped double quote hiding chain operators (CR-3)
2 | PASSED: issue #663 remediation denies an unquoted escaped double quote opening a scan-only span
2 | PASSED: issue #663 remediation denies an unquoted escaped single quote opening a scan-only span
2 | PASSED: issue #663 remediation denies a backslash inside a double-quoted message
2 | PASSED: issue #663 exempts a double-quoted message carrying a Co-Authored-By trailer and an apostrophe
2 | PASSED: allows a backslash-spelled operand after separator normalization (D4 row 18)
2 | PASSED: allows issue #671 LACS allow 3 - backslash-spelled absolute selector normalized before the rooting test
1 | PASSED: keeps all four surface copies of the helpers module byte-identical by SHA256 hash
1 | PASSED: keeps every surface copy of the helpers module under the 500-line cap
```

PassedCount 643 equals the [P0-T7] PassedCount 631 plus 12.
...
Discovery found 643 tests in 537ms.
Running tests.
