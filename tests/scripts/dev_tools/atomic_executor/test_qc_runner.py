...
class TestQCRunnerInit:
    ...
    def test_init_stores_workspace(self, mem_fs_path: Path) -> None:
        """__init__() stores workspace path."""
        runner = QCRunner(mem_fs_path)
        assert runner.workspace == mem_fs_path
...
class TestQCRunnerGitHasChanges:
    ...
    def test_git_has_changes_ignores_artifacts(
        ...
        has_changes = runner._git_has_changes(  # pyright: ignore[reportPrivateUsage]
            exclude_paths=[
                mem_fs_path / "artifacts/ck12_catalog_baseline_black.txt",
                mem_fs_path / "artifacts/ck12_catalog_baseline_ruff.txt",
            ]
        )
    ...
    def test_git_has_changes_reports_non_artifact_changes(
        ...
        has_changes = runner._git_has_changes(  # pyright: ignore[reportPrivateUsage]
            exclude_paths=[mem_fs_path / "artifacts/ck12_catalog_baseline_black.txt"]
        )
...
class TestQCRunnerFullLoop:
    ...
    def test_full_loop_completes_when_black_changes_nothing(
        ...
        result = runner.run_full_loop_with_artifacts(
            artifact_paths={
                "black": mem_fs_path / "artifacts/black.txt",
                "ruff": mem_fs_path / "artifacts/ruff.txt",
                "pyright": mem_fs_path / "artifacts/pyright.txt",
                "pytest": mem_fs_path / "artifacts/pytest.txt",
            },
            max_loops=1,
        )
...
class TestQCRunnerFilterHelpers:
    ...
    def test_filter_python_files_keeps_py_only(self, mem_fs_path: Path) -> None:
    ...
    def test_filter_python_files_returns_empty_for_no_py(
    ...
    def test_filter_test_files_keeps_tests_only(self, mem_fs_path: Path) -> None:
    ...
    def test_filter_test_files_requires_py_extension(self, mem_fs_path: Path) -> None:
...
class TestQCRunnerEdgeCases:
    ...
    def test_run_helper_passes_cwd_to_subprocess(
        ...
        assert captured_kwargs["cwd"] == mem_fs_path
