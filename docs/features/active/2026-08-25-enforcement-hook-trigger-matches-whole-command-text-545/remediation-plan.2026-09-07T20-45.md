...
- [ ] [P0-T9] Record the per-file PowerShell coverage baseline. Transcribe the seven
...
- [ ] [P1-T1] Open batch A. First run `ls -1 .claude/state/` and record the listing. Record every
...
- [ ] [P2-T1] Open batch B with a budget reset, using the `[P1-T1]` procedure. Write
      `evidence/qa-gates/batch-b-budget-reset.<capture-timestamp>.md`.
      **Acceptance:** as `[P1-T1]`.

- [ ] [P2-T2] `[expect-fail]` Add the Claude decision-surface cases to
...
- [ ] [P2-T3] `[expect-fail]` Add the Codex decision-surface cases to
...
- [ ] [P2-T6] Apply Edit 3 to `.codex/hooks/enforce-epic-merge-gate.ps1`.
...
- [ ] [P2-T7] Mirror both merge-gate copies and verify parity:
...
- [ ] [P2-T8] Record the pass-after state. Re-run the `[P2-T4]` command and write
...
- [ ] [P2-T9] Run the batch-B toolchain gate with the `[P1-T8]` procedure and acceptance, over the
...
- [ ] [P2-T10] Close batch B with a budget reset, using the `[P1-T1]` procedure. Write
      `evidence/qa-gates/batch-b-close-reset.<capture-timestamp>.md`.
      **Acceptance:** as `[P1-T9]`.

### Phase 3 — Batch C: R-2.b and R-2.c

- [ ] [P3-T1] Open batch C with a budget reset, using the `[P1-T1]` procedure. Write
      `evidence/qa-gates/batch-c-budget-reset.<capture-timestamp>.md`.
      **Acceptance:** as `[P1-T1]`.

- [ ] [P3-T2] `[expect-fail]` Add the abandon-gate decision-surface cases to
...
- [ ] [P3-T3] `[expect-fail]` Add the pr-author decision-surface cases to
...
- [ ] [P3-T4] `[expect-fail]` Record the fail-before state of the eight new cases. Run
...
- [ ] [P3-T7] Mirror both files and verify parity:
...
- [ ] [P3-T8] Record the `--body-file` routing derivation, so the reaudit sees the R-2.c ordering as
...
- [ ] [P3-T9] Record the pass-after state. Re-run the `[P3-T4]` command and write
...
- [ ] [P3-T10] Run the batch-C toolchain gate with the `[P1-T8]` procedure and acceptance, over the
...
- [ ] [P3-T11] Close batch C with a budget reset, using the `[P1-T1]` procedure. Write
      `evidence/qa-gates/batch-c-close-reset.<capture-timestamp>.md`.
      **Acceptance:** as `[P1-T9]`.

### Phase 4 — Batch D: R-2.d, the `cd`-chained read rule

- [ ] [P4-T1] Open batch D with a budget reset, using the `[P1-T1]` procedure. Write
      `evidence/qa-gates/batch-d-budget-reset.<capture-timestamp>.md`.
      **Acceptance:** as `[P1-T1]`.

- [ ] [P4-T2] `[expect-fail]` Add the `cd`-chain cases to
...
- [ ] [P4-T3] `[expect-fail]` Record the fail-before state. Run `mcp__drm-copilot__run_poshqc_test`
...
- [ ] [P4-T4] Apply Edit 6 to `.claude/hooks/validate-bash.ps1`.
...
- [ ] [P4-T5] Mirror and verify parity:
...
- [ ] [P4-T6] Record the pass-after state. Re-run the `[P4-T3]` command and write
...
- [ ] [P4-T7] Run the batch-D toolchain gate with the `[P1-T8]` procedure and acceptance, over the
...
- [ ] [P4-T8] Close batch D with a budget reset, using the `[P1-T1]` procedure. Write
...
- [ ] [P5-T2] Re-run the three Python contract modules:
      `poetry run pytest tests/scripts/dev_tools/test_parallel_abandon_token_seam.py tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py tests/scripts/dev_tools/test_poshqc_bundled_parity.py -p no:cacheprovider --no-header -q`.
...
- [ ] [P5-T3] Re-verify that all six fail-closed-correct call sites are byte-unchanged. For each of
...
- [ ] [P5-T4] Assert the cycle-2 changed-file scope. Run
