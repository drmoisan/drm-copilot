...
- Use `drmCopilotExtension.newPotentialEntry` for features and `drmCopilotExtension.newPotentialBugEntry` for bugs.
- Use `drmCopilotExtension.potentialToIssue` with explicit work mode `minor-audit`, `full-feature`, or `full-bug`.
- Use `drmCopilotExtension.newActiveFeatureFolder` with `--issue-number`, the numeric issue number, and the explicit selected work mode.
