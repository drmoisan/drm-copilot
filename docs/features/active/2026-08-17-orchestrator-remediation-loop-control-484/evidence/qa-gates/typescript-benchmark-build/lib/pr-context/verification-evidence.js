...
exports.CANONICAL_GLOBS = exports.EXPECTED_EXIT_CODE_FIELD = exports.REQUIRED_FIELDS = void 0;
exports.normalizeResult = normalizeResult;
exports.discoverCanonicalEvidenceFiles = discoverCanonicalEvidenceFiles;
exports.parseVerificationEvidenceMarkdown = parseVerificationEvidenceMarkdown;
exports.parseVerificationEvidenceFile = parseVerificationEvidenceFile;
const file_system_1 = require("../file-system");
/** Required schema fields parsed from an evidence markdown file. */
exports.REQUIRED_FIELDS = ["Timestamp", "Command", "EXIT_CODE"];
/** Optional schema field declaring the exit code a gate is expected to produce. */
exports.EXPECTED_EXIT_CODE_FIELD = "ExpectedExitCode";
/** Canonical evidence glob roots, searched in fixed order. */
exports.CANONICAL_GLOBS = [
 ...
 */
function discoverCanonicalEvidenceFiles(fs, root, feature) {
    ...
    for (const pattern of exports.CANONICAL_GLOBS) {
...
function parseVerificationEvidenceMarkdown(params) {
    ...
    for (const rawLine of splitLines(markdown)) {
        ...
        if (exports.REQUIRED_FIELDS.includes(key) &&
            !parsed.has(key)) {
        ...
        if (key === exports.EXPECTED_EXIT_CODE_FIELD && !parsed.has(key)) {
            parsed.set(key, value);
        }
    }
    ...
    const expectedExitCodeRaw = parsed.get(exports.EXPECTED_EXIT_CODE_FIELD);
    ...
    const expectedExitCode = expectedExitCodeRaw === undefined
        ? 0
    ...
    const normalizedResult = normalizeResult(exitCode, expectedExitCode);
    return {
        feature,
        sourceFile,
        timestamp,
...
function parseVerificationEvidenceFile(params) {
    ...
    const normalizedRoot = (0, file_system_1.toPosixPath)(root).replace(/\/+$/u, "");
    const relPosix = (0, file_system_1.toPosixPath)(relativePath).replace(/^\/+/u, "");
 ...
 */
function relativeToPosix(root, absolute) {
    const normalized = (0, file_system_1.toPosixPath)(absolute);
