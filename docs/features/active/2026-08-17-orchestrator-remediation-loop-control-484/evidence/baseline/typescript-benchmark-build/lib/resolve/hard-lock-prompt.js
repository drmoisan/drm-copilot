...
exports.resolveIssueFileForTarget = resolveIssueFileForTarget;
exports.resolveWorkModeFromIssue = resolveWorkModeFromIssue;
exports.resolveHardLockPrompt = resolveHardLockPrompt;
exports.resolveExecuteHardLockCommand = resolveExecuteHardLockCommand;
 ...
 */
function resolveTemplatePath(templateName, workspaceRoot, templateRoot, fs) {
    const candidates = [];
    if (templateRoot !== null) {
        candidates.push(joinPosix(templateRoot, templateName));
    }
    candidates.push(joinPosix(joinPosix((0, file_system_1.toPosixPath)(workspaceRoot), ".github/codex"), templateName));
 ...
 */
function resolveIssueFileForTarget(targetPath, workspaceRoot, fs) {
    ...
    if (planDirName.startsWith("v") && planDirParts.length >= 2) {
        const parentIssue = joinPosix(joinPosix(normalizedRoot, parentOf(planDir)), "issue.md");
 ...
 */
function resolveWorkModeFromIssue(targetPath, workspaceRoot, fs) {
 ...
 */
function resolveHardLockPrompt(templateContent, targetPath, workspaceRoot, fs) {
    const relativeTarget = tryRelativeToWorkspace(targetPath, workspaceRoot);
    const planPathValue = (0, file_system_1.toPosixPath)(relativeTarget);
    const { mode, fallbackReason } = resolveWorkModeFromIssue(targetPath, workspaceRoot, fs);
 ...
 *   code.
 */
function resolveExecuteHardLockCommand(input) {
    ...
    if (input.quiet === true && input.output === undefined) {
        emit("Error: --quiet requires --output; --quiet alone would suppress all output.");
    ...
    const { path: templatePath, checked } = resolveTemplatePath(templateName, input.workspaceRoot, input.templateRoot, input.fs);
    if (templatePath === null) {
        const checkedText = checked.map((candidate) => `- ${candidate}`).join("\n");
        emit(`Error: Template '${templateName}' not found. Checked locations:\n${checkedText}`);
        return { resolved: null, exitCode: 1 };
    }
    ...
    const resolved = resolveHardLockPrompt(templateContent, input.targetPath, input.workspaceRoot, input.fs);
 ...
 */
function resolveOutputPath(output, workspaceRoot) {
    if (isAbsolute(output)) {
        return (0, file_system_1.toPosixPath)(output);
    }
    return joinPosix((0, file_system_1.toPosixPath)(workspaceRoot), output);
}
/**
 ...
 */
function parentOf(pathValue) {
    const normalized = (0, file_system_1.toPosixPath)(pathValue).replace(/\/+$/, "");
...
function splitParts(pathValue) {
    return (0, file_system_1.toPosixPath)(pathValue)
        .split("/")
