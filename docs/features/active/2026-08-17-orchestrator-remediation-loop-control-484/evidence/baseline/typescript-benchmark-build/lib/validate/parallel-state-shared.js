...
exports.ROOT_PATH = exports.PROHIBITED_ANY_LEVEL_KEYS = exports.BLAST_RADIUS_LIST_FIELDS = exports.BLOCKED_MERGE_STATUSES = exports.MERGED_MERGE_STATUSES = exports.VALID_DRIFT_ACTIONS = exports.VALID_EDGE_REASONS = exports.VALID_DISPOSITIONS = exports.VALID_MUTATION_OPS = exports.VALID_MODES = exports.VALID_KINDS = exports.VALID_SOURCES = exports.VALID_MERGE_STATUS = exports.VALID_ITEM_STATES = void 0;
exports.isObject = isObject;
exports.pythonRepr = pythonRepr;
exports.pythonStr = pythonStr;
exports.isEnumMember = isEnumMember;
exports.isNonEmptyString = isNonEmptyString;
exports.isPositiveInteger = isPositiveInteger;
exports.isNonNegativeInteger = isNonNegativeInteger;
exports.isStringList = isStringList;
exports.inBoundedRange = inBoundedRange;
exports.enumError = enumError;
exports.itemContext = itemContext;
exports.validateBlastRadiusBlock = validateBlastRadiusBlock;
exports.validateItemRecord = validateItemRecord;
exports.validateItems = validateItems;
exports.scanProhibitedKeys = scanProhibitedKeys;
/**
...
exports.VALID_ITEM_STATES = words("proposed admitted prepared scheduled in_flight merged withdrawn blocked");
/**
 ...
 */
exports.VALID_MERGE_STATUS = words("not_started worktree_created pr_open ci_green merged worktree_removed blocked_drift blocked_ci_loop_limit");
/** Blast-radius confidence sources (spec S4, design section 5.2). */
exports.VALID_SOURCES = words("derived declared observed");
/** Work-item kinds carried by the manifest and the planner checkpoint. */
exports.VALID_KINDS = words("feature bug");
/** Run modes (spec S4); `closed` is the default. */
exports.VALID_MODES = words("closed open");
/** Mutation operations recorded in `mutations[]` (spec S4). */
exports.VALID_MUTATION_OPS = words("add remove close requeue");
/** Dispositions for an in-flight removal; a null disposition is absence. */
exports.VALID_DISPOSITIONS = words("detach abandon");
/** Conflict-edge reasons, in design section 5.4 disjunct evaluation order. */
exports.VALID_EDGE_REASONS = words("path_overlap module_overlap shared_surface_overlap contract_dependency");
/** Drift-response actions (spec S4, assumption A8). */
exports.VALID_DRIFT_ACTIONS = words("raised_blocking_finding halted_later_started_item");
/** Merge-status values meaning the item reached a terminal merged outcome. */
exports.MERGED_MERGE_STATUSES = words("merged worktree_removed");
/** Merge-status values meaning the item is blocked (invariant 8). */
exports.BLOCKED_MERGE_STATUSES = words("blocked_drift blocked_ci_loop_limit");
/** The four `blast_radius` collection fields in serialization order. */
exports.BLAST_RADIUS_LIST_FIELDS = words("paths modules shared_surfaces contracts");
/**
 ...
 */
exports.PROHIBITED_ANY_LEVEL_KEYS = words("depends_on integration_branch epic_merge_pr");
/** Path label for the document root in prohibited-key error strings. */
exports.ROOT_PATH = "<root>";
...
function pythonRepr(value) {
    ...
    if (isObject(value)) {
        const pairs = Object.entries(value).map(([key, item]) => `${pythonRepr(key)}: ${pythonRepr(item)}`);
        return `{${pairs.join(", ")}}`;
    }
    return String(value);
}
...
function validateBlastRadiusBlock(radius, context) {
    ...
    for (const field of exports.BLAST_RADIUS_LIST_FIELDS) {
        if (!isStringList(radius[field])) {
            errors.push(`${context} blast_radius.${field} must be a list of non-empty strings.`);
        }
    }
    const source = radius["source"];
    if (!isEnumMember(exports.VALID_SOURCES, source)) {
        errors.push(enumError(context, "blast_radius.source", exports.VALID_SOURCES, source));
    }
    if (!isNonEmptyString(radius["computed_at"])) {
        errors.push(`${context} blast_radius.computed_at must be a non-empty string.`);
    }
    return errors;
}
/**
 ...
 */
function validateMergeStatus(record, context, state) {
    if (!("merge_status" in record)) {
        return [];
    }
    const mergeStatus = record["merge_status"];
    if (!isEnumMember(exports.VALID_MERGE_STATUS, mergeStatus)) {
        return [
            enumError(context, "merge_status", exports.VALID_MERGE_STATUS, mergeStatus),
        ];
    }
    ...
    if (isEnumMember(exports.MERGED_MERGE_STATUSES, mergeStatus) && state !== "merged") {
        return [
            `${context} merge_status ${pythonRepr(mergeStatus)} requires state 'merged'; found: ${pythonRepr(state)}.`,
        ];
    }
    if (isEnumMember(exports.BLOCKED_MERGE_STATUSES, mergeStatus) &&
        state !== "blocked") {
        return [
...
function validateItemRecord(item, context, requireKind = false) {
    ...
    if (!isPositiveInteger(issueNum)) {
        errors.push(`${context} issue_num must be a positive integer; found: ${pythonRepr(issueNum)}.`);
    }
    if (!isNonEmptyString(item["feature_folder"])) {
        errors.push(`${context} feature_folder must be a non-empty string.`);
    }
    const state = item["state"];
    if (!isEnumMember(exports.VALID_ITEM_STATES, state)) {
        errors.push(enumError(context, "state", exports.VALID_ITEM_STATES, state));
    }
    if (requireKind) {
        const kind = item["kind"];
        if (!isEnumMember(exports.VALID_KINDS, kind)) {
            errors.push(enumError(context, "kind", exports.VALID_KINDS, kind));
        }
    }
...
function validateItems(items, context, requireKind = false) {
    ...
    items.forEach((entry, index) => {
        errors.push(...validateItemRecord(entry, itemContext(context, index), requireKind));
        if (!isObject(entry)) {
            return;
        }
 ...
 */
function prohibitedKeyErrors(value, path, context, keys) {
    const errors = [];
    ...
    if (isObject(value)) {
        for (const [key, child] of Object.entries(value)) {
            ...
            const childPath = path === exports.ROOT_PATH ? key : `${path}.${key}`;
            errors.push(...prohibitedKeyErrors(child, childPath, context, keys));
        }
    }
    else if (Array.isArray(value)) {
        value.forEach((entry, index) => {
            errors.push(...prohibitedKeyErrors(entry, `${path}[${index}]`, context, keys));
        });
    }
    return errors;
}
 ...
 */
function scanProhibitedKeys(root, context, deepKeys = exports.PROHIBITED_ANY_LEVEL_KEYS, topLevelKeys = []) {
    const errors = prohibitedKeyErrors(root, exports.ROOT_PATH, context, deepKeys);
    ...
    if (isObject(root) && topLevelKeys.length > 0) {
        for (const key of topLevelKeys) {
            if (key in root) {
                errors.push(`${context} carries prohibited key '${key}' at ${exports.ROOT_PATH}.`);
            }
        }
    }
    return errors;
