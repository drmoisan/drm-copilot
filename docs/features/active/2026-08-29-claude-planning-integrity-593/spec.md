...
- [ ] A numeric count, enumeration, or population is approved in `spec.md` only when its research record identifies the complete symbol or method family, inclusion and exclusion rules, the member set, and an independently constructed cross-check that agrees with the first derivation; focused tests reject a numeric claim without that provenance.
