...
- [x] `"scripts/vscode/**"` is added to the `mandate_reads` array in `config/blast-radius.json` (repo root).
- [x] `"scripts/vscode/**"` is added to the `mandate_reads` array in `extensions/drm-copilot/resources/claude-customizations/config/blast-radius.json` (bundled copy).
- [x] The `version`, `over_breadth_fraction`, and `mandate_reads` keys remain byte-identical between the two copies, per the parity contract in `.claude/rules/parallel-orchestration.md`.
- [x] `tests/scripts/dev_tools/test_blast_radius_config_parity.py` passes.
- [x] `tests/scripts/claude-lib/blast-radius/BlastRadius.KeyPartition.Tests.ps1` passes.
- [ ] **DEFERRED** — `push_down_claude_customizations` is run after the config change so downstream repositories (TaskMaster included) receive the update.
  - **Rationale:** `mcp__drm-copilot__push_down_claude_customizations` serves the payload bundled in the MCP server backing the current session (the published `@danmoisan/drm-copilot-mcp` npm package or the installed VS Code extension's own `out/mcp-server.js`), not this repo's live/uncommitted source. Neither delivery path can serve an unreleased fix; running the tool during this feature verifiably pushed a stale (pre-fix) `mandate_reads` array to TaskMaster (confirmed: the installed extension copy and the pushed file both lacked `scripts/vscode/**`). The push was executed but did not achieve the AC's intent, so it is recorded as deferred rather than checked off.
  - **Follow-up:** tracked as a separate feature (dev-loop MCP routing override + beta-coexisting side-load, so a local unreleased build can be verified end-to-end) — see the active feature folder created for that work once promoted. Until that lands or a release publishes this fix, propagating `mandate_reads` to TaskMaster (and any other downstream repo) requires either a normal release cycle or a manual hand-patch of the destination's `config/blast-radius.json`.
- [x] No change is made to the planner's obligation to declare a genuine write under `scripts/vscode/` explicitly when a diff actually touches a file there; the exclusion covers only the read/run citation shape.
