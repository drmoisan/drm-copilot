...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-15 and AC-17.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 314 an unterminated final line receives a terminator before the append
    ok 315 an advisory line ending mismatch emits ADVISORY-MISMATCH
