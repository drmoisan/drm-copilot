...
exports.PARALLEL_SURFACE = exports.PARALLEL_LAUNCH_SCHEMA_VERSION = void 0;
exports.validateParallelStateIsStandalone = validateParallelStateIsStandalone;
exports.validateParallelLaunchProvenance = validateParallelLaunchProvenance;
exports.validateZeroLostLedger = validateZeroLostLedger;
exports.validateParallelCodexCheckpointReadiness = validateParallelCodexCheckpointReadiness;
...
exports.PARALLEL_LAUNCH_SCHEMA_VERSION = 2;
exports.PARALLEL_SURFACE = "parallel";
...
function mixedStatePaths(value, path = "") {
    const paths = [];
    if ((0, parallel_state_shared_1.isObject)(value)) {
        for (const [key, child] of Object.entries(value)) {
            const childPath = path.length > 0 ? `${path}.${key}` : key;
            if (MIXED_STATE_KEYS.has(key))
...
function validateParallelStateIsStandalone(state, context) {
    return mixedStatePaths(state).map((path) => `${context} contains prohibited epic or fan-in key at ${path}.`);
}
function guardedPath(value, field) {
...
function validateParallelLaunchProvenance(value, expected) {
    ...
    const pairs = [
        ["schema_version", exports.PARALLEL_LAUNCH_SCHEMA_VERSION],
        ["surface", exports.PARALLEL_SURFACE],
    ...
    const errors = pairs.flatMap(([key, wanted]) => value[key] === wanted
        ? []
        : [
            `${prefix}.${key} must be ${(0, parallel_state_shared_1.pythonRepr)(wanted)}; ` +
                `found: ${(0, parallel_state_shared_1.pythonRepr)(value[key])}.`,
        ]);
    const receiptPath = value["launch_receipt_path"];
    if (receiptPath !== undefined &&
        receiptPath !== null &&
        receiptPath !== expected.launchReceiptPath) {
        errors.push(`${prefix}.launch_receipt_path must match the guarded path ` +
            `${(0, parallel_state_shared_1.pythonRepr)(expected.launchReceiptPath)}; found: ${(0, parallel_state_shared_1.pythonRepr)(receiptPath)}.`);
    }
    for (const key of [
        "deployment_agent",
        "model",
    ...
    if (typeof value["launch_spec_sha256"] !== "string" ||
        !SHA256_RE.test(value["launch_spec_sha256"])) {
        errors.push(`${prefix}.launch_spec_sha256 must be 64 lowercase hex characters.`);
    }
    return errors;
}
function validateZeroLostLedger(value, context) {
    ...
    value.forEach((item, index) => {
        ...
        if (!(0, parallel_state_shared_1.isObject)(item)) {
        ...
        if (!isNonEmptyString(gateId)) {
            ...
            errors.push(`${itemPrefix}.gate_id must be unique; found: ${(0, parallel_state_shared_1.pythonRepr)(gateId)}.`);
        }
        else {
            seen.add(gateId);
        }
        if (typeof status !== "string" || !VALID_LEDGER_STATUSES.has(status)) {
            errors.push(`${itemPrefix}.status must be one of PRESERVED, DEGRADED, LOST; ` +
                `found: ${(0, parallel_state_shared_1.pythonRepr)(status)}.`);
        }
        else if (status === "LOST") {
...
function validateKickoffIdentity(state, evidence, context) {
    if (!("kickoff_prompt_path" in state))
        return [];
    const prefix = `${context} committed kickoff identity`;
    const identity = evidence.kickoffIdentity;
    if (!(0, parallel_state_shared_1.isObject)(identity)) {
    ...
    const errors = expected.flatMap(([key, wanted]) => identity[key] === wanted
        ? []
        : [
            `${prefix}.${key} must be ${(0, parallel_state_shared_1.pythonRepr)(wanted)}; ` +
                `found: ${(0, parallel_state_shared_1.pythonRepr)(identity[key])}.`,
        ]);
    if (state["kickoff_prompt_path"] !== expectedPath) {
        errors.push(`${prefix}.path must match checkpoint kickoff_prompt_path ` +
            `${(0, parallel_state_shared_1.pythonRepr)(expectedPath)}; found: ${(0, parallel_state_shared_1.pythonRepr)(state["kickoff_prompt_path"])}.`);
    }
    if (typeof identity["planning_commit"] !== "string" ||
        !COMMIT_RE.test(identity["planning_commit"])) {
        errors.push(`${prefix}.planning_commit must be 40-64 lowercase hex characters.`);
    }
    if (typeof identity["blob_sha256"] !== "string" ||
...
function validateStatus(value, context, launchReceiptPath) {
    const prefix = `${context} external launch status`;
    if (!(0, parallel_state_shared_1.isObject)(value)) {
        return [`${prefix} must be an object loaded from the guarded status path.`];
    }
    const errors = [];
    if (value["schema_version"] !== exports.PARALLEL_LAUNCH_SCHEMA_VERSION) {
        errors.push(`${prefix}.schema_version must be ${String(exports.PARALLEL_LAUNCH_SCHEMA_VERSION)}; ` +
            `found: ${(0, parallel_state_shared_1.pythonRepr)(value["schema_version"])}.`);
    }
    if (value["state"] !== "completed") {
        errors.push(`${prefix}.state must be 'completed'; found: ${(0, parallel_state_shared_1.pythonRepr)(value["state"])}.`);
    }
    if (value["launch_receipt_path"] !== launchReceiptPath) {
        errors.push(`${prefix}.launch_receipt_path must match the guarded launch path.`);
    }
    return errors;
}
function receiptDocument(evidence, record, key, context) {
    ...
    if (!(0, parallel_state_shared_1.isObject)(value)) {
        return {
            errors: [
                `${context} ${label} receipt is missing at ${(0, parallel_state_shared_1.pythonRepr)(path)}.`,
            ],
        };
    }
    return { document: value, errors: [] };
}
function validateReferencedReceipts(evidence, record, context, parallelSlug, itemKey) {
    const documents = {};
    const errors = [];
    for (const key of [
        "authority_receipt_path",
    ...
    if (authority !== undefined) {
        for (const [key, wanted] of [
            ...
            if (authority[key] !== wanted) {
                errors.push(`${context} authority receipt.${key} must be ${(0, parallel_state_shared_1.pythonRepr)(wanted)}; ` +
                    `found: ${(0, parallel_state_shared_1.pythonRepr)(authority[key])}.`);
            }
        }
    }
    ...
    if (delegation !== undefined) {
        if (!isNonEmptyString(delegation["delegation_id"])) {
            errors.push(`${context} delegation receipt.delegation_id must be non-empty.`);
        }
        if (delegation["agent_name"] !== record["deployment_agent"]) {
            errors.push(`${context} delegation receipt.agent_name must match launch agent.`);
        }
    }
    const topology = documents["topology_receipt_path"];
    if (topology !== undefined) {
        errors.push(...(0, orchestrator_state_codex_topology_1.validateCodexTopologyReceipt)(topology, `${context} topology receipt`));
    }
    const model = documents["model_routing_receipt_path"];
    if (model !== undefined) {
        errors.push(...(0, orchestrator_state_codex_model_routing_1.validateCodexModelRoutingReceipt)(model, `${context} model-routing receipt`));
...
function validateParallelCodexCheckpointReadiness(state, context, evidence) {
    ...
    items.forEach((value, index) => {
        ...
        const receipt = guardedPath(value["launch_receipt_path"], `${itemContext}.launch_receipt_path`);
        const status = guardedPath(value["launch_status_path"], `${itemContext}.launch_status_path`);
        ...
        if (receipt.path === undefined || status.path === undefined)
            return;
        ...
        if (!(0, parallel_state_shared_1.isObject)(record)) {
            errors.push(`${itemContext} external launch record is missing at ` +
                `${(0, parallel_state_shared_1.pythonRepr)(receipt.path)}.`);
            return;
        }
        errors.push(...validateParallelLaunchProvenance(record, {
            context: itemContext,
        ...
        errors.push(...validateStatus(evidence.statusRecords[status.path], itemContext, receipt.path));
        errors.push(...validateReferencedReceipts(evidence, record, itemContext, state["parallel_slug"], value["issue_num"]));
    });
    return errors;
}
