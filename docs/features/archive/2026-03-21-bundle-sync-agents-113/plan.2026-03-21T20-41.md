...
- [x] [P4-T5] Run the focused extension sync suite with `npm --prefix extensions/drm-copilot run test:unit -- --runTestsByPath test/extension.test.ts test/extension.integration.test.ts --testNamePattern="syncAgentsFromInstructions|activate registers drmCopilotExtension.syncAgentsFromInstructions"`
...
- [x] [P5-T1] [expect-fail] Add pytest `tests/scripts/dev_tools/test_push_down_copilot_customizations.py::test_sync_agents_script_reference_rewrites_to_live_command`
...
- [x] [P5-T2] Update `scripts/dev_tools/push_down_copilot_customizations_rewrites.py` for the sync-agents live command
...
- [x] [P5-T3] Sync `extensions/drm-copilot/resources/scripts/dev_tools/push_down_copilot_customizations_rewrites.py` to the updated root rewrite catalog
...
- [x] [P5-T4] Run the focused rewrite-catalog suite with `poetry run pytest tests/scripts/dev_tools/test_push_down_copilot_customizations.py tests/scripts/dev_tools/test_push_down_copilot_customizations_helpers.py -k "sync_agents_script_reference_rewrites_to_live_command or mirror"`
...
- [x] [P6-T1] Update the root command-surface documentation in `README.md`
...
- [x] [P6-T2] Update the extension command-surface documentation in `extensions/drm-copilot/README.md`
...
- [x] [P7-T1] Run `npm --prefix extensions/drm-copilot run format` and store the final result in `evidence/qa-gates/typescript-format.*.md`
...
- [x] [P7-T2] Run `npm --prefix extensions/drm-copilot run lint` and store the final result in `evidence/qa-gates/typescript-lint.*.md`
...
- [x] [P7-T3] Run `npm --prefix extensions/drm-copilot run typecheck` and store the final result in `evidence/qa-gates/typescript-typecheck.*.md`
...
- [x] [P7-T4] Run `npm --prefix extensions/drm-copilot run test:unit -- --coverage --coverageReporters=text-summary --coverageReporters=json-summary` and store the final result in `evidence/qa-gates/typescript-test.*.md`
...
- [x] [P7-T5] Record the TypeScript coverage delta in `evidence/qa-gates/typescript-coverage-delta.*.md`
...
- [x] [P7-T6] Run `pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Import-Module ./scripts/powershell/PoshQC; Invoke-PoshQCFormat -Root ."` and store the final result in `evidence/qa-gates/powershell-format.*.md`
...
- [x] [P7-T7] Run `pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Import-Module ./scripts/powershell/PoshQC; Invoke-PoshQCAnalyze -Root ."` and store the final result in `evidence/qa-gates/powershell-analyze.*.md`
...
- [x] [P7-T8] Run `pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "Import-Module ./scripts/powershell/PoshQC; Invoke-PoshQCTest -Root ."` and store the final result in `evidence/qa-gates/powershell-test.*.md`
...
- [x] [P7-T9] Record the PowerShell coverage delta in `evidence/qa-gates/powershell-coverage-delta.*.md`
...
- [x] [P7-T10] Run `poetry run black .` and store the final result in `evidence/qa-gates/python-format.*.md`
...
- [x] [P7-T11] Run `poetry run ruff check` and store the final result in `evidence/qa-gates/python-lint.*.md`
...
- [x] [P7-T12] Run `poetry run pyright` and store the final result in `evidence/qa-gates/python-typecheck.*.md`
...
- [x] [P7-T13] Run `poetry run pytest --cov=src/lexile_corpus_tuner --cov=scripts/dev_tools --cov-report=term-missing` and store the final result in `evidence/qa-gates/python-test.*.md`
...
- [x] [P7-T14] Record the Python coverage delta in `evidence/qa-gates/python-coverage-delta.*.md`
...
- [x] [P7-T15] Record the final multi-language delivery summary in `evidence/qa-gates/bundle-sync-agents-summary.*.md`
