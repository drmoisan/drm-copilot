...
- [x] `enforce-pr-author-skill.ps1` blocks with `ORCHESTRATOR_STATE_PREFLIGHT_FAILED` when the checkpoint exists but fails the preflight validator, verified by a corresponding Pester test. (Superseded in remediation cycle 2: the hook's default `$Invoker` now invokes `--require-pr-creation-ready`, not `--require-complete` — see the `## Addendum` below. `--require-complete` itself is unchanged and remains correct for its original, post-PR-creation completion context.)
- [x] `enforce-pr-author-skill.ps1` allows `gh pr create --body-file`/`gh pr edit --body-file` (subject to the existing five receipt checks passing) when the checkpoint exists and passes the preflight validator, verified by extending the existing `Context 'allowed commands'` tests with a passing preflight mock. (Superseded in remediation cycle 2: same `--require-pr-creation-ready` correction as above.)
...
- [x] The orchestrator invokes the orchestrator-state validator before delegating to `Agent(pr-author)` and records the result under a new `pr_author_preflight` field in `artifacts/orchestration/orchestrator-state.json`, matching the field shape in Technical Specifications. (Satisfied by this feature's own live orchestrator session: `poetry run python -m scripts.dev_tools.validate_orchestration_artifacts orchestrator-state artifacts/orchestration/orchestrator-state.json --require-pr-creation-ready` exited 0 against the real checkpoint on 2026-07-02T23:45:00Z, recorded verbatim under `pr_author_preflight` in `artifacts/orchestration/orchestrator-state.json`.)
...
- [x] Full PowerShell toolchain pass completed (format → analyze → test via PoshQC) with zero errors and no coverage regression (line >= 85%, branch >= 75%) on the changed file. (Re-checked in remediation cycle 1 after regenerating the canonical coverage artifact: `artifacts/pester/powershell-coverage.xml` now contains a `<class>` entry for `.claude/hooks/enforce-pr-author-skill.ps1` at 88.49% command-level / 89.19% line-level coverage, both above the 85% floor, with no regression on changed lines. Corroborating evidence: `evidence/qa-gates/coverage-artifact-class-verification.md` and `evidence/qa-gates/coverage-regeneration-delta.md`.)
...
## Addendum (Remediation Cycle 2 — 2026-07-02T22-05)

**Structural deadlock discovered:** The originally-implemented preflight check invoked the orchestrator-state validator with `--require-complete`, a flag that requires `ci_gate`, `pr_gate`, and a `pr-author` delegation receipt to already exist. All three of those values can only exist *after* the first `gh pr create` of a branch has already succeeded and CI has run against the resulting PR head SHA. Consequently, `--require-complete` could never pass at the exact moment the hook/preflight is invoked (immediately before that first `gh pr create` call), for any real orchestration session using a route with `requires_pr_gate: true`. This turned a gate that was previously an inert CI no-op into a hook-level check that permanently blocked legitimate PR creation — a more severe regression than the original bug. See `remediation/2026-07-02T22-05/remediation-inputs.md` for the full independently-verified reproduction against this feature's own live checkpoint.

**Corrected mechanism:** A new, additive `--require-pr-creation-ready` CLI flag was added to the existing `orchestrator-state` subcommand in `scripts/dev_tools/validate_orchestration_artifacts.py`, backed by a new pure function `validate_orchestrator_state_pr_creation_readiness` in `scripts/dev_tools/_orchestrator_state_pr_creation_readiness.py` (extracted from `scripts/dev_tools/validate_orchestrator_state.py` to respect the 500-line file-size cap). The new flag validates only that: upstream steps 5-8 (`step5_status`-`step8_status`) are not `pending`/`blocked`, `blocked_reason` is `none`, and the `local_execution_overrides`/`delegation_bypasses` lists are empty when present. It does not call `validate_completion_pr_gate`, `_validate_completion_ci_gate`, `validate_phase_completeness`, or `validate_routing_contract`, so it never requires `ci_gate`, `pr_gate`, or routing-contract delegation receipts. Both `.claude/hooks/enforce-pr-author-skill.ps1`'s default `$Invoker` and the orchestrator's own `pr_author_preflight` step now invoke this new flag instead of `--require-complete`.

**`--require-complete` is unchanged:** The pre-existing `--require-complete` flag, its full-lifecycle completion semantics (`ci_gate`, `pr_gate`, routing-contract receipts), and every test that exercises it remain unmodified by this remediation cycle. It remains the correct check for the true post-PR/CI completion context (Step S9 — CI Green Gate; PR Creation Gate condition 6), where those values are structurally satisfiable because the gated actions have already occurred.

**References:** `docs/features/active/2026-07-02-local-preflight-orchestrator-state-gate-272/remediation/2026-07-02T22-05/remediation-inputs.md`, `docs/features/active/2026-07-02-local-preflight-orchestrator-state-gate-272/remediation/2026-07-02T22-05/remediation-plan.md`.
