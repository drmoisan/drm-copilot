...
- `/workspaces/drm-copilot/eslint.config.mjs`
  - Verified ESLint config is currently minimal and *not configured for typed linting* (no `parserOptions.projectService` / `project` specified; no shared configs from typescript-eslint are extended).
...
- #fetch:https://typescript-eslint.io/getting-started/typed-linting
  - Verified the recommended typed-linting setup for typescript-eslint flat config:
    - Use `tseslint.configs.recommendedTypeChecked` (or `strictTypeChecked`) and optionally `stylisticTypeChecked`.
    - Enable typed linting with `languageOptions.parserOptions.projectService: true` (recommended) and `tsconfigRootDir`.
    - Noted explicit warning that typed linting incurs a performance cost (TypeScript project analysis) and points to typed-linting troubleshooting.
- #fetch:https://typescript-eslint.io/users/configs/
  - Verified availability and intended use of shared configs:
    - Without typed linting: `recommended` + `stylistic` as a starting point.
    - With typed linting: `recommendedTypeChecked` + `stylisticTypeChecked` (or `strictTypeChecked` for more opinionated enforcement).
    - Mentions a `disableTypeChecked` utility config for selectively disabling typed rules in overrides.
- #fetch:https://typescript-eslint.io/users/what-about-formatting
  - Verified guidance: use Prettier (or a dedicated formatter) rather than ESLint formatting rules; notes performance issues when running Prettier inside ESLint (`eslint-plugin-prettier`).
...
- ESLint configuration is intentionally lightweight at present; it uses the typescript-eslint parser/plugin but does not enable typed linting.

### Measured impact: enabling type-aware rules on current branch

- Using a scratch flat config that scopes `tseslint.configs.recommendedTypeChecked` + `stylisticTypeChecked` to TS-family files and sets `parserOptions.projectService: true`, ESLint reports **8 new errors** in TS sources on this branch:
  - `src/drm-task-provider.ts`: `@typescript-eslint/no-unused-vars` (`_task` unused)
  - `src/task-command-map.ts`: `@typescript-eslint/consistent-type-definitions` (prefer `interface` over `type`) + `@typescript-eslint/no-unsafe-member-access` (computed name resolves to `any`)
  - `src/utilities/utility-dispatcher.ts`: `@typescript-eslint/no-unsafe-member-access` (computed name resolves to `any`) + `@typescript-eslint/no-base-to-string` (stringification of object)
  - `src/utilities/utility-spec.ts`: `@typescript-eslint/consistent-type-definitions` (prefer `interface`)

These results are from running:

- `npx eslint --no-config-lookup -c artifacts/research/eslint.typed-linting.config.mjs src tests`

This run was intentionally structured to avoid breaking on repo JS files by *scoping* typed linting to TS-family globs.
...
- typescript-eslint “typed linting” recommended config uses `parserOptions.projectService: true` + `recommendedTypeChecked`/`strictTypeChecked` preset families.

### Configuration Examples

```js
// From typescript-eslint typed linting docs (flat config):
// Enable type-aware rules with project service.
...
Include a short “optional future enhancement” section describing how typed linting *could* be enabled using `projectService` and `recommendedTypeChecked`, but avoid making it a hard requirement if the repo has not adopted it yet.

**Update based on user request (2026-02-04):** typed linting is now an explicit objective. The cleanest repo-aligned approach is:

- Update `eslint.config.mjs` to use `typescript-eslint`’s `tseslint.config(...)` helper.
- Scope `recommendedTypeChecked` and `stylisticTypeChecked` under a config object with `files: ["**/*.ts", "**/*.tsx", "**/*.mts", "**/*.cts"]`.
- Set `languageOptions.parserOptions.projectService: true` and `tsconfigRootDir` for those TS files.
- Add a small JS config (or leave JS effectively un-linted) to avoid accidental typed parsing of `.js` files included by `eslint src tests`.

This avoids a common pitfall: spreading `recommendedTypeChecked` at top-level introduces config entries that apply the TypeScript parser and rules globally, which can break linting for `.js` files.
...
- **Key Tasks (typed linting enablement)**:
  - Refactor `eslint.config.mjs` to TS-scope typed linting via `files` + `extends: [...tseslint.configs.recommendedTypeChecked, ...tseslint.configs.stylisticTypeChecked]`.
  - Add `parserOptions.projectService: true` + `tsconfigRootDir` for TS files.
  - Fix the initial wave of type-aware lint errors (measured list above) so `npm run lint` is green.
...
- **Success Criteria**:
  ...
  - Guidance does not contradict repo config (e.g., does not claim typed linting is enabled today).
