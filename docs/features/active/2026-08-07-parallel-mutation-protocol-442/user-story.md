...
- [x] The pinning invariant holds: in-flight items are never rescheduled by any mutation; recoloring is a pure function of `(remaining subgraph, pinned set, pinned cohort index)` and never places an unstarted item in the pinned items' cohort when the two conflict; determinism under mutation against a live in-flight set is proven by unit and property-based tests.
