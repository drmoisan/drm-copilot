...
exports.selectDefaultBase = selectDefaultBase;
exports.formatDiffPath = formatDiffPath;
exports.convertNumstat = convertNumstat;
exports.extensionSummary = extensionSummary;
exports.extractIssueReferences = extractIssueReferences;
exports.extractMergePrNumbers = extractMergePrNumbers;
exports.summarizeConventionalCommits = summarizeConventionalCommits;
exports.formatIssueDetails = formatIssueDetails;
exports.formatPrDetails = formatPrDetails;
exports.buildCloseCandidatesSection = buildCloseCandidatesSection;
exports.buildIssuesToAutocloseSection = buildIssuesToAutocloseSection;
exports.extractChangedPaths = extractChangedPaths;
 ...
 */
function selectDefaultBase(git) {
 ...
 */
function convertNumstat(numstatText) {
    let adds = 0;
    let dels = 0;
    const files = [];
 ...
 */
function extractMergePrNumbers(subjects) {
 ...
 */
function summarizeConventionalCommits(subjects) {
    const counts = new Map();
    // Initialize every recognized type plus the "other" bucket to zero.
    for (const key of models_1.CONVENTIONAL_TYPES) {
        counts.set(key, 0);
    }
    counts.set("other", 0);
...
function formatIssueDetails(issue) {
    const commentsText = (0, models_1.formatList)(issue.comments, "(no comments)");
    const lines = [
        ...
        "",
        (0, models_1.truncate)(issue.body, 1200),
        "",
        "Comments:",
        commentsText,
    ];
    if (issue.userStoryContent) {
        lines.push("", `User story (${issue.userStoryPath ?? "user-story.md"}):`, (0, models_1.truncate)(issue.userStoryContent, 1200));
    }
    return lines.join("\n");
}
/**
...
function formatPrDetails(pr) {
    return [
        ...
        (0, models_1.truncate)(pr.body, 1200),
        "",
        "Auto-close issues (from this PR):",
        (0, models_1.formatList)(pr.closingIssues, "(none)"),
        "",
        "Files (first 15):",
        (0, models_1.formatList)(pr.filesChanged.slice(0, 15), "(none)"),
    ].join("\n");
}
/**
 ...
 */
function buildCloseCandidatesSection(params) {
    const { verified, authorAsserted, referenced, verifiedReason, authorReason } = params;
    ...
    const referencedOnly = referenced
        ...
        .sort(compareCodePoint);
    return [
        (0, models_1.section)("Close candidates"),
        "Auto-close issues (verified from GitHub PR metadata):",
        (0, models_1.formatList)(verified, verifiedReason),
        "",
        "Auto-close issues (author asserted):",
        (0, models_1.formatList)(authorAutoClose, authorReason),
        "",
        "Referenced issues (detected):",
        (0, models_1.formatList)(referencedOnly, "(none)"),
    ].join("\n");
}
/**
 ...
 */
function buildIssuesToAutocloseSection(params) {
    ...
    let body;
    if (ordered.length > 0) {
        body = (0, models_1.formatList)(ordered, "(none)");
    }
    ...
    return [(0, models_1.section)("Issues to autoclose (verified or pending)"), body].join("\n");
}
/**
