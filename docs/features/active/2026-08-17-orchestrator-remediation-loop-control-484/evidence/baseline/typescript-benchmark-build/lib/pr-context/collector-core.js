...
exports.APPENDIX_CHAR_BUDGET = exports.SUMMARY_CHAR_BUDGET = exports.APPENDIX_PATH_DEFAULT = exports.SUMMARY_PATH_DEFAULT = void 0;
exports.collectPrContext = collectPrContext;
...
const feature_docs_1 = require("./feature-docs");
...
exports.SUMMARY_PATH_DEFAULT = "artifacts/pr_context.summary.txt";
/** Default appendix output path (repo-relative). */
exports.APPENDIX_PATH_DEFAULT = "artifacts/pr_context.appendix.txt";
/** Summary character budget (10x: ~2300 lines at 70 chars/line). */
exports.SUMMARY_CHAR_BUDGET = 160000;
/** Appendix character budget (10x: ~6900 lines at 70 chars/line). */
exports.APPENDIX_CHAR_BUDGET = 480000;
/**
...
function collectPrContext(options) {
    ...
    let git = new git_client_1.GitClient(runner, repoRoot, fs);
    const resolvedRoot = git.resolveRoot();
    git = new git_client_1.GitClient(runner, resolvedRoot, fs);
    const gh = new gh_client_core_1.GhClient({
        runner,
        cwd: resolvedRoot,
        fileSystem: fs,
    ...
    let contextResult = (0, render_1.buildPrContext)({
        git,
        gh,
        baseRef: base,
        headRef: head,
    ...
    const changedPaths = (0, render_pr_helpers_1.extractChangedPaths)(contextResult.text);
    const featureDocs = (0, feature_docs_1.gatherFeatureExcerpts)(fs, resolvedRoot, changedPaths);
    const additionalContextFiles = sortedSet(featureDocs.flatMap((doc) => doc.contextFiles).filter((path) => path));
    const featureIssueRefs = sortedSet(featureDocs.flatMap((doc) => doc.issueRefs).filter((ref) => ref.trim()));
    // When feature refs are found, rebuild context so they participate.
    if (featureIssueRefs.length > 0) {
        contextResult = (0, render_1.buildPrContext)({
            git,
            gh,
            baseRef: base,
            headRef: head,
    ...
    const branchRefs = (0, feature_docs_parsers_1.extractIssueReferences)(git.branchName());
    const pathRefs = (0, feature_docs_parsers_1.extractIssueReferences)(changedPaths.join("\n"));
    classifyReferences({
        gh,
        ghAvailable,
        featureIssueRefs,
    ...
    const readinessSignals = sortedSet(featureDocs
        .map((doc) => doc.readinessSignal)
        .filter((signal) => Boolean(signal)));
    const issuesToAutocloseSection = (0, render_pr_helpers_1.buildIssuesToAutocloseSection)({
        verified,
        pendingPrimary,
        readinessSignals,
    });
    ...
    const [, , perFileStats] = (0, summary_helpers_1.parseNumstatDetailed)(numstatText);
    const statusMap = (0, summary_helpers_1.parseNameStatusMap)(nameStatusText);
    const scopingChanges = (0, summary_helpers_1.scopingDocChanges)({
        git,
        fs,
    ...
    const [ciStatus, ciJobs] = ciTarget && ghAvailable ? gh.ciStatus(ciTarget) : [null, []];
    const bucketCore = [];
    const bucketRenames = [];
    const bucketDocs = [];
 ...
 */
function classifyReferences(options) {
    const { gh, ghAvailable, featureIssueRefs, branchRefs, pathRefs, referencedIssuesSet, referencedPrsSet, invalidRefsSet, } = options;
    if (ghAvailable) {
        // Classify the feature refs first, then the combined branch/path refs.
        for (const ref of featureIssueRefs) {
            classifyOne(gh, ref, referencedIssuesSet, referencedPrsSet, invalidRefsSet);
        }
...
function classifyOne(gh, ref, issuesSet, prsSet, invalidSet) {
