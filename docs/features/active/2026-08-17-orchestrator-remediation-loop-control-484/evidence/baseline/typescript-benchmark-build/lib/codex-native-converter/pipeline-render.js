...
exports.renderTargetContent = renderTargetContent;
exports.renderMergedStandingGuidance = renderMergedStandingGuidance;
exports.renderSectionEmissionContent = renderSectionEmissionContent;
 ...
 */
function readSourceText(fileSystem, sourceRoot, sourcePath) {
    const normalizedRoot = sourceRoot.replace(/\/+$/, "");
    const normalizedRelative = sourcePath.replace(/^\/+/, "");
    const absolute = normalizedRoot === ""
        ? normalizedRelative
 ...
 */
function rewriteText(runOptions, sourceText, standingGuidanceSourcePaths) {
    const [rewrittenText, appliedRewrites] = (0, rewrites_1.rewriteSupportedAutomationReference)(sourceText, {
        enableRepoPrompts: runOptions.enableRepoPrompts,
        standingGuidanceSourcePaths,
    });
    const rewriteSummary = appliedRewrites.length > 0
...
function wrapRenderedTargetContent(options) {
    ...
    if (targetRole === models_1.TargetRole.STANDING_GUIDANCE) {
        return ("# Converted standing guidance\n\n" +
            `Applied rewrites:\n${rewriteSummary}\n\n` +
            `${rewrittenText}\n`);
    }
    if (targetRole === models_1.TargetRole.SHARED_SKILL) {
        return ("# Converted skill\n\n" +
            `Applied rewrites:\n${rewriteSummary}\n\n` +
            `${rewrittenText}\n`);
    }
    if (targetRole === models_1.TargetRole.SUBAGENT) {
    ...
    if (targetRole === models_1.TargetRole.MCP_CONFIG) {
        return ("# Review and merge native MCP, hook, and approval settings " +
            "intentionally.\n\n" +
            `${rewrittenText}\n`);
    }
    if (targetRole === models_1.TargetRole.HOOK) {
        return ("# Converted hook\n" +
            "# Review the generated hook behavior before enabling it.\n\n" +
            `${rewrittenText}\n`);
    }
    if (targetRole === models_1.TargetRole.APPROVAL_RULE) {
        return ("# Converted approval rule candidate\n" +
            "# Review the generated rule semantics before enforcement.\n\n" +
            `${rewrittenText}\n`);
    }
    if (targetRole === models_1.TargetRole.LAUNCHER) {
 ...
 * @throws Error When the source file cannot be read.
 */
function renderTargetContent(fileSystem, runOptions, mappingRecord, standingGuidanceSourcePaths) {
    const sourceText = readSourceText(fileSystem, runOptions.sourceRoot, mappingRecord.sourcePath);
    const [rewrittenText, rewriteSummary] = rewriteText(runOptions, sourceText, standingGuidanceSourcePaths);
 ...
 */
function renderMergedStandingGuidance(fileSystem, runOptions, mappingRecords) {
    const standingGuidanceSourcePaths = mappingRecords.map((record) => record.sourcePath);
    ...
    const labelFor = (sourcePath) => {
        const [rewrittenLabel] = (0, rewrites_1.rewriteSupportedAutomationReference)(posixName(sourcePath), {
            enableRepoPrompts: runOptions.enableRepoPrompts,
            standingGuidanceSourcePaths,
        });
    ...
    for (const mappingRecord of mappingRecords) {
        const renderedText = rstrip(renderTargetContent(fileSystem, runOptions, mappingRecord, standingGuidanceSourcePaths));
        const sectionLabel = labelFor(mappingRecord.sourcePath);
        renderedSections.push(`## Source: \`${sectionLabel}\``, "", renderedText, "");
    }
 ...
 */
function renderSectionEmissionContent(runOptions, targetPath, plannedEmissions, sectionLookupById, standingGuidanceSourcePaths) {
    if (plannedEmissions.length === 0) {
        return "";
    }
    const targetRole = plannedEmissions[0].targetRole;
    const mergedSections = [];
    const allRewriteDescriptions = [];
    ...
    for (const plannedEmission of plannedEmissions) {
        ...
        const [rewrittenText, rewriteSummary] = rewriteText(runOptions, sourceSection.content, standingGuidanceSourcePaths);
        const rewriteLines = rewriteSummary
            .split("\n")
            .filter((line) => line !== "" && line !== "- None");
        allRewriteDescriptions.push(...rewriteLines);
        mergedSections.push(`## Source section: \`${plannedEmission.heading}\``, "", "Applied rewrites:", ...(rewriteLines.length > 0 ? rewriteLines : ["- None"]), "", rewrittenText, "");
    }
    const uniqueRewriteDescriptions = [...new Set(allRewriteDescriptions)];
    const rewriteSummary = uniqueRewriteDescriptions.length > 0
