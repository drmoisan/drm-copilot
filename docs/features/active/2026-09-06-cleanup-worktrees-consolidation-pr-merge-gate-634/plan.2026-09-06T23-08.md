...
- [x] [P0-T1] Read the repository policy files in the order enumerated below and write the Phase 0 policy-read artifact.
...
- [x] [P0-T2] Read the four requirement sources and record the acceptance-criteria inventory as a baseline artifact.
...
- [x] [P0-T3] Fetch the integration ref this branch is measured against.
...
- [x] [P0-T4] Rebase this branch onto the fetched integration ref before any no-diff assertion is executed, and record the result as baseline evidence.
...
- [x] [P0-T5] Capture the pre-change absence of all nine asserted tokens from the skill document; this is the alternative proof consumed by the Phase 2 fail-before dossier.
...
- [x] [P0-T6] Capture the pre-change baseline of the push-down parity test that observes this change.
...
- [x] [P0-T7] Record the reasoned determination that no coverage baseline and no language toolchain baseline is captured for this feature.
...
- [x] [P0-T8] Capture the post-rebase, pre-change no-diff baseline for the out-of-scope surfaces, so any later difference is attributable to this feature's work.
...
- [x] [P1-T1] Apply edit (a) to the End-to-End Workflow step 5 item in `.claude/skills/cleanup-merged-worktrees/SKILL.md`: name the human as the actor performing the consolidation merge and state that the merge occurs outside the agent session.
...
- [x] [P1-T2] Apply edit (b) to the step 5 body: state why the merge is human-performed, naming the three blockers recorded in `spec.md` R3.
...
- [x] [P1-T3] Apply edit (c) to the step 5 body: state what the agent does at the handoff boundary and that it then stops.
...
- [x] [P1-T4] Apply edit (d): add one entry to the `## Prohibited Shortcuts` section of the same file, written in the same register as the existing `gh pr create` prohibition.
...
- [x] [P1-T5] Apply edit (e): add one entry to the `## Cross-References` section of the same file.
...
- [x] [P1-T6] Mirror the edited file byte-identically to `extensions/drm-copilot/resources/claude-customizations/.claude/skills/cleanup-merged-worktrees/SKILL.md`.
...
- [x] [P2-T1] Verify AC-1 and check it off in `spec.md`.
...
- [x] [P2-T2] Verify AC-2 and check it off in `spec.md`.
...
- [x] [P2-T3] Verify AC-3 and check it off in `spec.md`.
...
- [x] [P2-T4] Verify AC-4 and check it off in `spec.md`.
...
- [x] [P2-T5] Verify AC-5 and check it off in `spec.md`.
...
- [x] [P2-T6] Verify AC-6 and check it off in `spec.md`.
...
- [x] [P2-T7] Verify AC-7 and check it off in `spec.md`.
...
- [x] [P2-T8] Verify AC-8 and check it off in `spec.md`.
  - Command: `poetry run pytest "tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py::test_bundled_claude_payload_contains_all_repo_runtime_contracts"`
...
- [x] [P2-T9] Verify AC-9 and check it off in `spec.md`.
...
- [x] [P2-T10] Verify AC-10 and check it off in `spec.md`.
...
- [x] [P2-T11] Verify AC-11 and check it off in `spec.md`.
...
- [x] [P2-T12] Verify AC-12 and check it off in `spec.md`.
...
- [x] [P2-T13] Write the fail-before exception dossier, then verify AC-13 and check it off in `spec.md`.
...
- [x] [P2-T14] Write the manual read-through record, then verify AC-14 and check it off in `spec.md`.
...
- [x] [P2-T15] Verify that step 5's existing git-native verification is preserved unchanged.
...
- [x] [P3-T1] Re-run the push-down parity test as the closing QA gate, after every Phase 1 edit and every Phase 2 evidence write.
  - Command: `poetry run pytest "tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py::test_bundled_claude_payload_contains_all_repo_runtime_contracts"`
...
- [x] [P3-T2] Verify the complete changed-path set against the integration ref and record it as the scope-boundary gate.
...
- [x] [P3-T3] Record the final QA-scope determination as an evidence artifact.
...
- [x] [P3-T4] Record the AC-15 deferral and leave the AC-15 checkbox in `spec.md` unchecked.
...
- [x] [P3-T5] Write the acceptance-criteria status summary required by `.claude/skills/acceptance-criteria-tracking/SKILL.md`.
