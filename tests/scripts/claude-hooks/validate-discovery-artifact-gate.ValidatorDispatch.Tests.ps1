...
<#
    Real-dispatch companion suite for `.claude/hooks/validate-discovery-artifact-gate.ps1`
    (issue #475).

    Why this file exists as a SIBLING of `validate-discovery-artifact-gate.Tests.ps1`
    rather than as additional `It`s inside it: `Invoke-DiscoveryValidatorExe` is the
    universally mocked seam in that suite (6 `Mock` registrations), so the seam's own
...
Describe 'validate-discovery-artifact-gate.ps1 real Invoke-DiscoveryValidatorExe dispatch' {
    BeforeAll {
