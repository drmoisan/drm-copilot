...
exports.planMappings = planMappings;
exports.renderGeneratedOutput = renderGeneratedOutput;
exports.buildTranslationTraces = buildTranslationTraces;
exports.buildPlannedEmissions = buildPlannedEmissions;
exports.writeDestinationOutputs = writeDestinationOutputs;
...
const pipeline_traces_1 = require("./pipeline-traces");
/**
 ...
 */
function planMappings(fileSystem, runOptions) {
    const discoveredPaths = (0, inventory_1.discoverSourceArtifacts)(fileSystem, runOptions.sourceRoot, runOptions.sourceEcosystem, runOptions.selectedPaths);
    ...
    for (const sourcePath of discoveredPaths) {
        const classifiedRecord = (0, classifier_1.classifySourceArtifact)(fileSystem, runOptions.sourceRoot, sourcePath, runOptions.sourceEcosystem);
        const mappedRecord = (0, mapping_1.planTargetPaths)(classifiedRecord, {
            enableRepoPrompts: runOptions.enableRepoPrompts,
        });
        mappingRecords.push(mappedRecord);
    }
    return [...mappingRecords].sort((left, right) => compareStrings(left.sourcePath, right.sourcePath));
}
/**
 ...
 */
function renderGeneratedOutput(fileSystem, runOptions, mappingRecords, plannedEmissions) {
    ...
    const mappingRecordsByTarget = new Map();
    const sectionEmissionsByTarget = new Map();
    ...
    for (const plannedEmission of plannedEmissions) {
        ...
        const bucket = sectionEmissionsByTarget.get(plannedEmission.targetPath) ?? [];
    ...
    for (const targetPath of [...mappingRecordsByTarget.keys()].sort(compareStrings)) {
        const recordsForTarget = mappingRecordsByTarget.get(targetPath) ?? [];
        if (recordsForTarget.length === 0) {
            continue;
        }
        if (targetPath === "AGENTS.md" &&
            recordsForTarget.every((record) => record.targetRole === models_1.TargetRole.STANDING_GUIDANCE) &&
            recordsForTarget.length > 1) {
            generatedOutput[targetPath] = (0, pipeline_render_1.renderMergedStandingGuidance)(fileSystem, runOptions, recordsForTarget);
            continue;
        }
        generatedOutput[targetPath] = (0, pipeline_render_1.renderTargetContent)(fileSystem, runOptions, recordsForTarget[recordsForTarget.length - 1], standingGuidanceSourcePaths);
    }
    const sectionLookupById = new Map();
    const parsedSourcePaths = [
        ...new Set(plannedEmissions
    ...
    for (const sourcePath of parsedSourcePaths) {
        const sourceArtifact = (0, parser_1.parseSourceArtifact)(fileSystem, runOptions.sourceRoot, sourcePath, runOptions.sourceEcosystem, models_1.SourceKind.LAUNCHER_PROMPT);
    ...
    for (const targetPath of [...sectionEmissionsByTarget.keys()].sort(compareStrings)) {
        if (targetPath in generatedOutput) {
            continue;
        }
        generatedOutput[targetPath] = (0, pipeline_render_1.renderSectionEmissionContent)(runOptions, targetPath, sectionEmissionsByTarget.get(targetPath) ?? [], sectionLookupById, standingGuidanceSourcePaths);
    }
 ...
 * @throws Error When a required source file cannot be read.
 */
function buildTranslationTraces(fileSystem, runOptions, mappingRecords) {
    const translationTraces = [];
    for (const mappingRecord of mappingRecords) {
        translationTraces.push(...(0, pipeline_traces_1.buildPromptTranslationTraces)(fileSystem, runOptions, mappingRecord));
    }
 ...
 */
function buildPlannedEmissions(translationTraces) {
    // Only shared-skill and hook traces with a concrete target become emissions.
    return translationTraces
        .filter((trace) => trace.targetPath !== null &&
        (trace.targetRole === models_1.TargetRole.SHARED_SKILL ||
            trace.targetRole === models_1.TargetRole.HOOK))
        .map((trace) => ({
 ...
 */
function writeDestinationOutputs(fileSystem, destinationRoot, generatedOutput) {
    ...
    for (const targetPath of Object.keys(generatedOutput).sort(compareStrings)) {
        const fullPath = normalizedRoot === "" ? targetPath : `${normalizedRoot}/${targetPath}`;
