...
- [ ] [P2-T13] Verify AC7. Command: `git grep -c -e "pull request" -- docs/features/completed/separate-version-bump-from-publish-214/runbooks/release-pr-merge-approval.runbook.md`. ExpectedExitCode: 0. Write evidence at `docs/features/active/2026-08-23-readme-misstates-npm-publish-credential-528/evidence/qa-gates/p2-t13-ac7-pullrequest.md` with the four required fields plus `Output Summary:` recording the observed count. `git grep -c` reports the number of matching lines, not the number of token occurrences on a line, so the count is expected to remain 6, unchanged from the P0-T16 baseline: the P1-T3 edit is confined to the file's line 7, which already matched the token once before the edit and continues to match it after the edit, so that line's contribution to the line-count is unchanged.
