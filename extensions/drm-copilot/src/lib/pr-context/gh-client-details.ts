 ...
 */
function extractClosingNumbers(raw: unknown): string[] {
 ...
 */
function authorLogin(record: Record<string, unknown>, key: string): string {
 ...
 */
export function issueDetailsImpl(
  client: GhClient,
  numberRef: string,
): IssueDetails {
  ...
  const payload = client.requestJson(
  ...
  const storyPath = findUserStoryLink(body);
 ...
 */
function fetchIssueComments(client: GhClient, commentsUrl: string): string[] {
  const commentPayload = client.requestJson(
    [client.ghExecutable, "api", `${commentsUrl}?per_page=50`],
 ...
 */
export function closingIssuesImpl(
  client: GhClient,
  prNumber?: string,
): string[] {
  client.ensureAvailable();
 ...
 */
export function currentPrImpl(client: GhClient): PullRequestDetails | null {
  if (!client.available) {
    return null;
  }

  const args = [client.ghExecutable, "pr", "view", "--json", CURRENT_PR_FIELDS];
  const result = runGh(client, args);
