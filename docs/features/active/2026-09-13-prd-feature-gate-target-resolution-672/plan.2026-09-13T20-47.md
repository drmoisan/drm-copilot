...
- **Status:** Revised for preflight round 5
- **Version:** 1.4
...
- [ ] [P1-T7] Run the extraction-only gate: invoke `mcp__drm-copilot__run_poshqc_analyze`, then `mcp__drm-copilot__run_poshqc_test`. This gate runs before Phase 3 adds any deliberately-failing case, so a green result is achievable here. Write `docs/features/active/2026-09-13-prd-feature-gate-target-resolution-672/evidence/qa-gates/batch-a2-gate.<ISO-8601>.md` with `Timestamp:`, `Command:`, `EXIT_CODE:`, and `Output Summary:` recording the analyzer finding count and the Pester passed and failed counts. Acceptance: analyzer findings are 0 and Pester failures are 0, proving the extraction changed no behaviour.
...
- [ ] [P6-T2] Linting. Invoke `mcp__drm-copilot__run_poshqc_analyze` and write `docs/features/active/2026-09-13-prd-feature-gate-target-resolution-672/evidence/qa-gates/final-qc-analyze.<ISO-8601>.md` with `Timestamp:`, `Command:`, `EXIT_CODE:`, and `Output Summary:` recording the finding count and, when non-zero, each finding's rule name and file path. Acceptance: the recorded finding count is 0. A non-zero count restarts the loop at `[P6-T1]` after remediation.

- [ ] [P6-T3] Testing with coverage. Invoke `mcp__drm-copilot__run_poshqc_test`, or the `[P2-T8]` fallback command verbatim when that fallback is in force, running with `CodeCoverage.Enabled` set to `$true` per `scripts/powershell/PoshQC/settings/pester.runsettings.psd1` line 18 and emitting `artifacts/pester/powershell-coverage.xml` per line 22. Write `docs/features/active/2026-09-13-prd-feature-gate-target-resolution-672/evidence/qa-gates/final-qc-test.<ISO-8601>.md` with `Timestamp:`, `Command:` (naming verbatim whichever command was run), `EXIT_CODE:`, and `Output Summary:` recording total tests, passed, failed, the overall line-coverage percentage, and the per-file line-coverage percentage for `.claude/hooks/enforce-prd-feature-before-planner.ps1` and for `.claude/hooks/enforce-prd-feature-before-planner-helpers.ps1`, both computed from the covered-line and total-line counts for those paths in `artifacts/pester/powershell-coverage.xml`, keyed on the full directory path of the enclosing element rather than on the bare file name. Record no branch-coverage value: Pester measures line and command coverage only, so a branch percentage is never printed and no branch gate applies. Acceptance: failed is 0 and six numeric values are recorded, none of them a placeholder.
...
- [ ] [P6-T5] Coverage delta and threshold verification. Write `docs/features/active/2026-09-13-prd-feature-gate-target-resolution-672/evidence/qa-gates/coverage-comparison.<ISO-8601>.md` reporting, as labelled numeric fields: the baseline line coverage for `.claude/hooks/enforce-prd-feature-before-planner.ps1` from the `[P0-T5]` artifact; the post-extraction reading for both production files from the `[P2-T10]` artifact; the post-change reading for both production files from the `[P6-T3]` artifact; the overall baseline and post-change line coverage; and the line coverage of the new and changed lines in both production files, derived from the changed-line set and the per-line hit data in `artifacts/pester/powershell-coverage.xml`. Name in the `Command:` field the command that produced the post-change reading, which is the `[P2-T8]` fallback command verbatim when that fallback is in force. Acceptance: every reported value is numeric and none is a placeholder; the post-change per-file line coverage is at or above 85 percent for both production files, as `spec.md` line 667 requires; the overall post-change line coverage is not lower than the overall baseline; and the new-and-changed-line coverage is recorded for both files. No branch-coverage figure is reported, because Pester does not measure it.
