...
# Issue #539 - command-branch pathspec exemption for the preimplementation gate.
#
# The sibling suite `enforce-orchestration-preimplementation-gate.Tests.ps1` is at 461
# of its 500 permitted lines, so these scenarios land here rather than extending it.
# Every decision below is driven through the pure seam
# `Invoke-OrchestrationPreimplementationGateDecision` with an explicitly not-ready
# checkpoint: no disk I/O, no child process, and no temporary file.
#
...
Describe 'enforce-orchestration-preimplementation-gate.ps1 command exemption (issue #539)' {
    BeforeAll {
        ...
        function ConvertTo-ExemptionCommandPayload {
            <#
                ...
                and redirection exactly as the shell would present it.
            #>
        ...
        function Get-ExemptionDecisionForCommand {
            <#
    ...
    Context 'issue #539 mixed pathspec deny cases' {
        ...
        # prefix and both phrases the pre-existing suite already asserts.
...
'@
    ...
    Context 'issue #671 worktree selector allow cases' {
        ...
        # and 15-19, so every other constraint still applies to the operands.
