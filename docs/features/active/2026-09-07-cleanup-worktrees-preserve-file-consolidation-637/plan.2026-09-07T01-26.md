...
- [x] [P0-T5] Capture the bash coverage baseline. Run
...
- [x] [P1-T9] Gate the stub-replay tests. Run
...
- [x] [P1-T11] Gate the CRLF fixture-integrity test. Run
...
- [x] [P2-T3] [expect-fail] Record fail-before evidence for the untracked-staging case. Run
      ...
      `not ok 1` line.
- [x] [P2-T4] [expect-fail] Record fail-before evidence for the stale advisory line-ending case. Run
...
- [x] [P2-T5] [expect-fail] Record fail-before evidence for the host-token hard-stop case. Run
...
- [x] [P3-T1] Create `scripts/bash/cleanup_worktrees_preserve_lib.sh` with a header comment stating
...
- [x] [P3-T2] Add `preserve_resolve_jq` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`. It
      ...
      writes to stderr.
- [x] [P3-T3] Add `preserve_read_manifest` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`. It
...
- [x] [P3-T4] Add `preserve_validate_record` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`
      ...
      in its body.
- [x] [P3-T5] Extend `preserve_validate_record` so that an absent `host_token_scan`, a
...
- [x] [P3-T6] Add the tests named by AC-01, AC-05, AC-06, AC-26, and AC-28 to
      `tests/shell/test_cleanup_worktrees_preserve.bats`:
      ...
      **The test asserts two things, not one: that the status is 127, and that stderr carries the
      ...
      assertion.
- [x] [P3-T7] Create the fixture groups these five tests drive under
...
- [x] [P4-T1] Add `preserve_plan` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`. It performs no
...
- [x] [P4-T2] Emit `ACTION|preserve-stage|<target-path>|MISSING-SOURCE` from `preserve_plan` when the
...
- [x] [P4-T3] Emit `ACTION|preserve-stage|<target-path>|IGNORED-TARGET` from `preserve_plan` when
...
- [x] [P4-T4] Add `preserve_commit_plan` to `scripts/bash/cleanup_worktrees_preserve_lib.sh` as the
...
- [x] [P4-T5] Add `run_preserve` to `scripts/bash/cleanup_worktrees_preserve_lib.sh` implementing the
      ...
      body.
- [x] [P4-T6] Add the fourth `source` block to `scripts/bash/cleanup-worktrees.sh` for
...
- [x] [P4-T7] Add the `preserve | --preserve` dispatch arm to `main` in
      ...
      `return 2`.
- [x] [P4-T8] Update the `usage()` heredoc in `scripts/bash/cleanup-worktrees.sh` to add the new
...
- [x] [P4-T9] Create the fixture groups for the Phase 4 tests under
...
- [x] [P4-T10] Add the tests named by AC-02, AC-03, AC-04, AC-07, AC-09, AC-10, AC-12, AC-29, AC-30,
...
- [x] [P4-T11] Exercise the writing phase without any temporary file, per `spec.md` D11: a test sets
...
- [x] [P5-T1] Add `preserve_index_has_entry` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`. A
...
- [x] [P5-T2] Add `preserve_render_index_append` to `scripts/bash/cleanup_worktrees_preserve_lib.sh`.
      ...
      in the file.
- [x] [P5-T3] Wire the index outcomes into `preserve_plan` and `preserve_commit_plan`: emit
...
- [x] [P5-T4] Create the fixture groups for this phase under
...
- [x] [P5-T5] Add the tests named by AC-11, AC-13, and AC-14 to
      `tests/shell/test_cleanup_worktrees_preserve.bats`:
...
- [x] [P5-T8] Gate the duplicate-index test. Run
      ...
      Satisfies **AC-14**.
- [x] [P5-T9] Confirm the placement constraint still holds after this phase. Run
...
- [x] [P6-T1] Add `preserve_derive_line_ending` to the preserve library. Given a path it echoes
      ...
      tokens.
- [x] [P6-T2] Replace the fixed line-feed terminator in `preserve_render_index_append` with the
...
- [x] [P6-T3] Handle the unterminated final line: before appending, when the target's final byte is
...
- [x] [P6-T4] Implement the mixed-endings refusal: emit
...
- [x] [P6-T5] Implement the advisory mismatch signal: when the manifest's advisory `line_ending`
...
- [x] [P6-T6] Create the fixture groups for this phase under
...
- [x] [P6-T7] Add the tests named by AC-15, AC-17, AC-18, and AC-19 to
      `tests/shell/test_cleanup_worktrees_preserve.bats`:
...
- [x] [P7-T1] Add `preserve_scan_host_tokens` to the preserve library implementing the pattern set
...
- [x] [P7-T2] Add the six patterns HT1 through HT6 to `preserve_scan_host_tokens`, transcribed from
...
- [x] [P7-T3] Scope the scan exactly as `spec.md` D5 requires: it reads the bytes of the file named
      ...
      index-line string.
- [x] [P7-T4] Make the scan a read-only pre-pass over every valid record that runs before any write
      ...
      records.
- [x] [P7-T5] Implement the hard stop: if any valid record's source bytes or `memory_index_line`
...
- [x] [P7-T6] Implement the unconditional local scan required by `spec.md` D12: a
...
- [x] [P7-T7] Implement the advisory pattern-set mismatch: a `pattern_set_id` other than
...
- [x] [P7-T8] Create the fixture groups for this phase under
...
- [x] [P7-T9] Add the tests named by AC-23, AC-24, AC-25, and AC-27 to
...
- [x] [P7-T13] Confirm the pattern-set identifier is present in the implementation. Run
...
- [x] [P8-T1] Append exactly one bullet to the `## Report Line Contract` list in
...
- [x] [P8-T2] Confirm the token is present in the skill text. Run
...
- [x] [P8-T3] Mirror the identical bullet into
...
- [x] [P8-T4] Verify byte identity of the pair. Run
...
- [x] [P8-T5] Run the push-down contract suite. Run
      `pwsh -NoProfile -Command "Set-Location -LiteralPath 'RESOLVED-WINDOWS-ROOT'; poetry run pytest tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py -q"`,
      substituting the `ResolvedWindowsRoot:` value that `[P0-T2]` recorded for the quoted
...
- [x] [P8-T6] Record the deliberate documentation inconsistency. Write
...
- [x] [P9-T1] Verify the contended library carries no diff. Run two separate Windows-side
...
- [x] [P9-T2] Verify the hooks tree carries no diff. Run two separate Windows-side invocations, in
      this order:
...
- [x] [P9-T3] Verify the 500-line cap per file. Run three separate Windows-side invocations, in this
      order:
      `git add -A`;
      then
      `git status --porcelain`;
...
- [x] [P9-T4] Verify no coverage exclusion was added for a production path. Run, on the Windows
      side:
...
- [x] [P9-T5] Verify the no-temporary-file rule across the changed test surface. Run
      ...
      Satisfies **AC-43**.
- [x] [P9-T6] Record the scope-boundary statement. Write
