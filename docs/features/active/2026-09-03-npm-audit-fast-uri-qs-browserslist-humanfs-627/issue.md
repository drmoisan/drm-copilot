...
- [x] `npm audit --audit-level=moderate` exits 0 in the repo root (`.`).
- [x] `npm audit --audit-level=moderate` exits 0 in `extensions/drm-copilot/`.
- [x] `npm audit --audit-level=moderate` exits 0 in `packages/mcp-server/`.
- [x] The fix is achieved via `npm audit fix` (non-breaking, semver-range-respecting) or explicit minor/patch version bumps in the affected lockfiles; no `--force`/breaking major-version bump is applied without being explicitly flagged and separately confirmed.
- [x] Each workspace's existing test suite passes with zero regressions after the dependency updates (TypeScript/Jest suites in `extensions/drm-copilot` and `packages/mcp-server`, plus any root-level suite).
- [x] No production source code changes beyond lockfiles (and package.json version-range edits strictly needed to admit the fixed versions, if any).
