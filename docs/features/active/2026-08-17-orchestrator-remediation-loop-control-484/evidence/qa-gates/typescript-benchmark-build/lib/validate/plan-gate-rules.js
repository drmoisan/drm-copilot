...
exports.WARNING_CHANNEL = exports.BLOCKING_CHANNEL = void 0;
exports.emptyPlanGateReport = emptyPlanGateReport;
exports.isPlaceholder = isPlaceholder;
exports.dottedRemedy = dottedRemedy;
exports.covValues = covValues;
exports.evaluateCovValue = evaluateCovValue;
exports.patternOperand = patternOperand;
exports.isCheckableLiteral = isCheckableLiteral;
exports.planQuotesLiteral = planQuotesLiteral;
exports.windowJoin = windowJoin;
exports.hasCrossLinePresence = hasCrossLinePresence;
...
exports.BLOCKING_CHANNEL = "blocking";
/** Warning severity channel name. */
exports.WARNING_CHANNEL = "warning";
...
function covValues(command) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (word === plan_gate_commands_1.COV_FLAG) {
        ...
        if (word.startsWith(plan_gate_commands_1.COV_FLAG_PREFIX)) {
            values.push({
                value: word.slice(plan_gate_commands_1.COV_FLAG_PREFIX.length),
...
function evaluateCovValue(report, command, cov, context) {
    ...
    if (cov.spaceSeparated) {
        report.warnings.push(`[${task}] --cov argument value \`${cov.value}\` is supplied ` +
    ...
    if (truncated.endsWith(PYTHON_SUFFIX)) {
        report.blocking.push(`[${task}] --cov argument \`${cov.value}\` names a filesystem path; ` +
...
function evaluateTrackedCovValue(report, task, cov, truncated, context) {
    ...
    if (context.git.isTrackedFile(truncated + PYTHON_SUFFIX)) {
        report.blocking.push(`[${task}] --cov argument \`${cov.value}\` names a tracked module file ` +
    ...
    report.warnings.push(`[${task}] --cov argument \`${cov.value}\` contains a path separator but ` +
...
function patternOperand(argv) {
    const executable = (0, plan_gate_commands_1.grepExecutableIndex)(argv);
    if (executable === null) {
        return null;
    }
 ...
 */
function planQuotesLiteral(text, literal, excludeSpan) {
 ...
 */
function hasCrossLinePresence(context, literal) {
