...
1. A shared eleven-shape fixture table (research §6.2) transcribed verbatim into both suites so a reviewer can diff them by eye. Shapes 01-05 and 07-11 each carry exactly one `EXIT_CODE:` line, so for those ten shapes the deferred duplicate-`EXIT_CODE` divergence cannot confound the comparison; shape 06 is the deliberate two-`EXIT_CODE:` exception whose expected record is runtime-specific, because it pins each runtime's existing duplicate-`EXIT_CODE` precedence and this spec defers convergence of that precedence (Option P2). Shape 06 is therefore excluded from the cross-runtime agreement assertion.
...
The eleven-shape fixture table of research §6.2 is transcribed verbatim into both suites (Invariant B mechanism 1). Shapes 01-05 and 07-11 each carry exactly one `EXIT_CODE:` line; shape 06 is the deliberate two-`EXIT_CODE:` exception whose expected record is runtime-specific, because it pins each runtime's existing duplicate-`EXIT_CODE` precedence and this spec defers convergence of that precedence (Option P2).
...
### Delivered outcome (recorded 2026-08-20)

The fix landed on branch `bug/pr-context-verification-cannot-express-expected-nonzero-exit-485` across
both runtimes in one change set: the optional `ExpectedExitCode` evidence key, the extracted pure
normalization helper, the conditional expectation row in both renderers, a new Python parser test
module (54 tests) and a new Python collector-level sibling module, 22 added TypeScript tests, and the
six-copy documentation update. Both toolchains passed in a single clean final pass — Python 3995 tests
with overall line coverage 92.45% and branch 84.93%; TypeScript 2580 tests with overall line coverage
96.62% and branch 89.98%. New/changed-code coverage is 100% across the four changed production files.
23 of the 25 acceptance criteria are checked off; the two that remain are recorded below.

Corpus counts measured at execution time and recorded in
`evidence/other/additive-corpus-parity.2026-08-20T09-53.md`:

- Artifacts discovered by `CANONICAL_GLOBS` under `docs/features/active/*`: **1293** (the research-time
  reference was 968 artifacts carrying a line-anchored `EXIT_CODE:`; the corpus has grown since).
- Rendered rows: 760 (Python leg) and 767 (TypeScript leg).
- Rendered-row differences, pre-change reference versus post-change output: **0 in both runtimes**
  (Invariant A, AC9).
- Artifacts carrying two or more `EXIT_CODE:` lines, excluded from the cross-runtime comparison and
  attributable to the deferred defect below: **165** (research measured 156 of 968).
- Cross-runtime comparison over the 641 single-`EXIT_CODE` artifacts: **5 content differences plus 1
  presence difference**, all of them pre-existing artifacts carrying a duplicated `Command:` or
  `Timestamp:` line. AC10 and AC17 are therefore NOT checked off. No difference touches an
  `EXIT_CODE` row, a `Normalized result` row, or the new `Expected EXIT_CODE` row, so rendering parity
  for this change's behavior is clean.

### Post-fix monitoring or clean-up tasks

- **The duplicate-key precedence divergence remains UNFIXED and is to be promoted separately via the
  potential-to-issue path.** Execution measured that the divergence is WIDER than the item below
  states: it is not confined to duplicated `EXIT_CODE:` lines but applies to any duplicated required
  key, because the asymmetry lives in the parse loop itself (Python assigns unconditionally, so last
  occurrence wins; TypeScript guards with `!parsed.has(key)`, so first occurrence wins). Six artifacts
  with a single `EXIT_CODE:` line but a duplicated `Command:` or `Timestamp:` line are reported
  differently by the two runtimes today, one of them rendering in TypeScript and being dropped as
  unparseable in Python. When the follow-up defect is promoted, its scope should be stated as
  duplicate-REQUIRED-KEY precedence, and AC10's exclusion clause should be widened to match.
