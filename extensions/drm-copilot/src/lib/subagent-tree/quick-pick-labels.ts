 ...
 */
export function formatLastActivityTimestamp(
...
/**
 * A single quick-pick entry for a discovered root session candidate.
 */
export interface RootSessionPickEntry {
  /** `${timestamp}  ${truncatedPath}` — timestamp first, then the tail. */
  readonly label: string;
  /** Full absolute path; disambiguates identical tails and stays copyable. */
  readonly detail: string;
  /** The selection payload returned when this entry is chosen. */
...
function compareCandidates(
  left: { readonly path: string; readonly lastActivityMs: number | undefined },
  right: { readonly path: string; readonly lastActivityMs: number | undefined },
): number {
