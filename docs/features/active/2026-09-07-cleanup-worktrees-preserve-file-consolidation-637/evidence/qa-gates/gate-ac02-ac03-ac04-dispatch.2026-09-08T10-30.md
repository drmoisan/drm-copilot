...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-02, AC-03, and AC-04.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 295 preserve subcommand dispatches to the preserve driver
    ok 296 an unknown subcommand still prints usage to stderr and returns 2
    ok 297 the manifest path is taken from CLEANUP_WT_MANIFEST_PATH
...
anywhere in the run, so none names any of the 3 test(s) above.
