...
6. **New `.github/workflows/publish-extension.yml`** — tag-triggered (`push: tags: v*`) plus `workflow_dispatch`. Checks out the tag, installs dependencies, builds, packages, and runs `vsce publish` with `--pat ${{ secrets.VSCE_PAT }}`. The publish step is gated to `if: github.event_name == 'push'` (mirroring `publish-mcp-npm.yml`) so a `workflow_dispatch` run exercises build/package without consuming a Marketplace version.
