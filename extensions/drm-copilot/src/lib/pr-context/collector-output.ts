...
import { buildCloseCandidatesSection } from "./render-pr-helpers";
import {
  type VerificationEvidenceRecord,
  parseVerificationEvidenceFile,
} from "./verification-evidence";

/** Options for {@link collectAndWrite}. */
export interface CollectAndWriteOptions extends CollectPrContextOptions {
  /** Summary output path (repo-relative or absolute). */
  readonly out: string;
  /** Appendix output path. */
  readonly appendixOut: string;
  /** When true, append to existing files instead of overwriting. */
  readonly append: boolean;
  /** Optional log sink for the two `Wrote context ...` lines. */
 ...
 */
export function renderVerificationEvidenceSection(
  fs: FileSystem,
  resolvedRoot: string,
  featureDocs: FeatureDocExcerpt[],
): string {
  ...
  const parseableRecords = records.filter(
    (item) =>
      item.normalizedResult === "pass" || item.normalizedResult === "fail",
  );
...
export function buildSummaryText(
  ...
  summarySections.push(
    "",
    collected.issuesToAutocloseSection,
    "",
    closeCandidates,
    "",
    section("Additional context files"),
    formatList(collected.additionalContextFiles, "(none)"),
    "",
    section("Feature doc excerpts"),
    featureSummary,
    "",
    section("Referenced issues (classified)"),
    formatList(collected.referencedIssues, "(none)") +
      (!collected.ghAvailable ? "\nNOTE: Unverified (GitHub unavailable)" : ""),
    "",
    section("PRs in range (classified)"),
    formatList(collected.referencedPrs, "(none)"),
    "",
    section("Invalid references (not found)"),
    formatList(collected.invalidRefs, "(none)"),
    "",
    section("Scoping docs changed"),
    scopingSummary,
    "",
    section("Changed files overview"),
    bucketText("Core logic changes", collected.bucketCore),
    "",
    bucketText("Mechanical moves/renames", collected.bucketRenames),
    "",
    bucketText("Docs/templates/agents/tooling", collected.bucketDocs),
    "",
  ...
  if (summaryText.length > SUMMARY_CHAR_BUDGET) {
    summaryText =
      summaryText.slice(0, SUMMARY_CHAR_BUDGET) +
...
export function buildAppendixText(
  ...
  const issueSections = collected.issueDetails.map((detail) =>
    issueAppendix(detail),
  );
  const prSections = collected.prDetailsList.map((detail) =>
    prAppendix(detail),
  );
  ...
  if (appendixText.length > APPENDIX_CHAR_BUDGET) {
    appendixText =
      appendixText.slice(0, APPENDIX_CHAR_BUDGET) +
...
function resolveGhStatusText(collected: CollectedPrContext): string {
  let ghStatusText =
    collected.ghStatusOverride ||
...
function buildFeatureSummary(featureDocs: FeatureDocExcerpt[]): string {
  const lines: string[] = [];
  for (const doc of featureDocs) {
    lines.push(
      `Feature: ${doc.feature}`,
      "Excerpt:",
      truncateLines(doc.excerpt, 80),
      "Context files:",
      formatList(doc.contextFiles, "(none)"),
      "",
    );
  }
...
function buildScopingSummaryLines(collected: CollectedPrContext): string[] {
  const lines: string[] = [];
...
function renderCiStatus(collected: CollectedPrContext): string {
  if (!collected.ciStatus) {
    return "(not available)";
  }
  return (
    `Status: ${collected.ciStatus}\n` +
