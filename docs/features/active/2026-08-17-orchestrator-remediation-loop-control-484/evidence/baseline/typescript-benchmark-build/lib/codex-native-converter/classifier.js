...
exports.classifyPromptSections = void 0;
exports.readOptionalText = readOptionalText;
exports.hasRepoWidePathsYaml = hasRepoWidePathsYaml;
exports.classifySourceArtifact = classifySourceArtifact;
const models_1 = require("./models");
const classifier_claude_1 = require("./classifier-claude");
 ...
 */
function joinSource(sourceRoot, sourcePath) {
 ...
 */
function readOptionalText(fileSystem, sourceRoot, sourcePath) {
    try {
 ...
 */
function hasRepoWideApplyTo(fileSystem, sourceRoot, sourcePath) {
    return REPO_WIDE_APPLY_TO_PATTERN.test(readOptionalText(fileSystem, sourceRoot, sourcePath));
}
/**
 ...
 *   entry is `**`.
 */
function hasRepoWidePathsYaml(fileSystem, sourceRoot, sourcePath) {
    return REPO_WIDE_PATHS_PATTERN.test(readOptionalText(fileSystem, sourceRoot, sourcePath));
}
/**
 ...
 */
function classifyGithubCopilot(fileSystem, sourceRoot, sourcePath) {
    const pathText = sourcePath;
    const notes = [];
    if (pathText === ".github/copilot-instructions.md") {
        return {
            sourcePath: pathText,
            sourceEcosystem: models_1.SourceEcosystem.GITHUB_COPILOT,
    ...
    if (pathText.startsWith(".github/instructions/") &&
    ...
    if (pathText.startsWith(".github/agents/") &&
        ...
        if (sourceText.toLowerCase().includes("handoff") ||
            ...
            notes.push("Agent manifest contains handoff semantics that require " +
    ...
    if (pathText.startsWith(".github/prompts/") && pathText.endsWith(".md")) {
        return {
            sourcePath: pathText,
            sourceEcosystem: models_1.SourceEcosystem.GITHUB_COPILOT,
            sourceKind: models_1.SourceKind.LAUNCHER_PROMPT,
            conversionClass: models_1.ConversionClass.REPO_CONVENTION,
 ...
 *   target path.
 */
function classifySourceArtifact(fileSystem, sourceRoot, sourcePath, sourceEcosystem) {
    if (sourceEcosystem === models_1.SourceEcosystem.GITHUB_COPILOT) {
        return classifyGithubCopilot(fileSystem, sourceRoot, sourcePath);
    }
    return (0, classifier_claude_1.classifyClaude)(fileSystem, sourceRoot, sourcePath);
}
var classifier_claude_2 = require("./classifier-claude");
Object.defineProperty(exports, "classifyPromptSections", { enumerable: true, get: function () { return classifier_claude_2.classifyPromptSections; } });
