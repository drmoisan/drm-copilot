...
exports.validateParallelResumeTruth = validateParallelResumeTruth;
...
const REASON_RELAUNCH_NOT_AUTHORIZED = "PARALLEL_RESUME_RELAUNCH_NOT_AUTHORIZED";
...
function firstIncomplete(items) {
    const candidates = items.filter((item) => !TERMINAL_ITEM_STATES.has(String(item["state"])));
    return candidates.reduce((first, item) => {
        if (first === undefined) {
            return item;
        }
        const left = ["cohort", "batch", "issue_num"].map((field) => numberOrZero(item[field]));
        const right = ["cohort", "batch", "issue_num"].map((field) => numberOrZero(first[field]));
...
function hasDuplicateIdentity(items) {
...
function latestMutationSequence(state) {
...
function selectedItem(items, selectedIssue) {
    const matches = items.filter((item) => item["issue_num"] === selectedIssue);
    return matches.length === 1 ? matches[0] : undefined;
}
function hasRequiredTruthFields(truth) {
    return [...REQUIRED_TRUTH_FIELDS].every((field) => Object.prototype.hasOwnProperty.call(truth, field));
}
function anyFieldMismatch(truth, item, fields) {
    return fields.some((field) => truth[field] !== item[field]);
}
/** Return ordered live-truth errors for an explicitly resumable checkpoint. */
function validateParallelResumeTruth(state, context) {
    void context;
    ...
    if (anyFieldMismatch(truth, selected, [
        "launch_id",
        "spec_sha256",
        "checkpoint_sha256",
    ])) {
    ...
    if (truth["unresolved_drift"] !== false ||
    ...
    if (truth["child_status_path"] !== selected["child_status_path"] ||
    ...
    if (truth["live_process_running"] === true &&
        truth["should_relaunch"] === true) {
        errors.push(REASON_PROCESS_RUNNING);
    }
    else if (truth["live_process_running"] === false &&
        truth["should_relaunch"] !== true) {
