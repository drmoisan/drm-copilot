...
- [ ] [P7-T1] Confirm AC-8's scope boundary against the anchor commit (the branch's merge-base with `origin/main`).
  Command:
  ```
  git diff --stat ae8d2ce32c95cf03d55ffb544f2514d83ebfc620 -- tests/scripts/claude-lib/blast-radius/BlastRadius.TruthTable.Tests.ps1
  ...
  ```
  Write `evidence/qa-gates/scope-diff.<TIMESTAMP>.md` recording all three outputs. Acceptance: (1) the `git diff --stat` output shows exactly one file, `tests/scripts/claude-lib/blast-radius/BlastRadius.TruthTable.Tests.ps1`; (2) the `git diff --name-only` output — scoped to the whole repository but excluding the feature folder's own documentation — lists exactly one path, `tests/scripts/claude-lib/blast-radius/BlastRadius.TruthTable.Tests.ps1`, and no other path (in particular no path under `.claude/lib/blast-radius/`); (3) `git status --porcelain` shows every listed path either under `docs/features/active/2026-08-23-truth-table-non-emptiness-assertions-cannot-fail-513/` (the plan, spec, and evidence files this work also touches) or equal to that same one test file path, confirming the `--name-only` diff is not hiding an untracked path outside the feature folder that it cannot see.
