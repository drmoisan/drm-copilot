...
- **Remediation cycle:** 1
- **Plan revision:** 2 (preflight deltas REV-1 through REV-5 applied; REV-5 applied per the adjudicated scope expansion, which supersedes the preflight's proposed REV-5 delta)
...
- Preflight revision deltas REV-1 through REV-5 (see `## Preflight Revision Log`)

`<FEATURE>` = `docs/features/active/2026-08-07-parallel-mutation-protocol-442`.

## Preflight Revision Log

This plan is revision 2. The five preflight deltas are applied as follows. Each entry names the
defect corrected and the tasks that now carry the correction, so a re-preflight can confirm the
delta landed rather than inferring it.

| Delta | Defect in revision 1 | Correction applied |
| --- | --- | --- |
...
- Every diff check uses the pinned base in the form `git diff c939b5b8 -- <path>`. The moving tip `origin/epic/parallel-orchestration-integration` is never used as a diff base.
...
**The defect.** `recolor_unstarted` (`scripts/dev_tools/parallel_mutation_protocol.py:214-224`)
...
`merged` or `worktree_removed` (`.claude/skills/parallel-orchestrate/SKILL.md:113-117`), and an
...
| `tests/scripts/dev_tools/test_parallel_mutation_recolor.py` | NEW | Unit scenarios for `recolor_unstarted` under the pinned-barrier offset, plus the C2 regression test. Receives the relocated `TestPinnedItemsNeverMove` class with its 7 call sites updated. | `<= 320` |
...
| `tests/scripts/dev_tools/test_parallel_mutation_protocol.py` | EDIT | Sheds `TestAdmissionOverAllItems` (lines 325-411, approximately 88 lines) and `TestPinnedItemsNeverMove` (lines 128-206, approximately 79 lines) and their now-unused imports; keeps 1 recolor call site (line 214) which is wrapped (+2); gains the four R2 binding tests (approximately +48). Net approximately 498 − 167 − 4 + 2 + 48 = **approximately 377**. | `<= 500` |
| `tests/scripts/dev_tools/test_parallel_mutation_protocol_properties.py` | EDIT | Sheds `TestPropertyThreePinStability` (lines 299-369, approximately 71 lines) and the per-function admission property (lines 374-398, approximately 25 lines); keeps P1, P2, purity and their 4 remaining recolor call sites plus 2 admission call sites (+12 wrapping); `GeneratedRun` gains a `current_cohort` attribute (+4); two P1/P2 tests are corrected in place at approximately constant size. Net approximately 499 − 96 + 16 = **approximately 419**. | `<= 500` |
...
- [ ] [P0-T1] Read the policy files in the required order and record the read evidence.
...
- [ ] [P0-T2] Read the four remediation input artifacts plus `<FEATURE>/spec.md`, `<FEATURE>/user-story.md`, `<FEATURE>/plan.md`, `.claude/rules/parallel-orchestration.md`, and the `F6` section of `docs/features/epics/parallel-orchestration/epic.md`, and record the finding inventory read from the artifacts themselves.
...
- [ ] [P0-T3] Capture the Python lint baseline for this cycle.
...
- [ ] [P0-T4] Capture the Python type-check baseline for this cycle.
...
- [ ] [P0-T5] Capture the Python test and coverage baseline for this cycle with numeric values.
...
- [ ] [P0-T6] Capture the PowerShell analyzer baseline for this cycle.
...
- [ ] [P0-T7] Capture the PowerShell Pester test and coverage baseline for this cycle with numeric values.
...
- [ ] [P0-T8] Capture the pre-remediation diff inventory against the pinned base.
  ...
  - Acceptance: artifact records the pre-remediation numstat for each listed path, including that `pyproject.toml` and `poetry.lock` are unchanged at baseline
- [ ] [P0-T9] Capture the test-module line-count and call-site baseline that the relocation arithmetic depends on.
...
- [ ] [P1-T2] Amend `<FEATURE>/spec.md` FR4 to state the pinned-barrier offset (C2).
...
- [ ] [P1-T3] Add a combined "Design corrections (spec 1.2)" note to `<FEATURE>/spec.md` immediately after FR1, recording the rationale for BOTH C1 and C2.
...
- [ ] [P1-T4] Amend the `## Recompute Boundary and Mutation-Log Entry Contents` wording in `<FEATURE>/spec.md` for coherence with C1, and state explicitly that C2 leaves the generation arithmetic unchanged.
...
- [ ] [P1-T5] Amend the `## API / CLI Surface` snippets in `<FEATURE>/spec.md` to the two mandated signatures.
...
- [ ] [P1-T6] Amend `<FEATURE>/spec.md` `## Test Strategy` scenario 4 (C1).
...
- [ ] [P1-T7] Add `## Test Strategy` scenario 9 to `<FEATURE>/spec.md` for the pinned-barrier offset (C2), and replace the planned property addition with property P4.
...
- [ ] [P1-T8] Amend `<FEATURE>/spec.md` acceptance criterion S2 (C1).
...
- [ ] [P1-T9] Amend `<FEATURE>/spec.md` acceptance criterion S5 (C2).
...
- [ ] [P1-T10] Amend acceptance criterion S9 and FR9 invariant 3 in `<FEATURE>/spec.md` to describe the delivered two-signal formalization (finding R3).
...
- [ ] [P1-T11] Amend acceptance criterion U1 in `<FEATURE>/user-story.md` (C1).
...
- [ ] [P1-T12] Amend acceptance criterion U5 in `<FEATURE>/user-story.md` (C2).
...
- [ ] [P1-T13] Bump the `<FEATURE>/spec.md` version once for the whole cycle and record both amendments in evidence.
...
- [ ] [P1-T14] Verify the acceptance-criteria set integrity after the amendments.
  - Checks: count `- [` items under `## Acceptance Criteria` in `<FEATURE>/spec.md` (expected 15) and in `<FEATURE>/user-story.md` (expected 9); confirm no criterion was added, removed, reordered, or renumbered relative to the pinned base by inspecting `git diff c939b5b8 -- <FEATURE>/spec.md <FEATURE>/user-story.md`
...
- [ ] [P2-T1] Create `tests/scripts/dev_tools/test_parallel_mutation_admission.py` containing only the C1 regression test, written against the CURRENT three-argument `decide_admission` signature.
...
- [ ] [P2-T2] [expect-fail] Run the C1 regression test alone and record the fail-before evidence.
...
- [ ] [P2-T3] Create `tests/scripts/dev_tools/test_parallel_mutation_recolor.py` containing only the C2 regression test, written against the CURRENT four-argument `recolor_unstarted` signature.
...
- [ ] [P2-T4] [expect-fail] Run the C2 regression test alone and record the fail-before evidence.
...
- [ ] [P2-T5] [expect-fail] Run the whole Python suite once with both regressions red, and prove the failures are isolated to the two new tests.
  ...
  - Acceptance: exactly the two expected failing ids appear and no others; `git diff c939b5b8 --stat -- tests/` shows no deletion of an existing test file and no removed assertion; `git diff --numstat c939b5b8 -- tests/scripts/dev_tools/test_parallel_mutation_protocol_ops.py` reports no change

### Phase 3 — Engine Corrections

- [ ] [P3-T1] Change `decide_admission`'s signature and decision logic in `scripts/dev_tools/parallel_mutation_protocol.py`.
...
- [ ] [P3-T2] Correct `decide_admission`'s docstring in `scripts/dev_tools/parallel_mutation_protocol.py`.
...
- [ ] [P3-T3] Change `recolor_unstarted`'s signature and add the pinned-barrier offset in `scripts/dev_tools/parallel_mutation_protocol.py`.
...
- [ ] [P3-T4] Correct the misleading induced-subgraph comment in `recolor_unstarted` (currently `scripts/dev_tools/parallel_mutation_protocol.py:217-219`).
...
- [ ] [P3-T5] Correct `recolor_unstarted`'s docstring in `scripts/dev_tools/parallel_mutation_protocol.py`.
...
- [ ] [P3-T6] Amend the `Pinning (spec FR4)` paragraph of the module docstring in `scripts/dev_tools/parallel_mutation_protocol.py`.
...
- [ ] [P3-T7] Amend the `RecolorResult` class docstring in `scripts/dev_tools/_parallel_mutation_models.py`.
  ...
  - Acceptance: the docstring states the absolute-index and verbatim-write contract; `git diff c939b5b8 -- scripts/dev_tools/_parallel_mutation_models.py` shows docstring-only change for this task
- [ ] [P3-T8] Prove no stale call site of either changed function remains anywhere in the repository.
  - Commands: `poetry run pyright`; then `grep -rn "decide_admission\|recolor_unstarted" scripts tests .claude extensions docs/features/active/2026-08-07-parallel-mutation-protocol-442`
  ...
  - Acceptance: Pyright reports 0 errors; every Python call site of `decide_admission` passes `current_cohort_members` and every Python call site of `recolor_unstarted` passes `current_cohort`; every documentation reference names the new form
- [ ] [P3-T9] Verify the production module's size and purity after both corrections.
  - Checks: line count of `scripts/dev_tools/parallel_mutation_protocol.py`; `grep -n "open(\|Path(\|datetime.now\|random\." scripts/dev_tools/parallel_mutation_protocol.py` returns no match introduced by this cycle
...
- [ ] [P3-T10] Correct the two remaining docstrings the C1 signature change falsifies.
  ...
  - Acceptance: neither module contains a docstring claim that admission or deferral turns on the in-flight subset alone; `git diff c939b5b8 -- scripts/dev_tools/_parallel_mutation_entries.py` shows a docstring-only change; `poetry run pytest tests/scripts/dev_tools -q` exits 0

### Phase 4 — Test Relocation, Corrected Scenarios, Contention Properties, and Pass-After Evidence

- [ ] [P4-T1] Update the C1 regression test in `tests/scripts/dev_tools/test_parallel_mutation_admission.py` to the corrected signature.
...
- [ ] [P4-T2] Relocate and correct the admission unit scenarios into `tests/scripts/dev_tools/test_parallel_mutation_admission.py`.
...
- [ ] [P4-T3] Update the C2 regression test in `tests/scripts/dev_tools/test_parallel_mutation_recolor.py` to the corrected signature.
...
- [ ] [P4-T5] Add the pinned-barrier offset scenarios to `tests/scripts/dev_tools/test_parallel_mutation_recolor.py`.
...
- [ ] [P4-T6] Remove the two relocated classes and now-unused imports from `tests/scripts/dev_tools/test_parallel_mutation_protocol.py`, wrap its one remaining recolor call site, and update its module docstring.
  ...
  - Acceptance: `poetry run ruff check tests/scripts/dev_tools/test_parallel_mutation_protocol.py` exits 0 with no F401; the file no longer references `decide_admission`; the file is `<= 500` lines and near the approximately 377-line figure computed in `## Test-Module Relocation Arithmetic`
- [ ] [P4-T7] Create `tests/scripts/dev_tools/test_parallel_mutation_contention_properties.py` with its self-contained seeded generator and the corrected per-function admission property.
...
- [ ] [P4-T8] Add property P4, the composed contention invariant, to `tests/scripts/dev_tools/test_parallel_mutation_contention_properties.py`.
  ...
  - Acceptance: the property passes for every seed; the non-vacuity assertions are present and themselves pass; the docstring states both binding relationships
- [ ] [P4-T9] Relocate the P3 pin-stability property into `tests/scripts/dev_tools/test_parallel_mutation_contention_properties.py`.
  - Move `TestPropertyThreePinStability` (currently `tests/scripts/dev_tools/test_parallel_mutation_protocol_properties.py:299-369`, both tests) into the new module, updating its `decide_admission` and `recolor_unstarted` call sites to the new signatures and binding them to the module's own generator. Every existing assertion is preserved
  - Acceptance: both relocated tests pass for every seed; no assertion was weakened; the module stays within its `<= 400` budget
- [ ] [P4-T10] Update `tests/scripts/dev_tools/test_parallel_mutation_protocol_properties.py` for the new signatures and correct the two tests that codify the defects.
  - Edits: add a `current_cohort` attribute to `GeneratedRun` and pass it through the `recolor` helper; update the remaining `recolor_unstarted` call sites (lines 220, 291, 493 and the helper at 172) and the remaining `decide_admission` call sites (lines 335 and 492), wrapping each so no line exceeds 88 characters; rewrite `test_cohort_indices_are_contiguous_from_zero` to assert contiguity from the computed offset AND that the offset equals `current_cohort + 1` when an unstarted-to-pinned edge exists and `current_cohort` otherwise; rewrite `test_edges_touching_a_pinned_vertex_do_not_constrain_the_coloring` to assert that pinned edges leave the induced coloring's class STRUCTURE unchanged while the final assignment is shifted past the pinned index; remove the two relocated blocks and update the module docstring's function and property lists accordingly
  - Acceptance: `poetry run pytest tests/scripts/dev_tools/test_parallel_mutation_protocol_properties.py -v` exits 0; the file is `<= 500` lines and near the approximately 419-line figure computed in `## Test-Module Relocation Arithmetic`; the P1 and P2 property classes still exist and no assertion of theirs was removed except the two rewritten tests, each replaced by a strictly stronger assertion
- [ ] [P4-T11] Create `tests/scripts/dev_tools/test_parallel_mutation_cohort_invariant_binding.py` and bind the corrected recolor output to F3's LANDED validator.
...
- [ ] [P4-T12] Prove that no test present before this cycle was dropped, renamed away, or weakened.
  - Checks: enumerate every `def test_` name in the pinned base's `tests/scripts/dev_tools/test_parallel_mutation_protocol.py` and `test_parallel_mutation_protocol_properties.py` via `git show c939b5b8:<path>`; enumerate every `def test_` name across the post-change set of six modules; produce the difference
...
- [ ] [P4-T13] Record the pass-after half of both regression demonstrations.
  ...
  - Acceptance: both artifacts carry a `## Fail-Before` section with exit code 1 and a `## Pass-After` section with exit code 0, each with its command; the `## Binding` sections name the specific tests that re-fail on reversion

### Phase 5 — Consumer Updates: Skills, the F6 Section, and Bundle Mirrors

- [ ] [P5-T1] Correct the FR1 admission and recolor procedure in `.claude/skills/parallel-add/SKILL.md`.
...
- [ ] [P5-T2] Correct the recolor call shape and add the verbatim-index obligation in `.claude/skills/parallel-remove/SKILL.md`.
  ...
  - Acceptance: no four-argument recolor call shape remains in the file; `git diff c939b5b8 -- .claude/skills/parallel-remove/SKILL.md` shows changes only in that one step
- [ ] [P5-T3] Correct the `## Mutation Protocol (F6)` section of `.claude/skills/parallel-orchestrate/SKILL.md` (F5-owned; confined in-section edit).
...
- [ ] [P5-T4] Mirror `.claude/skills/parallel-add/SKILL.md` into the extension bundle byte-for-byte.
...
- [ ] [P5-T5] Mirror `.claude/skills/parallel-remove/SKILL.md` into the extension bundle byte-for-byte.
...
- [ ] [P5-T6] Mirror `.claude/skills/parallel-orchestrate/SKILL.md` into the extension bundle byte-for-byte.
  - Target: `extensions/drm-copilot/resources/claude-customizations/.claude/skills/parallel-orchestrate/SKILL.md`
  - Acceptance: `diff` between the two files reports no difference; no other bundle file is edited by this task
- [ ] [P5-T7] Prove no stale call shape and no unnecessary skill edit remains across the documentation surface.
  - Commands: `grep -rn "decide_admission\|recolor_unstarted" .claude extensions/drm-copilot/resources/claude-customizations`; then `git diff --numstat c939b5b8 -- .claude/skills/parallel-close/SKILL.md`
...
- [ ] [P5-T8] Run the landed contract suites that bind the edited skills and their mirrors.
...
- [ ] [P6-T1] Replace the copied op-classification tuples in `scripts/dev_tools/_parallel_mutation_models.py` with imports of F3's constants (finding R2).
...
- [ ] [P6-T2] Replace the copied op-classification tuples in `scripts/dev_tools/_parallel_orchestrator_state_mutations.py` with imports of F3's constants (finding R2).
...
- [ ] [P6-T3] Add the binding assertions that pin F6's op classification to F3's, in `tests/scripts/dev_tools/test_parallel_mutation_protocol.py`.
...
- [ ] [P6-T4] Authorize S311 for the two seeded-RNG test modules through a confined `pyproject.toml` edit (finding R5).
...
- [ ] [P6-T5] Delete both `# noqa: S311` suppressions from `tests/scripts/dev_tools/test_parallel_mutation_protocol_properties.py` (finding R5).
  ...
  - Acceptance: `grep -rn "noqa: S311" --include=*.py .` returns no match; `poetry run ruff check .` exits 0
- [ ] [P6-T6] Correct the S603 suppression placement in `scripts/dev_tools/parallel_mutation_abandon_cli.py` (finding R6).
...
- [ ] [P6-T7] Record the Python/TypeScript parity gap as an explicit deferral (finding R4).
...
- [ ] [P6-T8] Record the consolidated finding disposition for the cycle.
...
- [ ] [P7-T8] Verify the coverage delta and thresholds against the Phase 0 remediation baseline.
...
- [ ] [P7-T9] Verify the 500-line cap on every touched and every new file against the budgets in `## Test-Module Relocation Arithmetic`.
  ...
  - Acceptance: every listed file is `<= 500` lines and within its planned budget; `test_parallel_mutation_protocol_ops.py` is still exactly 500 lines and byte-unchanged; the cap is not relaxed for any file
- [ ] [P7-T10] Re-verify shared-file confinement and the additive-only constraint against the pinned base.
  - Check A — no epic contention: `git diff --stat c939b5b8 -- .` shows no modified file under `.claude/hooks/enforce-epic-*`, no modified epic skill, agent, or validator, and no modified file under `.claude/rules/`
  ...
  - Check E — no schema growth: no field added to `mutations[]`, `drift_events[]`, or `conflict_edges[]`, and no member added to any of the nine parallel enums
  ...
  - Check G — base plan untouched by this cycle: `git diff --numstat c939b5b8 -- <FEATURE>/plan.md` returns the same numstat values recorded for that path in `<FEATURE>/evidence/remediation-baseline/remediation1-baseline-diff-inventory.md`
  ...
  - Check I — ops test module byte-unchanged: `git diff c939b5b8 -- tests/scripts/dev_tools/test_parallel_mutation_protocol_ops.py` produces empty output and `git diff --numstat` for that path returns no row, proving this cycle added and removed no line in the 500-line module
  ...
  - Check K — `POPULATED_RESERVED_HEADINGS` untouched: the F7/F8 one-line append is byte-identical to the base
  ...
  - Artifact: `<FEATURE>/evidence/qa-gates/remediation1-confinement-verification.md` (`Timestamp:`, `Command:` for each check in `git diff c939b5b8 -- <path>` form, `EXIT_CODE:` per check, `Output Summary:` = per-check verdicts A–K)
  - Acceptance: every check A–K passes; any violation is remediation-required
- [ ] [P7-T11] Re-verify the acceptance criteria against the amended text and confirm check-off honesty.
...
- [ ] [P7-T12] Record the contention-guarantee closure statement from executed results.
...
- [ ] [P7-T13] Record the remediation-cycle completion summary.
