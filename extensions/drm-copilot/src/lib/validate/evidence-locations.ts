 ...
 */
const FORBIDDEN_PREFIX_TO_CANONICAL: ReadonlyArray<readonly [string, string]> =
  [
...
/** A single forbidden-location violation. */
export interface EvidenceLocationViolation {
  /** Absolute-style POSIX path of the offending file. */
  readonly path: string;
  /** Canonical replacement hint for that forbidden prefix. */
...
export function findForbiddenPaths(
  fs: FileSystem,
  root: string,
): EvidenceLocationViolation[] {
  const normalizedRoot = toPosixPath(root).replace(/\/+$/, "");
  const violations: EvidenceLocationViolation[] = [];
 ...
 */
export function formatViolation(path: string, canonical: string): string {
