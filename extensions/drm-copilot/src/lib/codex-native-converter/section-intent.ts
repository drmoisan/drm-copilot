...
const IDENTITY_HEADING_PATTERN =
  /\b(overview|about|introduction|what is|who |identity|name|role|purpose)\b/i;
// Heading keywords suggesting rule-level policy content.
const RULE_HEADING_PATTERN =
  /\b(rule|policy|constraint|convention|standard|principle|guideline)\b/i;
// Heading keywords suggesting configuration, settings, or permissions content.
const CONFIG_HEADING_PATTERN =
  /\b(setting|config|permission|option|flag|parameter|environment|env)\b/i;
 ...
 */
function cueKinds(section: SourceSection): ReadonlySet<SemanticCueKind> {
 ...
 */
export function classifySectionIntent(
  section: SourceSection,
  sourceArtifact: SourceArtifact,
): SectionIntent {
  ...
  if (
    sourceArtifact.sourceKind === SourceKind.LAUNCHER_PROMPT &&
    cues.has(SemanticCueKind.LAUNCHER_WRAPPER) &&
    !cues.has(SemanticCueKind.HARD_GATE) &&
    !cues.has(SemanticCueKind.FORBIDDEN_PATTERN) &&
    !cues.has(SemanticCueKind.NUMBERED_WORKFLOW)
  ) {
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.LAUNCHER_ONLY,
      "Section is a launcher-only block in a launcher-prompt source.",
    );
  }
  ...
  if (
    ...
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.HOOK_CANDIDATE,
  ...
  if (cues.has(SemanticCueKind.NUMBERED_WORKFLOW)) {
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.SHARED_WORKFLOW,
  ...
  if (
    cues.has(SemanticCueKind.TOOL_REQUIREMENT) &&
    CONFIG_HEADING_PATTERN.test(heading)
  ) {
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.CONFIG_CANDIDATE,
  ...
  if (
    ...
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.RULE_CANDIDATE,
  ...
  if (RULE_HEADING_PATTERN.test(heading)) {
    return buildIntent(
      ...
      sourceArtifact,
      SectionIntentKind.RULE_CANDIDATE,
      "Section heading indicates rule or policy content.",
    );
  }
  ...
  if (CONFIG_HEADING_PATTERN.test(heading)) {
    return buildIntent(
      ...
      sourceArtifact,
      SectionIntentKind.CONFIG_CANDIDATE,
      "Section heading indicates configuration or settings content.",
    );
  }
  ...
  if (IDENTITY_HEADING_PATTERN.test(heading)) {
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.IDENTITY,
      "Section heading indicates identity or overview content.",
    );
  }

  // A heading cue with no specific signal becomes general standing guidance.
  if (cues.has(SemanticCueKind.HEADING)) {
    return buildIntent(
      section,
      sourceArtifact,
      SectionIntentKind.STANDING_GUIDANCE,
