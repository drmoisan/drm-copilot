...
- Out of scope / non-goals:
  - Everything owned by sibling issue #622: auto-close issue-number scraping and filtering (`extractIssueReferences` and its Python twin), the raw-reference fallback that adds every raw reference as an issue when `gh` is unavailable, author-asserted derivation, pending-primary derivation, and any rendering of a non-empty autoclose list or omission of the autoclose section. #588 owns only `gh` availability detection and the unavailable-vs-absent status text.
...
- **D6 Relationship with #622.**
  - Option A (adopted): #588 ships the additive unavailable empty-body text only; #622 may later change autoclose rendering, including omitting the section when unavailable.
  - Option B: defer D2 entirely to #622 and ship only the resolver fix.
  - Rationale: B leaves the collapse that `issue.md` asks to address unresolved in #588; the additive text does not conflict with #622.
- **D7 Python parity.**
  - Option A (adopted): apply the D2 status-distinction change to `scripts/dev_tools/pr_context/render_pr_helpers.py` and its call site in `scripts/dev_tools/pr_context/collector.py`; no Python resolver change.
...
- Add the optional availability flag to `buildIssuesToAutocloseSection` (`render-pr-helpers.ts`) and pass availability from its call site in `collector-core.ts` (D2).
- Apply the equivalent change to `build_issues_to_autoclose_section` (`render_pr_helpers.py`) and its call site in `collector.py` (D7).
...
- The non-empty autoclose rendering, reference extraction, raw-reference fallback, author-asserted and pending-primary derivations are unchanged (#622 boundary).
...
- None blocking. Sibling #622 touches adjacent code in `collector-core.ts` and `collector.py`; merge-order conflicts are possible but not blocking.
...
- `extensions/drm-copilot/src/lib/pr-context/collector-core.ts` (call-site change only)
- `extensions/drm-copilot/jest.config.cjs` (coverage entries)
- `scripts/dev_tools/pr_context/render_pr_helpers.py`
- `scripts/dev_tools/pr_context/collector.py` (call-site change only)
...
- Constraints (budget, performance, compatibility): 500-line file cap (`collector-output.ts` 494, `render-pr-helpers.ts` 481, `collector-core.ts` 475 lines before the change); TS/Python output parity by convention; tests must be hermetic on Linux CI.
...
- [ ] The composition-root test in `extensions/drm-copilot/test/extension.collect-pr-context.test.ts`, with a POSIX `PATH` and mocked `existsSync`, asserts that mocked `spawnSync` receives the resolved `gh` path with `auth status`. The test fails on the pre-fix code.
...
- [ ] The not-installed, not-authenticated, and repository-unresolved status messages, the non-empty autoclose rendering, and the #622-owned derivation code are unchanged; `gh-client-core.ts` and `github.py` have no diff.
- [ ] New and touched TypeScript production files meet line coverage >= 85% and branch coverage >= 75%, and `extensions/drm-copilot/jest.config.cjs` contains per-file `coverageThreshold` entries for `./src/lib/executable-resolver.ts` and `./src/lib/pr-context/render-pr-helpers.ts`.
