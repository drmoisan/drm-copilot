...
- [ ] AC1: A native bash wrapper `scripts/bash/shell-qc.sh` provides `check`, `format`, and
...
- [ ] AC3: The wrapper reproduces the prior discovery rules: searches `tools/` and
      ...
      present.
- [ ] AC4: `scripts/dev_tools/shell_qc.py` is removed and all five Poetry console-script
      entries referencing it are removed from `pyproject.toml`.
- [ ] AC5: No component of the shell toolchain (local or CI) invokes Python or Poetry.
