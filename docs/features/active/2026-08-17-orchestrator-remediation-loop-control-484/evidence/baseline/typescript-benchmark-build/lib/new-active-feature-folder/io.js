...
exports.resolveCodeCli = exports.isInsidersSession = exports.INSIDERS_SIGNAL_NAMES = exports.defaultWhichLookup = exports.defaultEnvLookup = exports.defaultCodeLauncher = void 0;
exports.findPotentialFile = findPotentialFile;
exports.parseIssueNumber = parseIssueNumber;
exports.buildFolderSlug = buildFolderSlug;
exports.copyTemplate = copyTemplate;
exports.copyFeatureTemplateForMinorAudit = copyFeatureTemplateForMinorAudit;
exports.materializePlanFile = materializePlanFile;
exports.defaultIssueFetcher = defaultIssueFetcher;
const file_system_1 = require("../file-system");
const models_1 = require("./models");
const markdown_1 = require("./markdown");
const io_launcher_1 = require("./io-launcher");
var io_launcher_2 = require("./io-launcher");
Object.defineProperty(exports, "defaultCodeLauncher", { enumerable: true, get: function () { return io_launcher_2.defaultCodeLauncher; } });
Object.defineProperty(exports, "defaultEnvLookup", { enumerable: true, get: function () { return io_launcher_2.defaultEnvLookup; } });
Object.defineProperty(exports, "defaultWhichLookup", { enumerable: true, get: function () { return io_launcher_2.defaultWhichLookup; } });
Object.defineProperty(exports, "INSIDERS_SIGNAL_NAMES", { enumerable: true, get: function () { return io_launcher_2.INSIDERS_SIGNAL_NAMES; } });
Object.defineProperty(exports, "isInsidersSession", { enumerable: true, get: function () { return io_launcher_2.isInsidersSession; } });
Object.defineProperty(exports, "resolveCodeCli", { enumerable: true, get: function () { return io_launcher_2.resolveCodeCli; } });
/**
 ...
 */
function findPotentialFile(featureName, workspace, fs) {
    const normalized = featureName.replace(/_/g, "-");
    const potentialDirs = [
        (0, models_1.joinPosix)(workspace, "docs/features/potential"),
        (0, models_1.joinPosix)(workspace, "docs/features/potential/promoted"),
    ];
    ...
    for (const directory of potentialDirs) {
        const candidates = fs
            .listFiles(directory)
            .filter((file) => suffix(file) === ".md" &&
            baseName(file).includes(normalized) &&
            !models_1.EXCLUDED_POTENTIAL_NAMES.has(baseName(file)));
        if (candidates.length > 0) {
            // Sort by basename descending so the highest-sorting file wins, matching
            // Python `sorted(..., key=name, reverse=True)[0]`.
            candidates.sort((left, right) => baseName(right).localeCompare(baseName(left)));
 ...
 */
function parseIssueNumber(content) {
 ...
 */
function buildFolderSlug(featureName, potentialFile, issueNumber) {
    ...
    if (!models_1.NAME_PATTERN.test(slug)) {
        throw new Error(`Aborted: '${slug}' is invalid. Use kebab/underscore-case letters/numbers ` +
 ...
 * @param fs Filesystem seam.
 */
function copyTemplate(featureType, templateDir, targetDir, fs) {
    ...
    if (featureType === "bug") {
        for (const name of ["spec.md", models_1.PLAN_TIMESTAMP_TEMPLATE_NAME, "plan.md"]) {
            const src = (0, models_1.joinPosix)(templateDir, name);
            if (fs.exists(src)) {
                fs.copyFile(src, (0, models_1.joinPosix)(targetDir, name));
                if (name === models_1.PLAN_TIMESTAMP_TEMPLATE_NAME) {
                    break;
                }
            }
        }
 ...
 * @param fs Filesystem seam.
 */
function copyFeatureTemplateForMinorAudit(templateDir, targetDir, fs) {
    const timestampedPlan = (0, models_1.joinPosix)(templateDir, models_1.PLAN_TIMESTAMP_TEMPLATE_NAME);
    if (fs.exists(timestampedPlan)) {
        fs.copyFile(timestampedPlan, (0, models_1.joinPosix)(targetDir, models_1.PLAN_TIMESTAMP_TEMPLATE_NAME));
        return;
    }
    const legacyPlan = (0, models_1.joinPosix)(templateDir, "plan.md");
    if (fs.exists(legacyPlan)) {
        fs.copyFile(legacyPlan, (0, models_1.joinPosix)(targetDir, "plan.md"));
    }
}
/**
 ...
 */
function materializePlanFile(featureType, targetDir, featureName, issueField, ownerField, parentField, statusField, versionField, planTimestamp, fs) {
    ...
    void featureType;
    const templatePlan = (0, models_1.joinPosix)(targetDir, models_1.PLAN_TIMESTAMP_TEMPLATE_NAME);
    if (fs.exists(templatePlan)) {
        const targetPlan = (0, models_1.joinPosix)(targetDir, `plan.${planTimestamp}.md`);
        fs.move(templatePlan, targetPlan);
        let content = fs.readText(targetPlan);
        const updatedField = planTimestamp;
        content = (0, markdown_1.setHeaderPlaceholder)(content, featureName, issueField, ownerField, updatedField, statusField, parentField, versionField);
        fs.writeText(targetPlan, content);
        return targetPlan;
    }
    const legacy = (0, models_1.joinPosix)(targetDir, "plan.md");
    if (fs.exists(legacy)) {
        return legacy;
    }
    return null;
 ...
 */
function defaultGhLookup() {
    return (0, io_launcher_1.defaultWhichLookup)("gh") ?? null;
}
/**
 ...
 */
function defaultIssueFetcher(issueNumber, runner, ghLookup = defaultGhLookup) {
    ...
    const record = parsed;
    const numberValue = record.number === undefined || record.number === null
        ? issueNumber
