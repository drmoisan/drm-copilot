...
const RESUME_RE =
  /(?:Every child|child features)\s+resumes?\s+at atomic execution\s+from\s+(?:its|their)\s+committed plan-path/i;
...
export interface KickoffFeature {
...
export interface ParsedEpicKickoff {
  readonly slug: string;
  readonly invocationSlug: string;
  readonly manifestPath: string;
  readonly integrationBranch: string;
  readonly features: ReadonlyArray<KickoffFeature>;
  readonly planningCommit?: string;
  readonly planHashes: Readonly<Record<string, string>>;
}

/** Result from parsing a kickoff. */
export interface EpicKickoffParseResult {
  readonly parsed?: ParsedEpicKickoff;
...
function parseFeatures(lines: ReadonlyArray<string>): {
  readonly features: KickoffFeature[];
  readonly errors: string[];
} {
  const table = tableRows(lines, FEATURE_HEADERS);
  const features: KickoffFeature[] = [];
...
export function parseEpicKickoff(text: string): EpicKickoffParseResult {
...
export function validateEpicKickoffText(text: string): string[] {
