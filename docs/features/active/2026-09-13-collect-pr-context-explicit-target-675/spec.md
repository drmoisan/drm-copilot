...
- [ ] The test `declares the same input-schema properties and required arrays on both tool-definition surfaces` exists in `extensions/drm-copilot/test/mcp-repo-automation-tool-definitions.test.ts` and passes. Its assertion derives the compared property key sets from the definition objects rather than hard-coding a property list, so it fails if a future parameter is added to only one surface.
- [ ] The test `resolves target_ref when supplied and omits it when absent` exists in `extensions/drm-copilot/test/mcp-tool-inputs.test.ts` and passes.
- [ ] The test `rejects an empty target_ref instead of treating it as absent` exists in `extensions/drm-copilot/test/mcp-tool-inputs.test.ts` and passes, covering an empty string, a whitespace-only string, and a non-string value, and asserting that the raised error names `target_ref`.
