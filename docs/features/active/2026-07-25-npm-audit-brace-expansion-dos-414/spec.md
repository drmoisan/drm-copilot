...
## Acceptance Criteria
- [ ] `npm audit --audit-level=moderate` exits 0 in the repository root (`.`).
- [ ] `npm audit --audit-level=moderate` exits 0 in `extensions/drm-copilot`.
- [ ] `npm audit --audit-level=moderate` exits 0 in `packages/mcp-server` (regression guard — it passes today and must continue to pass).
- [ ] No `brace-expansion` node resolving to a version `<=5.0.7` (specifically `2.1.2` or `5.0.7`) remains in `package-lock.json` or `extensions/drm-copilot/package-lock.json` (verify by grep for `node_modules/brace-expansion` entries and their `"version"` fields).
- [ ] No `minimatch@9.x` node remains in `package-lock.json` or `extensions/drm-copilot/package-lock.json` (verify by grep for `"version": "9.` under `minimatch` nodes).
- [ ] The `"c8": { "brace-expansion": "^5.0.7" }` override block is removed from both `package.json` and `extensions/drm-copilot/package.json`, and both manifests carry unscoped `"brace-expansion": "^5.0.8"` and `"minimatch": "^10.2.5"` overrides.
- [ ] Root toolchain passes: `npm run format:check`, `npm run lint`, `npm run typecheck`, and `npm run test:unit:coverage` all exit 0 in the repository root. `npm run test:integration` (`vscode-test`) is not a runnable gate in this repository — no `.vscode-test.{json,js,cjs,mjs}` config exists, so `@vscode/test-cli` exits 1 before starting a runner, both locally and on CI, and no workflow invokes it. For this criterion it is sufficient that the post-change output of `npm run test:integration` is identical to the recorded pre-edit baseline failure, and that mocha's forced `minimatch` consumption path is verified directly instead.
- [ ] Extension toolchain passes: `npm run lint`, `npm run typecheck`, `npm run compile`, and `npm run test:coverage` all exit 0 in `extensions/drm-copilot`.
- [ ] All three `NPM Audit Gate` jobs succeed on the branch head SHA, verified via `gh run view <id> --json jobs`.
- [ ] `packages/mcp-server/package.json` and `packages/mcp-server/package-lock.json` are unmodified (`git diff --name-only main...HEAD` lists no `packages/mcp-server` paths).
