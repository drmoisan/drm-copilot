...
const VALID_EPIC_TYPES: ReadonlySet<string> = new Set(["business", "enabler"]);
...
/** The union index used to resolve depends_on references. */
export interface FeatureReferenceIndex {
  /** Normalized feature_folder basename -> raw feature_folder. */
  readonly byFolderHint: Map<string, string>;
  /** issue_num -> raw feature_folder. */
  readonly byIssueNum: Map<unknown, string>;
}
 ...
 */
export function buildFeatureReferenceIndex(
  features: Record<string, unknown>[],
): FeatureReferenceIndex {
  const byFolderHint = new Map<string, string>();
  const byIssueNum = new Map<unknown, string>();
 ...
 */
export function resolveFeatureReference(
  dependency: unknown,
  index: FeatureReferenceIndex,
): string | null {
 ...
 */
export function validateIntentBlock(state: Record<string, unknown>): string[] {
