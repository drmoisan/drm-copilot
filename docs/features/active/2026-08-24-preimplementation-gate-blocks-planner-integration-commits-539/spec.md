...
| bare `git -C ../x add docs/...` / `--git-dir` / `--work-tree` where the trigger regex does not match the line | allow-by-non-match (pre-existing trigger limitation, recorded with D8; unchanged by this fix) **Superseded in part by issue #545:** as of #545 the bare relocating spelling classifies, so it no longer passes by non-match; the NEVER-EXEMPT disposition of D4 row 14 is unchanged. |
| `git add ':(exclude)scripts/' docs/...` or any `:`-magic operand | deny |
| `git add docs/... && poetry run pytest` (chained, non-exempt segment) | deny |
| Heredoc/message body containing the literal `git add` | deny (unchanged; residual known limitation per D8) **Superseded in part by issue #545:** as of #545 a heredoc mention of the literal in a non-wrapper segment allows, because the mention is masked out of the scan text; a heredoc body that feeds a wrapper segment still denies. |
...
**Superseded by issue #545.** D8 is resolved by issue #545. The remedy adopted there is a masked-trigger model: the trigger literals are matched against a scan text in which quoted spans, heredoc bodies, and other non-executing regions are masked out, rather than being scoped to a segment-leading command name. That model closes the over-match without the fail-open exposure D8 records, because a wrapper-led segment keeps its payload in the scan text and still denies. The under-match direction is closed in the same change: a relocating spelling now classifies. The normative behaviour is the D3 table in `docs/features/active/2026-08-25-enforcement-hook-trigger-matches-whole-command-text-545/spec.md`.
...
- [x] The PoshQC toolchain passes clean in a single pass — `mcp__drm-copilot__run_poshqc_format`, `mcp__drm-copilot__run_poshqc_analyze`, `mcp__drm-copilot__run_poshqc_test` — over all changed and added PowerShell files.
- [x] No out-of-scope changes: the trigger regex `(^|\s)git\s+(add|commit)\b` is unmodified (the whole-command-text over-match is not narrowed; a here-string/message-body literal still denies, asserted by a named deny case per side); no alternative readiness source is added; `Test-OrchestrationReady` is unchanged; no file under `.github/instructions/` or `.claude/rules/` is modified; and no Python leg is added to any enforcement hook — verified by the plan's diff-scope gate and the deny cases above.
...
- Known deferrals recorded, not fixed here: the whole-command-text over-match trigger scoping (D8; separate issue candidate, not filed), index inspection for pathless commits (deferred enhancement), and issue #516 absolute-path normalization (composes upstream of rule 16).
  - **Superseded in part by issue #545:** the whole-command-text over-match deferral recorded above (D8) is closed by issue #545, which delivers the masked-trigger model and also closes the under-match direction. The remaining two deferrals in this bullet — index inspection for pathless commits, and issue #516 absolute-path normalization — are unaffected and remain deferred.
