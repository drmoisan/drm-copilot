...
exports.validateDriftProtocol = validateDriftProtocol;
...
function pathIsCovered(path, declared) {
...
function itemViews(state) {
    ...
    for (const entry of rawItems) {
        ...
        items.push({
            ...
            radius: (0, parallel_state_shared_1.isObject)(entry["blast_radius"]) ? entry["blast_radius"] : null,
        });
    }
    return items.sort((left, right) => left.key - right.key);
}
function mutationViews(state) {
    ...
    rawMutations.forEach((entry, position) => {
        if (!(0, parallel_state_shared_1.isObject)(entry) ||
            ...
            !(0, parallel_state_shared_1.isNonNegativeInteger)(entry["recolor_generation"]) ||
...
function latestEvents(events) {
    ...
    return [...latest.values()].sort((left, right) => left.itemKey - right.itemKey);
}
function eventIsResolved(event, item) {
    ...
    return (radius["source"] === "observed" &&
        typeof computedAt === "string" &&
        CANONICAL_TIMESTAMP.test(computedAt) &&
        CANONICAL_TIMESTAMP.test(event.at) &&
        computedAt > event.at);
}
function observedConflictPeers(event, items) {
    return items
        .filter((item) => item.key !== event.itemKey && item.state === "in_flight")
        .filter((item) => {
        const paths = item.radius === null ? null : stringList(item.radius["paths"]);
        return (paths === null ||
            event.observed.some((observedPath) => pathIsCovered(observedPath, paths)));
    })
        .map((item) => item.key)
        .sort((left, right) => left - right);
}
/** Validate persisted halt intent, affected-item order, and generation binding. */
function validateHaltProtocol(state, events, items, mutations, context) {
    const errors = [];
    const haltedEvents = events.filter((event) => event.action === "halted_later_started_item");
    for (const event of haltedEvents) {
        const requeues = mutations.filter((mutation) => mutation.op === "requeue" && mutation.at >= event.at);
        const keys = requeues
            .map((mutation) => mutation.itemKey)
            .filter((key) => key !== null);
        const ordered = [...keys].sort((left, right) => left - right);
        if (keys.length === 0) {
            errors.push(`${context} drift_events[${event.position}] halted_later_started_item action requires a persisted requeue mutation.`);
        }
        else if (keys.some((key, index) => key !== ordered[index])) {
            errors.push(`${context} requeue mutation item order must be ascending; found: ${(0, parallel_state_shared_1.pythonRepr)(keys)}.`);
        }
        if (keys.includes(event.itemKey)) {
            errors.push(`${context} drifted item ${event.itemKey} must not be the halted requeue target.`);
        }
        ...
        if (expectedPeers.length > 0 &&
            ...
            errors.push(`${context} drift_events[${event.position}] requires halted peer order ${(0, parallel_state_shared_1.pythonRepr)(expectedPeers)}; found requeues: ${(0, parallel_state_shared_1.pythonRepr)(ordered)}.`);
        }
        for (const requeue of requeues) {
            if (requeue.priorState !== "in_flight" ||
                requeue.newState !== "blocked") {
                errors.push(`${context} mutations[${requeue.position}] must requeue in_flight to blocked for semantic drift.`);
            }
            const item = items.find((candidate) => candidate.key === requeue.itemKey);
            if (item?.state !== "blocked" || item.mergeStatus !== "blocked_drift") {
                errors.push(`${context} requeued item ${String(requeue.itemKey)} must persist state 'blocked' with merge_status 'blocked_drift'.`);
            }
        }
        const finalGeneration = requeues.at(-1)?.generation;
        if (finalGeneration !== undefined &&
            state["recolor_generation"] !== finalGeneration) {
            errors.push(`${context} drift resolution generation must match final requeue generation ${finalGeneration}; found: ${(0, parallel_state_shared_1.pythonRepr)(state["recolor_generation"])}.`);
        }
    }
    return errors;
}
...
function localColors(vertices, edges) {
    const vertexSet = new Set(vertices);
    const neighbors = new Map(vertices.map((key) => [key, new Set()]));
    ...
    const ordered = [...vertices].sort((left, right) => {
        const degreeDifference = (neighbors.get(right)?.size ?? 0) - (neighbors.get(left)?.size ?? 0);
        return degreeDifference === 0 ? left - right : degreeDifference;
    });
    const colors = new Map();
    for (const key of ordered) {
        const occupied = new Set([...(neighbors.get(key) ?? [])]
...
function validateRecolor(state, events, items, mutations, context) {
    if (!events.some((event) => event.action === "halted_later_started_item") ||
    ...
    if (!(0, parallel_state_shared_1.isNonNegativeInteger)(currentCohort) ||
        ...
        !(0, parallel_state_shared_1.isNonNegativeInteger)(generation) ||
    ...
    const vertices = items
        ...
        .map((item) => item.key);
    const pinned = new Set(items.filter((item) => item.state === "in_flight").map((item) => item.key));
    const edges = conflictEdges(state);
    const crossesPinned = edges.some(([left, right]) => (vertices.includes(left) && pinned.has(right)) ||
        (vertices.includes(right) && pinned.has(left)));
    const offset = crossesPinned ? currentCohort + 1 : currentCohort;
    const expected = new Map([...localColors(vertices, edges)].map(([key, color]) => [
        key,
        offset + color,
    ]));
    ...
    if (Array.isArray(cohorts)) {
        for (const cohort of cohorts) {
            if (!(0, parallel_state_shared_1.isObject)(cohort) ||
                cohort["generation"] !== generation ||
                typeof cohort["index"] !== "number" ||
                !Array.isArray(cohort["item_keys"])) {
                continue;
            }
            const orderedKeys = cohort["item_keys"].filter((key) => typeof key === "number" && vertices.includes(key));
            if (orderedKeys.some((key, index) => key !== [...orderedKeys].sort((a, b) => a - b)[index])) {
    ...
    const mismatch = actual.size !== expected.size ||
...
function validateDriftProtocol(state, context) {
    ...
    if (unresolved.length > 0) {
        errors.push(`${context} unresolved drift for items ${(0, parallel_state_shared_1.pythonRepr)(unresolved)} blocks admission and completion.`);
    }
    const haltedEvents = events.filter((event) => event.action === "halted_later_started_item");
    if (haltedEvents.length === 0 &&
        mutations.some((mutation) => mutation.op === "requeue")) {
        errors.push(`${context} semantic-drift requeue requires a persisted halted_later_started_item event.`);
    }
    errors.push(...validateHaltProtocol(state, events, items, mutations, context));
