...
exports.VALID_COMPLEXITY_BANDS = exports.REQUIRED_ITEM_KEYS = exports.REQUIRED_KEYS = void 0;
exports.validateParallelPlannerStateText = validateParallelPlannerStateText;
...
exports.VALID_COMPLEXITY_BANDS = [
    "C1",
    "C2",
    "C3",
    "C4",
...
const READY_ITEM_PATH_KEYS = ["research_path", "plan_path"];
/**
...
function kickoffPathFor(slug) {
    return `docs/features/parallel/${(0, parallel_state_shared_1.pythonStr)(slug)}/parallel-kickoff.md`;
}
/**
...
function validateIdentity(state) {
    ...
    for (const key of ["parallel_slug", "parallel_manifest_path"]) {
        if (key in state && !(0, parallel_state_shared_1.isNonEmptyString)(state[key])) {
...
function validateItemContract(items) {
    ...
    for (const { index, record } of itemRecords(items)) {
        const entryContext = (0, parallel_state_shared_1.itemContext)(CONTEXT, index);
        for (const key of exports.REQUIRED_ITEM_KEYS) {
            if (!(key in record)) {
        ...
        if ("complexity_band" in record &&
            !(0, parallel_state_shared_1.isEnumMember)(exports.VALID_COMPLEXITY_BANDS, band)) {
            errors.push((0, parallel_state_shared_1.enumError)(entryContext, "complexity_band", exports.VALID_COMPLEXITY_BANDS, band));
        }
    }
    return errors;
}
...
function validateCollections(state) {
    ...
    if ("items" in state) {
        errors.push(...(0, parallel_state_shared_1.validateItems)(items, CONTEXT, true));
 ...
 * blast-radius source.
 */
function validateReadyItem(record, entryContext) {
    const errors = [];
    const preparationStatus = record["preparation_status"];
    if (preparationStatus !== READY_PREPARATION_STATUS) {
        errors.push(`${entryContext} preparation_status must be ${(0, parallel_state_shared_1.pythonRepr)(READY_PREPARATION_STATUS)}; found: ${(0, parallel_state_shared_1.pythonRepr)(preparationStatus)}.`);
    }
    const preflightStatus = record["preflight_status"];
    if (preflightStatus !== READY_PREFLIGHT_STATUS) {
        errors.push(`${entryContext} preflight_status must be ${(0, parallel_state_shared_1.pythonRepr)(READY_PREFLIGHT_STATUS)}; found: ${(0, parallel_state_shared_1.pythonRepr)(preflightStatus)}.`);
    }
    ...
    for (const key of READY_ITEM_PATH_KEYS) {
        if (!(0, parallel_state_shared_1.isNonEmptyString)(record[key])) {
            errors.push(`${entryContext} ${key} must be a non-empty string.`);
        }
    }
    const radius = record["blast_radius"];
    const source = (0, parallel_state_shared_1.isObject)(radius) ? radius["source"] : null;
    if (source !== READY_RADIUS_SOURCE) {
        errors.push(`${entryContext} blast_radius.source must be ${(0, parallel_state_shared_1.pythonRepr)(READY_RADIUS_SOURCE)} for execution readiness; found: ${(0, parallel_state_shared_1.pythonRepr)(source)}.`);
    }
    return errors;
}
/**
...
function validateReadyGate(state) {
    ...
    if (Array.isArray(items)) {
        ...
        if (count < MINIMUM_READY_ITEMS) {
            errors.push(`${CONTEXT} requires at least ${MINIMUM_READY_ITEMS} items for execution readiness; found: ${count}.`);
        }
        for (const { index, record } of itemRecords(items)) {
            errors.push(...validateReadyItem(record, (0, parallel_state_shared_1.itemContext)(CONTEXT, index)));
        }
    }
    const nextStep = state["next_step"];
    if (nextStep !== READY_NEXT_STEP) {
        errors.push(`${CONTEXT} next_step must be ${(0, parallel_state_shared_1.pythonRepr)(READY_NEXT_STEP)}; found: ${(0, parallel_state_shared_1.pythonRepr)(nextStep)}.`);
    }
    ...
    if (kickoffPromptPath !== expectedKickoff) {
        errors.push(`${CONTEXT} kickoff_prompt_path must be ${(0, parallel_state_shared_1.pythonRepr)(expectedKickoff)}; found: ${(0, parallel_state_shared_1.pythonRepr)(kickoffPromptPath)}.`);
    }
    return errors;
}
/**
