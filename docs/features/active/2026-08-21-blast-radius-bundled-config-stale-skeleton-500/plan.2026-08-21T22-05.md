...
`node_modules` is absent at the repository root, at `extensions/drm-copilot`, and at
`packages/mcp-server`. The root `package.json` declares no npm workspaces, so each location installs
independently. The Prettier, ESLint, `tsc`, and Jest scripts this plan invokes are declared in
`extensions/drm-copilot/package.json` (`format`, `lint`, `typecheck`, `test:unit`,
`test:coverage`), so P0-T3 installs at `extensions/drm-copilot` specifically. The coverage script at
that location is `test:coverage`; there is no `test:unit:coverage` script there.
...
- [ ] [P0-T2] Delete the unfilled scaffold plan `docs/features/active/2026-08-21-blast-radius-bundled-config-stale-skeleton-500/plan.2026-08-21T17-26.md` created by the promotion tooling. Record the removal at `<FEATURE>/evidence/other/stale-plan-removal.<ISO8601>.md` with `Timestamp:`, `Command:`, `EXIT_CODE:`, `Output Summary:`. Acceptance: a `Test-Path` probe on that exact path reports `False`, and this plan file `plan.2026-08-21T22-05.md` is the only `plan.*.md` in the feature folder.

- [ ] [P0-T3] Install the npm dependency tree for the workspace that owns the Prettier, ESLint, `tsc`, and Jest scripts by running `npm ci` with the working directory set to `extensions/drm-copilot`. Record the result at `<FEATURE>/evidence/baseline/npm-ci-extension.<ISO8601>.md` with the four required fields. Acceptance: `EXIT_CODE: 0` and `extensions/drm-copilot/node_modules` exists.
...
- [ ] [P0-T8] Capture the Python test and coverage baseline by running, from the worktree root, `poetry run pytest tests/scripts/dev_tools/test_blast_radius_config.py tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py tests/scripts/dev_tools/test_orchestration_routing_config_parity.py --cov=scripts.dev_tools --cov-branch --cov-report=term-missing`. Artifact: `<FEATURE>/evidence/baseline/python-pytest-coverage.<ISO8601>.md` with the four required fields, and `Output Summary:` must carry the numeric passed and failed counts plus the numeric total line-coverage and branch-coverage percentages. Acceptance: the artifact records both numeric coverage values; the literal string `UNVERIFIED` must not appear in the artifact.
...
- [ ] [P8-T4] Run `poetry run pytest tests/scripts/dev_tools/test_blast_radius_config.py tests/scripts/dev_tools/test_blast_radius_config_parity.py tests/scripts/dev_tools/test_push_down_claude_resource_contracts.py tests/scripts/dev_tools/test_orchestration_routing_config_parity.py --cov=scripts.dev_tools --cov-branch --cov-report=term-missing` from the worktree root. Artifact: `<FEATURE>/evidence/qa-gates/final-python-pytest-coverage.<ISO8601>.md` with the four required fields, and `Output Summary:` carrying the numeric passed and failed counts and the numeric total line-coverage and branch-coverage percentages. Acceptance: `EXIT_CODE: 0`, line coverage at or above 85%, branch coverage at or above 75%.
...
- [ ] [P8-T12] Write the coverage delta and threshold verification at `<FEATURE>/evidence/qa-gates/coverage-delta-verification.<ISO8601>.md` with `Timestamp:`, `Command:` naming the three coverage commands compared, `EXIT_CODE:`, and `Output Summary:` carrying, per language, the baseline coverage from Phase 0, the post-change coverage from this phase, and the changed-lines coverage for the files this plan edited. Acceptance: the artifact records numeric values in all three columns for Python and TypeScript and in the baseline and post-change columns for PowerShell, shows no regression against the Phase 0 baselines, and shows no reduction in coverage for the changed lines. No `exclude` entry was added to any coverage configuration, and the artifact states that explicitly.
