...
- [x] Move to active feature folder / branch

## Restoration Note

This file was reconstructed on 2026-08-15 from the active feature folder's `issue.md`
(`docs/features/active/2026-08-15-enforcement-hooks-must-not-invoke-python-475/issue.md`),
which carries the identical promoted content.

`mcp__drm-copilot__potential_to_issue` reported this path as its `destination_path`, and the
file was confirmed present immediately after that call (6951 bytes, timestamped 12:47). It was
absent from disk on a later check in the same session, while the source record at
`docs/features/potential/2026-08-15-enforcement-hooks-must-not-invoke-python.md` remained
deleted. Without this restoration the net effect would have been silent loss of the record.

This is the same promoted-file-disappears defect observed previously. The new detail is that an
immediate post-call existence probe is NOT sufficient to detect it, because the file can be
removed later in the same session. The probe must be repeated before the pre-review commit.
