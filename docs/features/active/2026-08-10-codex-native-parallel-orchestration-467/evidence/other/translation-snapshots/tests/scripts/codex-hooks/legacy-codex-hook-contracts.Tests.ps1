...
Describe 'Legacy Codex hooks use native lifecycle contracts' {
    BeforeAll {
        ...
        $script:ParallelRegisteredHookNames = @(
    ...
    It 'parse-checks each root and bundled hook and keeps every file within 500 lines' {
        foreach ($name in @($script:StaticCheckNames) + @($script:ParallelRegisteredHookNames)) {
            $roots = if ($name -in $script:ParallelRegisteredHookNames) {
                @($script:HookRoot)
            } else {
                @($script:HookRoot, $script:BundleHookRoot)
            }
