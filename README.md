...
- **Delegation** — planning goes to `atomic-planner`, task-by-task execution to `atomic-executor`, audits to `feature-review`/`epic-review`/`staged-review`, investigation to `task-researcher`, and PR creation exclusively to `pr-author` (the only authorized caller of `gh pr create`/`gh pr edit --body`, gated by the `enforce-pr-author-skill.ps1` hook and a short-lived authorization sentinel).
