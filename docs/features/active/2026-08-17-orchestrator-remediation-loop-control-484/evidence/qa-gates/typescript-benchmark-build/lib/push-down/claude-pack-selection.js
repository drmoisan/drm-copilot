...
exports.ManifestError = exports.LEGACY_VARIANT_SOURCE_PREFIX = exports.CSHARP_PACK_NAMES = exports.CSHARP_CANONICAL_PATHS = exports.CORE_PACK_NAME = void 0;
...
exports.LEGACY_VARIANT_SOURCE_PREFIX = ".claude-variants/csharp-legacy";
/**
...
function loadPackManifests(manifestDir, selectedPackNames, fs) {
    ...
    for (const name of [...namesToLoad].sort()) {
        ...
        if (!fs.isFile(manifestPath)) {
            throw new ManifestError(`Pack manifest is missing for pack '${name}': ${manifestPath}`);
        }
...
function parseManifest(name, manifestPath, rawText) {
    ...
    try {
        ...
        throw new ManifestError(`Pack manifest is not valid JSON for pack '${name}': ${manifestPath}`);
    }
    // A JSON array, string, or number is not an object manifest.
    if (loaded === null || typeof loaded !== "object" || Array.isArray(loaded)) {
        throw new ManifestError(`Pack manifest must be a JSON object for pack '${name}': ${manifestPath}`);
    }
    const parsed = loaded;
    ...
    if (typeof manifestName !== "string" || manifestName === "") {
        throw new ManifestError(`Pack manifest 'name' must be a non-empty string: ${manifestPath}`);
    }
    if (typeof manifestLabel !== "string" || manifestLabel === "") {
        throw new ManifestError(`Pack manifest 'label' must be a non-empty string: ${manifestPath}`);
    }
    // Validate the paths array is a list of strings.
    if (!Array.isArray(manifestPaths)) {
        throw new ManifestError(`Pack manifest 'paths' must be a list of strings: ${manifestPath}`);
    }
    const typedPaths = [];
    ...
    if (sourcePrefix !== undefined &&
        ...
        throw new ManifestError(`Pack manifest 'source_prefix' must be a string when present: ${manifestPath}`);
    }
    return {
        name: manifestName,
        label: manifestLabel,
...
function computePublishedPaths(selectedPackNames, manifests) {
    ...
    for (const name of effectiveNames) {
        ...
        if (manifest === undefined) {
            throw new ManifestError(`No loaded manifest for selected pack '${name}'.`);
        }
...
function resolveVariantSourcePath(destinationRelativePath, csharpVariant) {
    ...
    if (csharpVariant === "legacy" &&
        ...
        return `${exports.LEGACY_VARIANT_SOURCE_PREFIX}/${tail}`;
    }
...
function assertSingleCsharpToolchain(publishedPaths, selectedPackNames) {
    ...
    if (selectedCsharp.length > 1) {
        throw new ManifestError("C# mutual exclusion violated: both modern and legacy C# packs were " +
