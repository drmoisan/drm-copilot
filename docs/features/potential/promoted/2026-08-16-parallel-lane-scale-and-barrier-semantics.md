# parallel-lane-scale-and-barrier-semantics (Potential Bug)

- Date captured: 2026-08-16
- Author: Dan Moisan
- Status: Promoted
- Promoted to: https://github.com/drmoisan/drm-copilot/issues/479

> Automation note: Keep the section headings below unchanged; the promotion tooling maps each of them into the GitHub bug issue template.

## Restoration Note

This promoted record was reconstructed by the orchestrator on 2026-08-16 after the promotion tooling deleted it. The content below is the authored pre-promotion text verbatim; only this note and the `Status`/`Promoted to` metadata lines were added.

Sequence observed, with evidence:

1. `potential_to_issue` reported `destination_path` `docs/features/potential/promoted/2026-08-16-parallel-lane-scale-and-barrier-semantics.md` and created issue #479.
2. The orchestrator probed that path immediately. The file was PRESENT at 6901 bytes and the pre-promotion source was correctly gone. The move had completed successfully.
3. `git checkout -b bug/parallel-lane-scale-and-barrier-semantics-479` ran.
4. `new_active_feature_folder` ran.
5. A later probe found the promoted file ABSENT, and `git status --porcelain -uall` did not list it as untracked.
6. `docs/features/potential/promoted` and `docs/features/active/2026-08-16-parallel-lane-scale-and-barrier-semantics-479` carry the identical mtime `2026-08-16 22:09:57.221665500 -0400`.

This is the fourth observation of a lost promoted record, but the first with a clean before/after bracketing the two calls. It reattributes the deletion: the record survives `potential_to_issue` and is removed during or after `new_active_feature_folder`, not by `potential_to_issue` as previously recorded. GitHub issue #479, the branch, and the active feature folder were unaffected, so the loss is local lifecycle-record only.
