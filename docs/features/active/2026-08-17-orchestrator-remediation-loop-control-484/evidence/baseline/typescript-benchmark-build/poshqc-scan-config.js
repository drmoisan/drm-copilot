...
exports.SCAN_CONFIG_RELATIVE_PATH = void 0;
exports.resolveScanConfigPath = resolveScanConfigPath;
exports.readPoshQcScanFolders = readPoshQcScanFolders;
exports.canonicalizeFolders = canonicalizeFolders;
exports.writePoshQcScanFolders = writePoshQcScanFolders;
 ...
 */
exports.SCAN_CONFIG_RELATIVE_PATH = "config/poshqc-scan.json";
/**
 ...
 */
function resolveScanConfigPath(workspaceRoot, configRelativePath = exports.SCAN_CONFIG_RELATIVE_PATH) {
    const normalizedRoot = (0, file_system_1.toPosixPath)(workspaceRoot).replace(/\/+$/, "");
    const normalizedRelative = (0, file_system_1.toPosixPath)(configRelativePath).replace(/^\/+/, "");
    if (normalizedRoot === "") {
...
function normalizeFolder(folder) {
    return (0, file_system_1.toPosixPath)(folder).replace(/^\.\//, "").replace(/\/+$/, "");
}
/**
 ...
 */
function assertValidFolderEntry(folder, configRelativePath) {
    if (typeof folder !== "string" || folder.trim() === "") {
        throw new Error(`Scan configuration '${configRelativePath}' contains a blank 'test.scanFolders' entry.`);
    }
    if (ABSOLUTE_PATH_PATTERN.test(folder)) {
        throw new Error(`Scan configuration '${configRelativePath}' entry '${folder}' must be a workspace-relative path, not an absolute path.`);
    }
    // Reject parent-directory traversal in any path segment.
    const segments = folder.split(/[\\/]+/);
    if (segments.includes("..")) {
        throw new Error(`Scan configuration '${configRelativePath}' entry '${folder}' must not contain '..' segments.`);
    }
}
/**
 ...
 */
function readPoshQcScanFolders(fileSystem, workspaceRoot, configRelativePath = exports.SCAN_CONFIG_RELATIVE_PATH) {
    ...
    try {
        ...
        throw new Error(`Invalid JSON in scan configuration '${configRelativePath}': ${detail}`, { cause: error });
    }
    if (parsed.version !== REQUIRED_CONFIG_VERSION) {
        throw new Error(`Scan configuration '${configRelativePath}' must declare 'version' equal to 1.`);
    }
    ...
    if (!Array.isArray(scanFolders)) {
        throw new Error(`Scan configuration '${configRelativePath}' 'test.scanFolders' must be an array of workspace-relative folder paths.`);
    }
    ...
    return canonicalizeFolders(scanFolders);
}
/**
 ...
 */
function canonicalizeFolders(folders) {
 ...
 * @param configRelativePath Workspace-relative configuration path.
 */
function writePoshQcScanFolders(fileSystem, workspaceRoot, folders, configRelativePath = exports.SCAN_CONFIG_RELATIVE_PATH) {
    const configPath = resolveScanConfigPath(workspaceRoot, configRelativePath);
