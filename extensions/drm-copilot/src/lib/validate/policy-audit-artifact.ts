...
export const COVERAGE_PERCENT_RE = /\b\d+(?:\.\d+)?%/;

/** Required policy-audit headings, in document order. */
export const POLICY_AUDIT_REQUIRED_HEADINGS = [
  "## Executive Summary",
...
export const POLICY_AUDIT_REQUIRED_CHECKLIST_LABELS = [
...
export const POLICY_AUDIT_COMPARISON_HEADING =
...
export const TEMPLATE_RESOLVER_TOOL = "resolve_policy_audit_template_asset";

/** Markers indicating the template resolver was unavailable. */
export const TEMPLATE_RESOLVER_MISSING_MARKERS = [
  "not exposed",
  "was not exposed",
...
/** One parsed coverage-table row keyed by canonical column name. */
interface PolicyAuditCoverageRow {
  readonly Language: string;
  readonly "Files Changed": string;
  readonly Tests: string;
  readonly "Test Result": string;
  readonly "Baseline Coverage": string;
  readonly "Post-Change Coverage": string;
  readonly "New Code Coverage": string;
}
 ...
 */
function hasNumericCoverage(value: string): boolean {
 ...
 */
function reportsMissingTemplateResolverSuccess(text: string): boolean {
 ...
 */
function extractPolicyAuditCoverageRows(
  text: string,
): PolicyAuditCoverageRow[] {
  const rows: PolicyAuditCoverageRow[] = [];
 ...
 */
function findPolicyAuditChecklistLine(
  text: string,
  label: string,
): string | null {
 ...
 */
function extractPolicyAuditComparisonLines(text: string): Map<string, string> {
  let inSection = false;
  const comparisonLines = new Map<string, string>();
 ...
 */
function comparisonLineHasLabelledPercentage(
  line: string,
  label: string,
): boolean {
 ...
 */
export function validatePolicyAuditSubstantiveRequirements(
  text: string,
): string[] {
  const errors: string[] = [];
  ...
  for (const row of coverageRows) {
    ...
    if (!comparisonLine.includes("Change:")) {
      errors.push(
        `Policy audit comparison line missing explicit change text for ${language}.`,
      );
    }
    if (
      !/Disposition:\s*(PASS|FAIL|N\/A|INCOMPLETE|BLOCKED)/.test(comparisonLine)
    ) {
      errors.push(
        `Policy audit comparison line missing disposition for ${language}.`,
      );
    }
    if (
      !isNaValue(newCode) &&
      !comparisonLineHasLabelledPercentage(
        comparisonLine,
        "New/changed-code coverage:",
      )
    ) {
      errors.push(
        `Policy audit comparison line missing numeric new/changed-code coverage for ${language}.`,
      );
    }
    if (!comparisonLine.includes("Evidence:")) {
      errors.push(
        `Policy audit comparison line missing evidence reference for ${language}.`,
      );
    } else if (hasPlaceholderMarker(comparisonLine)) {
      errors.push(
        `Policy audit comparison line still contains placeholder text for ${language}.`,
      );
    }
  }
 ...
 */
export function validatePolicyAuditText(text: string): string[] {
  const errors: string[] = [];
  ...
  if (reportsMissingTemplateResolverSuccess(text)) {
    errors.push(
      "Policy audit cannot report PASS or READY when " +
        `${TEMPLATE_RESOLVER_TOOL} is reported as missing or not exposed.`,
    );
  }
