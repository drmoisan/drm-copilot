...
export const EXPECTED_NEXT_STEP = "S5_atomic_execution";

/** Execution-through-CI statuses that remain out of scope during preparation. */
export const OUT_OF_SCOPE_STEP_KEYS = [
  "step5_status",
  "step6_status",
  "step7_status",
  "step8_status",
 ...
 */
export function validatePreparationTerminalContract(
  ...
  if (state["next_step"] !== EXPECTED_NEXT_STEP) {
    errors.push(
      "Preparation checkpoint next_step must be " +
        `${pythonRepr(EXPECTED_NEXT_STEP)}, found ${pythonRepr(state["next_step"])}.`,
    );
  }
