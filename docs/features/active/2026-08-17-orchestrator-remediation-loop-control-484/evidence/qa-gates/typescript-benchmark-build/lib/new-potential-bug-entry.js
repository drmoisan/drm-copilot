...
exports.SHORT_NAME_PATTERN = void 0;
exports.validateShortName = validateShortName;
exports.renderContent = renderContent;
exports.defaultGitConfigLookup = defaultGitConfigLookup;
...
exports.createBugEntry = createBugEntry;
...
function validateShortName(shortName) {
    ...
    if (!shortName || !exports.SHORT_NAME_PATTERN.test(shortName)) {
        throw new Error(`Aborted: '${shortName}' is invalid. Use kebab-case letters/numbers only ` +
            "(e.g., api-timeout).");
    }
}
/**
 ...
 */
function renderContent(template, shortName, entryDate, author) {
 ...
 *   `undefined`.
 */
function defaultGitConfigLookup(runner, key) {
 ...
 */
function getAuthor(gitLookup, envLookup) {
 ...
 */
function isInsidersSession(envLookup) {
 ...
 */
function resolveCodeCli(whichLookup, envLookup) {
 ...
 */
function buildDefaultAuthorProvider(runner) {
    return () => getAuthor((key) => defaultGitConfigLookup(runner, key), defaultEnvLookup);
}
/**
 ...
 */
function createBugEntry(options) {
    ...
    const authorProvider = options.authorProvider ??
        buildDefaultAuthorProvider(new subprocess_runner_1.SubprocessRunner());
    const codeLauncher = options.codeLauncher ??
        ((files) => defaultCodeLauncher(files, {
    ...
    // and OS-neutral.
    const targetDir = nodePath.posix.join((0, file_system_1.toPosixPath)(options.workspace), "docs", "features", "potential");
    const targetPath = nodePath.posix.join(targetDir, `${dateStr}-${options.shortName}.md`);
    ...
    const templatePath = options.templateRoot !== undefined
        ? nodePath.posix.join((0, file_system_1.toPosixPath)(options.templateRoot), "bug", "potential_bug.md")
        : nodePath.posix.join((0, file_system_1.toPosixPath)(options.workspace), "docs", "features", "templates", "bug", "potential_bug.md");
    ...
    if (!codeLauncher([targetPath])) {
        options.log?.("WARNING: VS Code 'code' command not found. Open file manually:");
