...
.DESCRIPTION
    ...
    Pure logic (repo-root resolution, prompt-string construction, and prompt
    colorization) is isolated into testable functions with injected
    filesystem/path seams so the behavior can be verified with deterministic
    Pester tests - no real venv, PATH dependence, live executables, or temp files.
    When the terminal background is a dark console color, the full prompt string is
    rendered in green via ANSI codes; the host background color is read only by the
    `global:prompt` shim and passed into the pure functions. The activation side
    effects are kept thin and separate from the pure logic. Tests import the
    function definitions via AST extraction without executing the entrypoint, so
    production control flow does not depend on the test harness.
...
function Test-IsDarkBackground {
    <#
    .SYNOPSIS
        Return $true when the supplied console background color is a dark color.

    .DESCRIPTION
        Pure predicate that classifies a [System.ConsoleColor] as dark. The dark
        set is Black, DarkBlue, DarkGreen, DarkCyan, DarkRed, DarkMagenta,
        DarkYellow, and DarkGray; every other console color returns $false. Used
        by the prompt color decision so the host background can be evaluated in a
        unit-testable function without host access.

    .PARAMETER BackgroundColor
        The console background color to classify.
    #>
    [CmdletBinding()]
    [OutputType([bool])]
    param(
    ...
    return $darkColors -contains $BackgroundColor
}

function Get-ColorizedPrompt {
    <#
    .SYNOPSIS
        Wrap a prompt string in green ANSI color codes when the background is dark.

    .DESCRIPTION
        Pure transform. When $IsDarkBackground is $true, returns $PromptText
        wrapped in the green ANSI sequence (ESC[32m ... ESC[0m), where ESC is
        built from [char]27. When $IsDarkBackground is $false, returns $PromptText
        unchanged. Contains no host access; the dark-background decision is passed
        in as a boolean so the function remains string-in, string-out.

    .PARAMETER PromptText
        The prompt string to colorize.

    .PARAMETER IsDarkBackground
        $true to apply the green color codes; $false to return the text unchanged.
    #>
    [CmdletBinding()]
    [OutputType([string])]
    param(
...
function Get-VenvAwarePrompt {
    ...
    .DESCRIPTION
        ...
        delegates to Get-RepoRelativePrompt. The resulting prompt string (both the
        venv-active and default forms) is then passed through Get-ColorizedPrompt,
        which wraps it in green ANSI codes when $BackgroundColor is a dark console
        color and leaves it unchanged otherwise.
    ...
    if ([string]::IsNullOrEmpty($VenvEnv)) {
        $promptText = Get-DefaultPrompt -CurrentPath $CurrentPath
    }
    else {
...
function Invoke-VenvActivation {
    ...
    function global:prompt {
        # The shim is the only place permitted to read the host background color;
        # it passes the value into the pure builder so the prompt is rendered in
        # green when the terminal is in dark mode. Pure functions stay host-free.
        Get-VenvAwarePrompt `
            -CurrentPath (Get-Location).Path `
            -VenvEnv $env:VIRTUAL_ENV `
