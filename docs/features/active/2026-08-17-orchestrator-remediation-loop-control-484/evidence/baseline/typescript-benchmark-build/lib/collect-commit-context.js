...
exports.collectCommitContext = collectCommitContext;
 ...
 */
function runGit(runner, cwd, args, allowError) {
 ...
 */
function collectCommitContext(options) {
    const { runner, fileSystem, cwd, outputPath, log } = options;
    const sections = [];
    sections.push("Please generate a commit message based on the following content:");
    sections.push("\n");
    ...
    sections.push("");
    const branch = runGit(runner, cwd, ["rev-parse", "--abbrev-ref", "HEAD"], false);
    sections.push(branch);
    sections.push("");
    // Upstream (optional; a repository may have no upstream configured).
    sections.push("===== Upstream =====");
    sections.push("");
    const upstream = runGit(runner, cwd, ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], true);
    ...
    sections.push("");
    const staged = runGit(runner, cwd, ["diff", "--cached", "--name-status"], true);
    ...
    sections.push("");
    const untracked = runGit(runner, cwd, ["ls-files", "--others", "--exclude-standard"], true);
    ...
    const pyFiles = allChanged
        ...
        : [];
    sections.push(pyFiles.length > 0 ? pyFiles.join("\n") : "(no Python files changed)");
    sections.push("");
    // Last commit (header only) (optional).
    sections.push("===== Last commit (header only) =====");
    sections.push("");
    const lastCommit = runGit(runner, cwd, ["log", "-1", "--format=%H%n%aN <%aE>%n%aD%n%cN <%cE>%n%cD%n%s%n%b"], true);
