...
The push-down publisher itself must also be exposed as a real bundled extension command, not only as a repo-local CLI entry point. That command should package and execute `scripts.dev_tools.push_down_copilot_customizations` through the same extension-side runtime/script-launch path already used for PR-context collection so destination workspaces can invoke the documented publisher flow without relying on local `scripts/` paths.
...
- Main path:
	...
	- Replace references to implemented tooling with canonical textual extension-command references aligned with the packaged command surface, including real bundled commands for push-down publishing and PR-context collection.
...
- Outputs (artifacts, logs, telemetry)
	...
	- Extension-side execution of the bundled push-down command should use the same output/error path and packaged-resource launch pattern as the bundled PR-context command.
...
- Contracts and validation rules:
	...
	- The bundled push-down extension command must execute the packaged Python resource rather than assuming the source repository's `scripts/` tree exists in the destination workspace.
...
- [ ] Behavior matches acceptance criteria in the documented Windows workspace flow, including overwrite semantics, textual command rewrites, bundled execution of the push-down command, and deterministic placeholder failures.
- [ ] Tests updated/added, including Python coverage for `push_down_copilot_customizations` and extension coverage for the real push-down command contribution, bundled-resource execution, PR-context command parity, and placeholder execution paths.
...
- [ ] Telemetry/logging added or updated via the push-down summary artifact and extension output/error reporting for both the real bundled push-down command and placeholder commands.
- [x] Toolchain pass completed (format → lint → type-check → test) for the touched Python and extension surfaces in one final clean pass.

## Seeded Test Conditions (from potential)
- [x] Unit coverage areas: root-folder enumeration, destination directory creation, overwrite of existing files, rewrite of known script-reference patterns, and pass-through of content that should not be rewritten.
- [ ] Integration scenarios: push into an empty workspace, push into a workspace with conflicting `.github` files, invoke the bundled push-down extension command from a destination workspace, and validate that rewritten references resolve to packaged extension commands instead of destination-workspace script paths.
- [ ] CLI/API examples: one-way push invocation from source repo to destination workspace, bundled extension-command invocation of the packaged push-down resource, expected artifact/log output for copied and rewritten files, and placeholder-command invocation that raises a deterministic not-implemented error.
- [x] Path/OS edge cases: Windows-style backslashes and mixed `scripts/dev_tools` vs `scripts/dev-tools` references should normalize to the same packaged-command target.
