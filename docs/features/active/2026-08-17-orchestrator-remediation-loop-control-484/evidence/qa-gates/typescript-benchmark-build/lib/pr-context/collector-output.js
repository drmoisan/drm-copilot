...
function renderVerificationEvidenceSection(fs, resolvedRoot, featureDocs) {
    ...
    for (const record of sorted) {
        ...
        lines.push(`- Feature: ${record.feature}`, `  - Source: ${record.sourceFile}`, `  - Timestamp: ${record.timestamp}`, `  - Command: ${record.command}`, `  - EXIT_CODE: ${record.exitCode}`, ...expectedRows, `  - Normalized result: ${record.normalizedResult}`);
    }
    return lines.join("\n");
}
/**
