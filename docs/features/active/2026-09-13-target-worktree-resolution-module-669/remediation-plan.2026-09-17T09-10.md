...
- [x] [P0-T1] Read the required policy files in order and record the read
...
- [x] [P0-T2] Confirm the remediation base SHA is an ancestor of `HEAD` and record the pre-run worktree state
...
- [x] [P0-T3] Record the pre-run last-write timestamps of the two canonical PoshQC run-output files
...
- [x] [P1-T1] Run the repo-wide self-hosted PoshQC test invocation with no scan-folder restriction
...
- [x] [P1-T2] Copy the two produced run-output files into feature evidence before any later run overwrites them
...
- [x] [P1-T3] Extract the JUnit run counts and failing-test set from the copied file
...
- [x] [P1-T4] Extract the report-level LINE coverage counter and the two `worktree-resolution` per-file LINE rows from the copied coverage XML
...
- [x] [P2-T1] Write the repo-wide coverage delta and threshold-verification artifact
...
- [x] [P2-T2] Verify the failing-test set is a subset of the baseline failure set
...
- [x] [P2-T3] Confirm no file outside this feature's evidence folder and the remediation plan itself changed
