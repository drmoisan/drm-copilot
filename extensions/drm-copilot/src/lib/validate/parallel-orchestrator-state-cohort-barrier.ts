 ...
 */
const GATING_KEYS: readonly string[] = ["conflict_edges", "cohorts"];
 ...
 */
const FOLDER_HINT_PREFIXES: readonly string[] = [
...
/** The two reference forms a cohort member or an edge endpoint may take. */
interface ReferenceIndex {
  /** Every well-keyed `items[]` entry, keyed by its `issue_num` primary key. */
  readonly records: Map<number, Record<string, unknown>>;
  /** Normalized `feature_folder` hints mapped onto the same primary keys. */
  readonly byFolderHint: Map<string, number>;
}
 ...
 */
function normalizeFolderHint(value: string): string {
 ...
 */
function buildReferenceIndex(items: unknown): ReferenceIndex {
  const records = new Map<number, Record<string, unknown>>();
  const byFolderHint = new Map<string, number>();
...
function cohortIndexByItem(
  ...
  for (const entry of cohorts) {
    if (!isObject(entry) || entry["generation"] !== recolorGeneration) {
      continue;
    }
    const cohortIndex = entry["index"];
    const itemKeys = entry["item_keys"];
    if (
      typeof cohortIndex !== "number" ||
      !isNonNegativeInteger(cohortIndex) ||
 ...
 */
function hasStarted(record: Record<string, unknown>): boolean {
  ...
  return (
    typeof mergeStatus === "string" && mergeStatus !== NOT_STARTED_MERGE_STATUS
  );
}
 ...
 * durably merged work.
 */
function satisfiesBarrier(record: Record<string, unknown>): boolean {
  return isEnumMember(MERGED_MERGE_STATUSES, record["merge_status"]);
}
 ...
 */
function mergeConfirmedAfterStart(
...
function violationEndpoints(
  ...
  // since moved past.
  const [earlierKey, laterKey]: readonly [number, number] =
 ...
 */
export function validateCohortBarrierOrdering(
