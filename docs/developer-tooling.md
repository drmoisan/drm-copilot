...
### Host Tooling Bootstrap (No Dev Container)
...
1. Run bootstrap once with install mode (`-Apply` / `--apply`).
2. Run the matching verify script and ensure it exits with code 0.
3. Run `poetry install --with dev` from the repo root.
4. Run `poetry run python -m scripts.dev_tools.fix_all`.

The host tooling manifest is derived from the devcontainer sources:
...
- **Pre-flight QC**: Before executing any tasks, the executor runs a full QC check based on auto-QC detection (Python: Black/Ruff/Pyright/Pytest; TypeScript: npm format/lint/typecheck/test:unit). If baseline QC fails, Copilot is invoked to fix the issues and must run the toolchain itself until passing. If a required executable (for example, `poetry` or `npm`) is missing on PATH, pre-flight QC fails fast with an actionable error. Use `--skip-preflight-qc` to bypass this check.
