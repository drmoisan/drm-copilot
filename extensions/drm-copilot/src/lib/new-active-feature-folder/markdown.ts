...
import { PLACEHOLDERS } from "./models";
 ...
 */
export function formatChecklist(text: string): string {
  const lines: string[] = [];
 ...
 */
export function getSection(content: string, name: string): string {
  const pattern = new RegExp(
    `^\\s*##\\s+${escapeRegExp(name)}\\s*\\r?\\n([\\s\\S]*?)(?=^\\s*##\\s+|$(?![\\s\\S]))`,
    "m",
  );
 ...
 */
export function upsertWorkModeMarker(content: string, mode: string): string {
  const markerLine = `- Work Mode: ${mode}`;
  const markerPattern =
    /^- Work Mode:\s*(minor-audit|full-feature|full-bug|full)\s*$/;
...
export function setSection(
  ...
  if (pattern.test(content)) {
    ...
    return content.replace(
      pattern,
      (_match, group1: string) => `${group1}${body}\n\n`,
    );
  }
 ...
 */
export function prependToSectionBody(
  sectionBody: string,
  prefix: string,
): string {
...
export function updateSectionBody(
  ...
  const pattern = new RegExp(
    `(^##\\s+${escapeRegExp(sectionName)}\\s*\\r?\\n)([\\s\\S]*?)(?=^\\s*##\\s+|$(?![\\s\\S]))`,
    "m",
  );
  const match = pattern.exec(content);
  if (!match) {
 ...
 */
export function setHeaderPlaceholder(
  content: string,
  featureName: string,
  issueField: string,
  ownerField: string,
  ...
  result = replaceRegexLiteral(
    result,
    /^-\s*\*\*Owner:\*\*\s+(?:name|<name>|.*)$/gm,
    `- **Owner:** ${ownerField}`,
  );
  result = replaceRegexLiteral(
    result,
    /^-\s*Owner\s*:\s+(?:name|<name>|.*)$/gm,
    `- Owner: ${ownerField}`,
  );
  ...
  if (parentField !== undefined) {
    result = replaceRegexLiteral(
      result,
      /^-\s*\*\*Parent \(optional\):\*\*\s+.*$/gm,
      `- **Parent (optional):** ${parentField}`,
    );
    result = replaceRegexLiteral(
      result,
      /^-\s*Parent \(optional\)\s*:\s+.*$/gm,
      `- Parent (optional): ${parentField}`,
    );
  }
