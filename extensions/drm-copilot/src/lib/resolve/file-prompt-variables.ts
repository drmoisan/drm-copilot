 ...
 */
export function splitPathPlatformAgnostic(pathStr: string): string[] {
 ...
 */
export function tryRelativeToWorkspace(
  path: string,
 ...
 */
export function resolveFeatureFoldername(folderpath: string): string {
 ...
 */
export function resolveNameFromFeatureFoldername(
 ...
 */
export function resolveSpecPath(folderpath: string): string {
 ...
 */
export function resolveUserStoryValue(
  folderpath: string,
  workspaceRoot: string,
  fs: FileSystem,
): string {
  const relativeStory = joinFolder(folderpath, "user-story.md");
  const fullStory = joinFolder(toPosixPath(workspaceRoot), relativeStory);
 ...
 */
export function resolveResearchValue(
  folderpath: string,
  workspaceRoot: string,
  fs: FileSystem,
): string | null {
  const relativeResearch = joinFolder(folderpath, "research.md");
  const fullResearch = joinFolder(toPosixPath(workspaceRoot), relativeResearch);
 ...
 */
function joinFolder(folder: string, leaf: string): string {
  const normalizedFolder = toPosixPath(folder).replace(/\/+$/, "");
...
/**
 * Resolved work-mode outcome for prompt substitution.
 */
...
export function resolveWorkModeFromIssue(
  ...
  const issuePath = joinFolder(
    joinFolder(toPosixPath(workspaceRoot), folderpath),
    "issue.md",
  );
