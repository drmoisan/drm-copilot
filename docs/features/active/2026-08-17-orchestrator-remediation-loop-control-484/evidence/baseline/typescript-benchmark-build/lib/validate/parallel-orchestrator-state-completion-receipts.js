...
exports.validateCompletionReceipts = validateCompletionReceipts;
...
function receiptMode(items) {
    return items.some((item) => COMPLETION_RECEIPT_FIELDS.some((field) => field in item));
}
...
function itemError(context, index, detail) {
    return `${context} items[${index}] completion receipt ${detail}.`;
}
function validatePrBinding(item, context, index) {
    const errors = [];
    ...
    if (!nonEmptyText(branchName)) {
        errors.push(itemError(context, index, "branch_name must be a non-empty string"));
    }
    if (item["pr_head_branch"] !== branchName) {
        errors.push(itemError(context, index, "PR head branch must match branch_name"));
    }
    const checkedHead = item["checked_head"];
    const prHead = item["pr_head_sha"];
    if (!isSha(checkedHead)) {
        errors.push(itemError(context, index, "checked_head must be a 40-character SHA"));
    }
    if (!isSha(prHead)) {
        errors.push(itemError(context, index, "pr_head_sha must be a 40-character SHA"));
    }
    else if (prHead !== checkedHead) {
        errors.push(itemError(context, index, "PR head SHA must match checked_head"));
    }
    return errors;
}
function validateCheckBinding(item, context, index) {
    const errors = [];
    const checksHead = item["checks_head_sha"];
    const prHead = item["pr_head_sha"];
    if (!isSha(checksHead)) {
        errors.push(itemError(context, index, "checks_head_sha must be a 40-character SHA"));
    }
    else if (checksHead !== prHead) {
        errors.push(itemError(context, index, "checks head SHA must match pr_head_sha"));
    }
    if (item["checks_conclusion"] !== "success") {
        errors.push(itemError(context, index, "required checks conclusion must be 'success'"));
    }
    return errors;
}
function validateMergeAndRemoval(item, context, index) {
    const errors = [];
    if (item["pr_state"] !== "MERGED") {
        errors.push(itemError(context, index, "PR state must be 'MERGED'"));
    }
    if (!nonEmptyText(item["merged_at"])) {
        errors.push(itemError(context, index, "merged_at must be a non-empty string"));
    }
    if (!isSha(item["merge_commit_sha"])) {
        errors.push(itemError(context, index, "merge_commit_sha must be a 40-character SHA"));
    }
    if (!isRepositoryRelativePath(item["merge_receipt_path"])) {
        errors.push(itemError(context, index, "merge_receipt_path must be repository-relative"));
    }
    if (item["merge_status"] !== "worktree_removed") {
        errors.push(itemError(context, index, "merge_status must be 'worktree_removed'"));
    }
    if (!nonEmptyText(item["worktree_removed_at"])) {
        errors.push(itemError(context, index, "worktree_removed_at must be a non-empty string"));
    }
    if (!isRepositoryRelativePath(item["worktree_removal_receipt_path"])) {
        errors.push(itemError(context, index, "worktree_removal_receipt_path must be repository-relative"));
    }
    if (!isRepositoryRelativePath(item["completion_receipt_path"])) {
        errors.push(itemError(context, index, "completion_receipt_path must be repository-relative"));
    }
    return errors;
}
/**
 ...
 */
function validateCompletionReceipts(state, context) {
    ...
    const duplicatePrNumbers = new Set();
    ...
    for (const prNumber of [...duplicatePrNumbers].sort((left, right) => left - right)) {
        errors.push(`${context} completion receipts assign PR ${prNumber} to multiple items.`);
    }
    return errors;
}
