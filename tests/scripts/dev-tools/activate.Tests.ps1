...
.DESCRIPTION
    ...
    venv, PATH dependence, live executables, or temp files:
      ...
      - Test-IsDarkBackground (dark console-color predicate)
      - Get-ColorizedPrompt  (green ANSI wrapping for dark backgrounds)
...
Describe 'Test-IsDarkBackground (dark-color predicate)' {
    It 'returns true for the dark console color <Color>' -ForEach @(
        ...
        Test-IsDarkBackground -BackgroundColor $Color | Should -BeTrue
    }

    It 'returns false for the non-dark console color <Color>' -ForEach @(
        ...
        Test-IsDarkBackground -BackgroundColor $Color | Should -BeFalse
    }

    It 'classifies the DarkGray boundary as dark and its Gray counterpart as not dark' {
        # Boundary case: DarkGray is in the dark set; Gray is not.
        Test-IsDarkBackground -BackgroundColor ([System.ConsoleColor]::DarkGray) | Should -BeTrue
        Test-IsDarkBackground -BackgroundColor ([System.ConsoleColor]::Gray) | Should -BeFalse
    }
}

Describe 'Get-ColorizedPrompt (green ANSI wrapping)' {
    BeforeAll {
        $script:Esc = [char]27
    ...
    It 'wraps the prompt in green ESC[32m ... ESC[0m when the background is dark' {
        Get-ColorizedPrompt -PromptText 'PS C:\repo> ' -IsDarkBackground $true |
            Should -Be "$script:Green`PS C:\repo> $script:Reset"
    }

    It 'returns the prompt unchanged when the background is not dark' {
        Get-ColorizedPrompt -PromptText 'PS C:\repo> ' -IsDarkBackground $false |
            Should -Be 'PS C:\repo> '
    }

    It 'returns an empty string unchanged when the background is not dark' {
        Get-ColorizedPrompt -PromptText '' -IsDarkBackground $false |
            Should -Be ''
    }

    It 'wraps an empty string with the green codes when the background is dark' {
        Get-ColorizedPrompt -PromptText '' -IsDarkBackground $true |
            Should -Be "$script:Green$script:Reset"
    }
}

Describe 'Get-VenvAwarePrompt (prompt decision used by the shim)' {
    BeforeAll {
        # ESC[32m ... ESC[0m green wrapping applied for dark backgrounds.
        $script:Esc = [char]27
        $script:Green = "$script:Esc[32m"
        $script:Reset = "$script:Esc[0m"
    }

    It 'falls back to the default form when the venv value is empty (light background, unchanged)' {
        Get-VenvAwarePrompt -CurrentPath 'C:\repo\mix-calculator' -VenvEnv '' `
            -BackgroundColor ([System.ConsoleColor]::White) |
            Should -Be 'PS C:\repo\mix-calculator> '
    }

    It 'falls back to the default form when the venv value is null (light background, unchanged)' {
        Get-VenvAwarePrompt -CurrentPath 'C:\repo\mix-calculator' -VenvEnv $null `
            -BackgroundColor ([System.ConsoleColor]::White) |
            Should -Be 'PS C:\repo\mix-calculator> '
    }

    It 'derives the repo root from the venv parent and renders the at-root prompt (light background)' {
    ...
    It 'renders a descendant prompt from the venv parent (light background)' {
        Get-VenvAwarePrompt -CurrentPath 'C:\repo\mix-calculator\.claude' -VenvEnv 'C:\repo\mix-calculator\.venv' `
            -BackgroundColor ([System.ConsoleColor]::White) |
            Should -Be '(mix-calculator)\.claude> '
    }

    It 'wraps the default form in green when the background is dark' {
        Get-VenvAwarePrompt -CurrentPath 'C:\repo\mix-calculator' -VenvEnv '' `
            -BackgroundColor ([System.ConsoleColor]::Black) |
            Should -Be "$script:Green`PS C:\repo\mix-calculator> $script:Reset"
    }

    It 'wraps the venv-active form in green when the background is dark' {
        Get-VenvAwarePrompt -CurrentPath 'C:\repo\mix-calculator' -VenvEnv 'C:\repo\mix-calculator\.venv' `
            -BackgroundColor ([System.ConsoleColor]::Black) |
...
Describe 'Invoke-VenvActivation (orchestration; seams mocked)' {
    ...
    It 'installs a prompt that delegates to the venv-aware builder' {
        ...
        # The prompt may be wrapped in a leading green ANSI sequence when the test
        # host background is dark, so allow an optional ESC[..m prefix before the
        # (project-name) segment.
        $env:VIRTUAL_ENV = 'C:\repo\mix-calculator\.venv'
        $esc = [char]27
        prompt | Should -Match "^($esc\[\d+m)?\(mix-calculator\)"
    }
}
