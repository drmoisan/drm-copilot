...
- **Owner:** drmoisan
- **Last Updated:** 2026-04-30T09-05
- **Status:** Superceded by 2.0
- **Version:** 1.0

## Overview

This feature defines a deterministic converter that ingests supported agent assets from other ecosystems and emits only Codex-native outputs for this repository. The v1 target is a full-feature conversion surface for supported GitHub Copilot and Claude inputs, with review-first reporting, explicit unsupported classifications, fail-closed enforcement handling, and a compiler-style intermediate state that captures parsing, classification, planning, and traceability before final emission.
...
The converter accepts a supported source root or an explicit subset of supported source files, determines the source ecosystem, parses source artifacts into a typed intermediate representation, classifies each source artifact and section, plans deterministic native emissions, and maps them into a Codex-native target surface or a blocking unsupported result.
...
2. The converter enumerates supported input artifacts in deterministic order and parses each item into a typed intermediate representation.
3. The converter classifies each artifact and section by source kind, semantic intent, conversion class, target role, and target surface.
4. The converter applies mapping rules and rewrite rules, including semantic MCP rewrites to `drmCopilotExtension` when a verified mapping exists.
5. The converter validates the proposed outputs for unresolved hard gates, unresolved handoff semantics, unsupported mappings, duplicate content placement, and lingering source-runtime references.
6. The converter writes a review artifact set under an artifact root without writing Codex-native runtime files into a destination root.
7. After approval, the user reruns the converter in apply mode with an explicit destination root.
8. The converter writes the approved Codex-native outputs and the same report artifacts, or stops without partial success when a blocking validation failure is present.
...
- If a GitHub source artifact expresses required handoff or delegation behavior, the converter must preserve that behavior through verified native composition such as subagents, shared skills, hooks, rules, and required configuration rather than downgrading it to advisory text.
- If the caller requests intermediate-state exposure, the converter emits machine-readable intermediate artifacts beneath the artifact root without changing conversion semantics.
...
- Intermediate-state exposure: optional boolean that writes the compiler-like intermediate state into the artifact root for user inspection.
...
- Review and apply report artifacts:
  ...
  - `proposed-tree/**`
- Optional intermediate-state artifacts when explicitly enabled:
  - `intermediate/source-artifacts.json`
  - `intermediate/section-intents.json`
  - `intermediate/planned-emissions.json`
  - `intermediate/translation-traces.json`
- Logs:
...
- Default intermediate-state exposure: disabled unless explicitly enabled.
...
- v1 must preserve a deterministic compiler-like intermediate state even when the caller does not request that the state be written into artifacts.
...
- Review mode
  ...
  - `poetry run python -m scripts.dev_tools.codex_native_converter review --source-root <path> --source-ecosystem <github-copilot|claude> [--selected-path <path> ...] [--artifact-root <path>] [--enable-repo-prompts] [--emit-intermediate-state]`
- Apply mode
  - `poetry run python -m scripts.dev_tools.codex_native_converter apply --source-root <path> --source-ecosystem <github-copilot|claude> --destination-root <path> [--selected-path <path> ...] [--artifact-root <path>] [--enable-repo-prompts] [--emit-intermediate-state]`
...
- `emit_intermediate_state` controls only artifact exposure; it must not alter classification, planning, validation, or emitted runtime content.
...
### Compiler-like intermediate state

The converter must maintain a typed intermediate state between discovery and final emission. This state is part of the required behavior, not an optional implementation detail.

Required intermediate entities:
...
- `SourceSection`
  ...
  - `cues`
- `SemanticCue`
  - cue kinds sufficient to capture heading structure, workflow structure, hard-gate language, forbidden-action language, launcher wrappers, tool requirements, and similar classification evidence
- `SectionIntent`
  - `source_path`
  - `section_id`
  - `heading`
...
- `PlannedEmission`
  ...
  - `notes`
- `TranslationTrace`
  - source section to target mapping trace used for reporting and topology visualization

Required section-intent coverage:
...
- Discovery must precede parsing.
- Parsing must precede section-level classification.
- Section-level classification must precede emission planning.
- Emission planning must precede rendering and validation.
- The same source set and options must produce the same intermediate state.
- The intermediate state must be sufficient to explain every emitted output and every blocking validation result.

Artifact exposure option:

- When `emit_intermediate_state` is enabled, the converter must write the intermediate state into the artifact root in a deterministic machine-readable form.
- When `emit_intermediate_state` is disabled, the converter must still construct the same intermediate state internally.
- Intermediate-state artifacts are review aids and must not be consumed as apply-mode runtime configuration.
...
- GitHub handoff and delegation semantics are mandatory conversion requirements. When a GitHub source artifact encodes required handoff behavior, the converter must map that behavior into verified native mechanisms such as subagents, shared skills, hooks, rules, and required MCP/config surfaces.
...
- Known prompt content may map to shared skills, hooks, or other native surfaces even when repository prompt launchers are disabled, provided the mapping is verified and deterministic.
...
- Persistent state is limited to the report artifact set, optional intermediate-state artifact exposure, and apply-mode target outputs.
...
- Handoff behavior is constrained by verified Codex-native mechanisms. There is no requirement for a first-class native `handoffs:` manifest field as long as the converter preserves the required behavior through verified composition using subagents, shared skills, hooks, rules, and required configuration.
...
- Build the converter as a compiler-style pipeline with explicit parse, classify, plan, render, and validate phases.
- The Python CLI is the authoritative converter surface; the TypeScript layer is a thin bundled command and MCP wrapper over the same Python engine.
- Define a machine-readable mapping catalog schema and validation schema.
- Define a machine-readable schema for the intermediate compiler-like state when artifact exposure is enabled.
...
- Source parser and section splitter
- Source artifact classifier module
- Section-intent classifier module
- Mapping planner module
- Section-emission planner module
...
- Optional intermediate-state artifact writer
...
- GitHub handoff intent is mandatory and must be preserved when it represents required runtime behavior.
- GitHub handoff preservation may be represented through thin `.codex/agents/*.toml` manifests plus shared skills, standing guidance, hooks, rules, and required configuration, provided the resulting composition preserves the original required behavior.
- The converter must not downgrade required GitHub handoff behavior into documentation-only text, comments, or manual follow-up instructions.
- If a required source handoff cannot be preserved through verified native composition, the converter must classify that behavior as unsupported and block apply mode.
