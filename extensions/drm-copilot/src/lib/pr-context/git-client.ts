...
/** Options accepted by {@link GitClient.run}. */
export interface GitRunOptions {
  /** When true, a non-zero exit is tolerated rather than raising. */
...
export class GitClient {
   ...
   */
  constructor(runner: CommandRunner, cwd: string, fileSystem: FileSystem) {
    this.runner = runner;
    this.cwdValue = cwd;
   ...
   */
  run(args: readonly string[], options?: GitRunOptions): CommandResult {
   ...
   */
  diffNameStatus(options: { staged: boolean }): string {
   ...
   */
  diffPatch(options: { staged: boolean }): string {
    const args = ["diff"];
    if (options.staged) {
      args.push("--cached");
    }
