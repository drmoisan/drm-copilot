# R2 Modified PowerShell Owners Batch A Expected-Red Evidence

Timestamp: 2026-08-13T17-43-04:00
...
```
EXIT_CODE: 1
Output Summary: All three named Batch A owners failed the 80% line threshold. The current bundled MCP XML omitted the owner classes, so the command did not infer counters; it transparently preserved the exact source-attributed pre-remediation values from the authoritative QA receipt and labeled each result `AUTHORITATIVE_PRE_REMEDIATION_RECEIPT_XML_OWNER_MISSING`.

| Owner | Covered | Total | Line coverage | Current XML status | Result |
|---|---:|---:|---:|---|---|
| `.codex/hooks/codex-authority-store.ps1` | 46 | 58 | 79.310345% | MISSING | FAIL_BELOW_80 |
| `.codex/hooks/enforce-codex-model-routing.ps1` | 46 | 79 | 58.227848% | MISSING | FAIL_BELOW_80 |
| `.codex/hooks/record-subagent-routing-attestation.ps1` | 111 | 229 | 48.471616% | MISSING | FAIL_BELOW_80 |
