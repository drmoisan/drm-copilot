...
class TestSubprocessRunner:
    ...
    def test_run_success(
        ...
        result = runner.run(["echo", "test"], cwd=mem_fs_path)
...
class TestGitClient:
    ...
    @pytest.fixture
    def git_client(self, mock_runner: Mock, mem_fs_path: Path) -> GitClient:
        """GitClient instance with mock runner."""
        return GitClient(mock_runner, mem_fs_path)
    ...
    def test_resolve_root_when_git_dir_exists(
        self, git_client: GitClient, mock_runner: Mock, mem_fs_path: Path
    ) -> None:
        """GitClient.resolve_root returns cwd when .git exists."""
        git_dir = mem_fs_path / ".git"
        git_dir.mkdir()

        root = git_client.resolve_root()

        assert root == mem_fs_path
