# Batch B21 gate — `.claude/lib/mermaid/MermaidLineScanner.psm1`

Timestamp: 2026-08-30T01-14
...
- Command 5 summary line, verbatim after removal of ANSI colour escapes: `Tests Passed: 3873, Failed: 0, Skipped: 9, Inconclusive: 0, NotRun: 0`. `Invoke-PoshQCTest` emits the summary across several `Write-Information` records, so the record stream was concatenated before the escape sequences were stripped.
...
- `Tests Passed: ` line shows `Failed: 0`. Holds; the passed and skipped counts equal the post-merge baseline `[P0-T16]` values of 3873 and 9.
