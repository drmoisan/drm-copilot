# Root Lockfile Regeneration — `npm install` (#414, [P1-T2])

Timestamp: 2026-07-25T21-40

Command: `npm install` (working directory: repository root, AFTER the [P1-T1] `overrides` edit)
EXIT_CODE: 0

## Verbatim Output

```text
removed 8 packages, changed 2 packages, and audited 526 packages in 1s
...
No `brace-expansion` node at `2.1.2` or `5.0.7` remains; no `minimatch` node at any `9.x` version remains. The previously nested `node_modules/minimatch/node_modules/brace-expansion@2.1.2` node is gone — the unscoped `minimatch: ^10.2.5` override deduplicated the tree to a single hoisted node of each package.
