 ...
 */
export const COHORT_COVERAGE_EXEMPT_STATES: readonly string[] =
  "withdrawn merged blocked".split(" ");

/** Optional receipt arrays validated for list type only (invariant 19). */
export const RECEIPT_ARRAY_KEYS: readonly string[] =
  "delegation_receipts skill_receipts mcp_call_receipts".split(" ");

/** One `(issueNum, state)` pair extracted from a well-formed `items[]` entry. */
export interface ItemKeyState {
  /** The entry's positive-integer primary key. */
  readonly issueNum: number;
  /** The entry's unvalidated `state` value, rendered verbatim by callers. */
 ...
 */
export function collectItemKeysAndStates(items: unknown): ItemKeyState[] {
  const pairs: ItemKeyState[] = [];
 ...
 */
export function collectIssueNumbers(items: unknown): Set<number> {
...
function validateCohortEntry(
  ...
  recolorGeneration: number | null,
): string[] {
  const errors: string[] = [];
  const index = entry["index"];
  if (!isNonNegativeInteger(index)) {
    errors.push(
      `${entryContext} index must be a non-negative integer; found: ${pythonRepr(index)}.`,
    );
  }
  ...
  if (!isNonNegativeInteger(generation)) {
    errors.push(
      `${entryContext} generation must be a non-negative integer; found: ${pythonRepr(generation)}.`,
    );
  } else if (
    recolorGeneration !== null &&
  ...
  for (const key of itemKeys) {
    if (
      ...
      errors.push(
        `${entryContext} item_keys entry ${pythonRepr(key)} does not resolve to an items[].issue_num.`,
      );
    }
  }
  return errors;
 ...
 */
export function validateCohortShapes(
  cohorts: unknown,
  issueNums: ReadonlySet<number>,
  ...
  if (!generationOk) {
    errors.push(
      `${context} recolor_generation must be a non-negative integer; found: ${pythonRepr(recolorGeneration)}.`,
    );
  }
  ...
  const bound =
    generationOk && typeof recolorGeneration === "number"
      ? recolorGeneration
      : null;
  ...
  cohorts.forEach((entry: unknown, position: number) => {
 ...
 */
function currentGenerationCohorts(
  cohorts: unknown,
  recolorGeneration: unknown,
 ...
 */
export function validateCurrentGenerationCohorts(
  cohorts: unknown,
  items: unknown,
  recolorGeneration: unknown,
  context: string,
  ...
  for (const { issueNum, state } of pairs) {
    ...
    errors.push(
      `${context} item ${issueNum} in state ${pythonRepr(state)} must appear in exactly one current-generation cohort; found ${count}.`,
    );
  }
  return errors;
}
 ...
 */
export function validateCurrentCohortBound(
  ...
  if (
    ...
    return [
      `${context} current_cohort must be a non-negative integer; found: ${pythonRepr(currentCohort)}.`,
    ];
  }
...
function validateEdgeEndpoints(
  ...
  entryContext: string,
  issueNums: ReadonlySet<number>,
): { errors: string[]; pair: [number, number] | null } {
  const errors: string[] = [];
  ...
  for (const field of ["a", "b"]) {
    ...
    if (
      ...
      errors.push(
        `${entryContext} ${field} ${pythonRepr(value)} does not resolve to an items[].issue_num.`,
      );
    }
  }
  if (endpoints.size !== 2) {
 ...
 */
export function validateConflictEdges(
  edges: unknown,
  issueNums: ReadonlySet<number>,
  context: string,
): string[] {
  ...
  const duplicates = new Map<string, [number, number]>();
  ...
  edges.forEach((entry: unknown, position: number) => {
 ...
 */
export function validateReceiptArrays(
