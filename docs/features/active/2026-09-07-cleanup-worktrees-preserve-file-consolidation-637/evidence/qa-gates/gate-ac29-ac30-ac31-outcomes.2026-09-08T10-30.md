...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-29, AC-30, and AC-31.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 302 a missing source file is reported as MISSING-SOURCE
    ok 303 an ignored target path is refused without a force flag
    ok 304 the preserve exit codes distinguish clean, skipped, and blocked runs
