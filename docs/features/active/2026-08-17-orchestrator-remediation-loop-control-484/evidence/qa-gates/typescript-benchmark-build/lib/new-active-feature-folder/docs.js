...
exports.updateFeatureDocs = updateFeatureDocs;
exports.shouldUseMinorAuditMode = shouldUseMinorAuditMode;
 ...
 */
function applyHeaderAndSections(path, featureName, issueField, ownerField, updatedField, parentField, statusField, versionField, fs, updates) {
    if (!fs.exists(path)) {
        return;
    }
    ...
    for (const [sectionName, body] of updates) {
        content = (0, markdown_1.setSection)(content, sectionName, body);
    }
 ...
 */
function updateFeatureDocs(featureType, featureName, targetDir, issueField, ownerField, updatedField, parentField, statusField, versionField, planUpdatedField, fs, sections, planPath) {
    const filesToOpen = [];
    ...
    if (featureType === "feature") {
        const userStory = (0, models_1.joinPosix)(targetDir, "user-story.md");
        const spec = (0, models_1.joinPosix)(targetDir, "spec.md");
        const plan = planPath ?? (0, models_1.joinPosix)(targetDir, "plan.md");
        applyHeaderAndSections(userStory, featureName, issueField, ownerField, updatedField, parentField, statusField, versionField, fs, [
            ["Problem / Why", get("problem")],
            ["Acceptance Criteria", (0, markdown_1.formatChecklist)(get("criteria"))],
        ]);
        applyHeaderAndSections(spec, featureName, issueField, ownerField, updatedField, parentField, statusField, versionField, fs, [
            ["Overview", get("problem")],
            ["Behavior", get("behavior")],
            ["Constraints & Risks", get("constraints")],
            [
                "Seeded Test Conditions (from potential)",
                (0, markdown_1.formatChecklist)(get("tests")),
            ],
        ]);
        applyHeaderAndSections(plan, featureName, issueField, ownerField, planUpdatedField, parentField, statusField, versionField, fs, []);
        ...
        const epic = (0, models_1.joinPosix)(targetDir, "epic.md");
        applyHeaderAndSections(epic, featureName, issueField, ownerField, updatedField, parentField, statusField, versionField, fs, []);
        ...
        applyHeaderAndSections(spec, featureName, issueField, ownerField, updatedField, parentField, statusField, versionField, fs, updates);
        ...
        if (bugValidation) {
            ...
            const updateTestStrategy = (body) => (0, markdown_1.prependToSectionBody)(body, `Seeded from issue:\n\n${bugValidation}`);
            [specContent] = (0, markdown_1.updateSectionBody)(specContent, "Test Strategy", updateTestStrategy);
 ...
 */
function shouldUseMinorAuditMode(workMode, featureType, potentialContent) {
    void featureType;
