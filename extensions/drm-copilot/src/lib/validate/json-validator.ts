...
import { iterGovernedFiles } from "../json-config";

/**
 * Governed-JSON `$schema` validator (in-process port).
 *
 * Purpose:
 *     Port `scripts/dev_tools/validate_json.py`. Validate JSON documents against
 *     their declared `$schema` using a minimal built-in schema checker and the
 *     F1 governed-file collection.
 *
 * Responsibilities:
 *     - `collectSchemaErrors`: the minimal `type: object` / `required` / `number`
 *       checker.
 *     - `validateFile`: load a JSON document, resolve its schema, and report
 *       success or a descriptive error.
 *     - `collectTargets`: resolve explicit path/dir targets or fall back to
 *       `iterGovernedFiles`.
 *     - `runValidation`: aggregate per-file results into a typed result.
 *
 * Accepted divergence:
 *     Remote (`http`/`https`) schema fetching is intentionally excluded from the
 *     in-process port. `loadSchema` supports local relative and `file:` URIs
 *     only and throws `Unsupported schema URI scheme: <scheme>` for `http`/
 *     `https`. See the divergence note recorded under
 *     `evidence/regression-testing/json-validator-remote-schema-divergence.md`.
 *
 * Invariants / Constraints:
 *     - Local-path error-message strings are identical to the Python source.
 *     - I/O is injected via the F1 `FileSystem` so validation stays hermetic.
 *
 * Side Effects:
 *     None directly; the injected `FileSystem` performs reads.
 */

/** Aggregate validation outcome for a run. */
export interface ValidateResult {
  /** True when at least one file failed validation. */
  failed: boolean;
  /** Collected status or error messages, safe to print directly. */
  messages: string[];
}
...
export function loadSchema(
  ...
  if (scheme === "") {
    ...
    return JSON.parse(fs.readTextFile(localPath)) as Record<string, unknown>;
  }
 ...
 */
export function validateFile(fs: FileSystem, path: string): [boolean, string] {
  let dataRaw: unknown;
  try {
...
export function collectTargets(
  ...
  if (paths.length > 0) {
    ...
    for (const candidate of paths) {
      ...
      if (isDirectory(fs, normalized)) {
        targets.push(
          ...fs.glob(normalized, "**/*.json").filter((p) => fs.isFile(p)),
        );
      } else {
  ...
  return iterGovernedFiles(fs, root);
}
...
export function runValidation(
  ...
  const result: ValidateResult = { failed: false, messages: [] };
