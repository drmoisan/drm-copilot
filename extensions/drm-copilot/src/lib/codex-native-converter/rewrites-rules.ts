 ...
 */
const BASE_REWRITE_RULES: ReadonlyArray<RewriteRule> = [
  {
  ...
  {
    pattern:
      /(?<![A-Za-z0-9_])\.github\/instructions\/([A-Za-z0-9_.-]+)\.instructions\.md\b/g,
    replacement: (_match, group1: string) =>
      `.agents/skills/${normalizeTargetName(group1)}/SKILL.md`,
    description:
      "Rewrite GitHub Copilot path-scoped instructions to shared skill paths.",
  },
  {
    pattern:
      /(?<![A-Za-z0-9_])\.github\/skills\/([A-Za-z0-9_.-]+)\/SKILL\.md\b/g,
    replacement: (_match, group1: string) =>
      `.agents/skills/${normalizeTargetName(group1)}/SKILL.md`,
    description:
  ...
  {
    pattern:
      /(?<![A-Za-z0-9_])\.github\/agents\/([A-Za-z0-9_.-]+)\.agent\.md\b/g,
    replacement: (_match, group1: string) =>
      `.codex/agents/${normalizeTargetName(group1)}.toml`,
    description:
  ...
  {
    pattern:
      /(?<![A-Za-z0-9_])\.claude\/skills\/([A-Za-z0-9_.-]+)\/SKILL\.md\b/g,
    replacement: (_match, group1: string) =>
      `.agents/skills/${normalizeTargetName(group1)}/SKILL.md`,
  ...
  },
  {
    ...
    replacement: (_match, group1: string) =>
      `.codex/hooks/${normalizeHookTargetName(group1)}.ps1`,
  ...
  },
  {
    ...
    description:
      "Rewrite orchestration validator command IDs to semantic MCP tool " +
      "usage.",
  },
  {
    pattern:
      /Return the finalized plan for validation-only preflight through `atomic-executor` and preserve the same target file path across revision loops\. Do not claim nested worker delegation from within planner execution\./g,
    replacement:
  ...
  },
  {
    ...
    description:
      "Rewrite PoshQC suite command IDs to the semantic MCP analyzer " +
      "surface.",
  },
  {
    pattern: /\bdrmCopilotExtension\.([A-Za-z0-9_]+)\b/g,
    replacement: (_match, group1: string) =>
      "mcp__drmCopilotExtension__" + camelOrPascalToSnake(group1),
    description:
 ...
 */
const PROMPT_SKILL_FALLBACKS: ReadonlyArray<readonly [string, string]> = [
  [
 ...
 */
export function buildRewriteRules(options: {
  readonly enableRepoPrompts: boolean;
  readonly standingGuidanceSourcePaths: ReadonlyArray<string>;
}): ReadonlyArray<RewriteRule> {
  const { enableRepoPrompts, standingGuidanceSourcePaths } = options;

  const standingGuidanceRules: RewriteRule[] = standingGuidanceSourcePaths.map(
    (sourcePath) => ({
      pattern: new RegExp(
        `(?<![A-Za-z0-9_])${escapeRegExp(sourcePath)}\\b`,
        "g",
      ),
      replacement: "AGENTS.md",
      description:
  ...
  const promptSkillFallbackRules: RewriteRule[] = [
  ...
  let promptRewriteRules: RewriteRule[] = [];
  if (enableRepoPrompts) {
    promptRewriteRules = [
      {
        pattern:
          /(?<![A-Za-z0-9_])\.github\/prompts\/([A-Za-z0-9_.-]+?)(?:\.prompt)?\.md\b/g,
        replacement: (_match, group1: string) =>
          `.codex/prompts/${normalizeTargetName(group1)}.md`,
        description:
