...
- [ ] The resource-contract parity test exempts `.claude/agent-memory/**` from byte-identical mirroring and asserts every NON-index bundled memory carries `scope: general` and every bundled `MEMORY.md` index carries `scope: repo`.
