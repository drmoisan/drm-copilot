...
exports.resolveAtomicPlanCommand = resolveAtomicPlanCommand;
const file_system_1 = require("../file-system");
const file_prompt_variables_1 = require("./file-prompt-variables");
const file_prompt_transforms_1 = require("./file-prompt-transforms");
/**
 ...
 */
function resolvePrompt(templateContent, targetPath, workspaceRoot, fs) {
    let content = (0, file_prompt_variables_1.stripFrontMatter)(templateContent);
    const relativeTarget = (0, file_prompt_variables_1.tryRelativeToWorkspace)(targetPath, workspaceRoot);
    const fileValue = (0, file_system_1.toPosixPath)(relativeTarget);
    const folderpath = (0, file_prompt_variables_1.resolveFolderpath)(targetPath, workspaceRoot);
    const featureFoldername = (0, file_prompt_variables_1.resolveFeatureFoldername)(folderpath);
    const name = (0, file_prompt_variables_1.resolveNameFromFeatureFoldername)(featureFoldername);
    const variables = {
        file: fileValue,
        folderpath,
        name,
        spec: (0, file_prompt_variables_1.resolveSpecPath)(folderpath),
        "user-story": (0, file_prompt_variables_1.resolveUserStoryValue)(folderpath, workspaceRoot, fs),
    };
    const { mode, fallbackReason } = (0, file_prompt_variables_1.resolveWorkModeFromIssue)(folderpath, workspaceRoot, fs);
    ...
    if (mode === "minor-audit") {
        content = (0, file_prompt_transforms_1.applyMinorAuditOverrides)(content);
    }
    ...
    const researchValue = (0, file_prompt_variables_1.resolveResearchValue)(folderpath, workspaceRoot, fs);
    if (researchValue === null) {
        content = (0, file_prompt_transforms_1.removeLinesReferencingVariable)(content, "research");
    }
    else {
    ...
    if ((variables["user-story"] ?? "").includes("(missing)")) {
        content = (0, file_prompt_transforms_1.removeUserStoryClauseWhenMissing)(content);
    }
    return (0, file_prompt_transforms_1.replaceAllVariables)(content, variables);
}
/**
 ...
 */
function resolveAtomicPlanCommand(input) {
    const emit = input.log ?? (() => undefined);
    const copyToClipboard = input.copyToClipboard ?? (() => false);
    ...
    let resolved;
    try {
        resolved = resolvePrompt(templateContent, input.targetPath, input.workspaceRoot, input.fs);
    }
    catch (error) {
