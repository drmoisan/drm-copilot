...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-18 and AC-19.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 316 a crlf target receives a crlf terminated index line
    ok 317 a mixed line ending target is refused and reported as EOL-MIXED
