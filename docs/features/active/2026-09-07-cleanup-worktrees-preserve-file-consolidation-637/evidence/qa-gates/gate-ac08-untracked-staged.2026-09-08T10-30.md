...
EXIT_CODE: 0
Output Summary: DISCHARGED by CI round C, run 34219866134, conclusion success. TAP plan line `1..375`; 375 passing; the run carried zero `not ok` lines. Satisfies AC-08 and completes the pass-after half of the fail-before record.
...
Verbatim TAP `ok` line for each test this gate names, read from the run log:
    ...
    ok 288 an untracked preserve record is staged and reported
