...
/**
 * Tree-backed in-memory `FileSystem` for codex-native-converter tests.
 *
 * Purpose:
 *     Provide a hermetic filesystem fake that supports the directory-aware
 *     predicates the converter relies on (`exists`, `isDirectory`,
 *     `listDirectory`, `glob`, `isFile`) plus UTF-8 text read/write and
 *     `ensureDir`. Directories are tracked explicitly; adding a file implicitly
 *     registers its ancestor directories so the tree predicates behave like a
 *     real filesystem.
 *
 * Path convention:
