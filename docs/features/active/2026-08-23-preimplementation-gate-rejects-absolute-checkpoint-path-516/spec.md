...
- [x] `Get-FileHash` reports identical SHA256 values for `.codex/hooks/enforce-orchestration-preimplementation-gate.ps1` and `extensions/drm-copilot/resources/codex-and-agents-customizations/.codex/hooks/enforce-orchestration-preimplementation-gate.ps1`, and the Codex pair lands in a single commit so the byte-identity assertion in `legacy-codex-hook-contracts.Tests.ps1` never observes a split state.
  - Evidence: `evidence/qa-gates/four-copy-parity-hashes.2026-08-23T23-25.md` records both Codex copies at `98DC6917AE5AE3239DBE89C31391960D260AB74B83A51D93FA9D575AA16DBABD`, and both were made byte-identical in the working tree before any commit existed. The single-commit half lies outside plan execution, so the executor left it unticked; `feature-review` confirmed it independently with `git show --name-only`, which shows `9c12d20a` touched documentation only and `b50f4e28` carried both Codex copies together, so no commit ever contains a split state.
